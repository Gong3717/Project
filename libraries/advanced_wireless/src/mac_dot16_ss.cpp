                                  ...
                                  int newDuration)
{
        ...
        > dot16Ss->ulDurationInPs)
    {
        ...
        // next subchannel
        burst->startTime = getSimTime(node);
    }
    else
    {
    ...
    ERROR_Assert(burst->startTime >= getSimTime(node) &&
        ...
        for (j = 0; j < DOT16_NUM_SERVICE_TYPES && bwGranted > 0; j ++)
        {
            ...
            while (sFlow != NULL && bwGranted > 0)
            {
                    ...
                    sFlow->serviceType == (MacDot16ServiceType)i)
                {
                    ...
                    sFlow->lastAllocTime = getSimTime(node);
                }
                sFlow = sFlow->next;
            } // while
        } // for i
...
//            }
            ...
            {
                if (sFlow->activated &&
                    (getSimTime(node) - sFlow->lastAllocTime) >=
                            ...
                            MacDataDot16* dot16)
{
    ...
    //add duration of current frame
        unavlblInt += getSimTime(node) - dot16Ss->servingBs->frameStartTime;
    return unavlblInt;
}
                             ...
                             clocktype rxBeginTime)
{
    ...
    timePassed = getSimTime(node) - rxBeginTime;
    dot16Ss->ulDurationInPs = (int)((dot16Ss->servingBs->frameDuration -
                               allocStartTime - dot16Ss->para.rtg) /
                               dot16->ulPsDuration);
    dot16Ss->ulStartTime = getSimTime(node) + allocStartTime -
                           timePassed;
    if (dot16Ss->ulStartTime < getSimTime(node))
    {
        dot16Ss->ulStartTime = getSimTime(node);
    }
                              ...
                              int freqOffsetAdjust)
{
    ...
    // At present QualNet does not support the max tx power for a node on phy
    // due to this constraint node continue increase our tx power
    if (powLevelAdjust != 0)
    {
                              ...
                              int startIndex)
{
        ...
        > getSimTime(node))
    {
        ...
        dot16Ss->servingBs->lastReportTime = getSimTime(node);
                              ...
                              TraceProtocolType appType)
{
    ...
    // increase statistics
    dot16Ss->stats.numDsaReqRcvd ++;
    dot16Ss->lastCSPacketRcvd = getSimTime(node);
    index = startIndex;
    ...
    for (i = 0; i < DOT16_NUM_SERVICE_TYPES; i ++)
    {
        ...
        while (sFlow != NULL)
        {
            if (sFlow->isEmpty == FALSE &&
                dot16Ss->outScheduler[i]->isEmpty(sFlow->queuePriority))
            {
                sFlow->emptyBeginTime = getSimTime(node);
                sFlow->isEmpty = TRUE;
            }
            sFlow = sFlow->next;
        }
                                   ...
                                   PhySignalMeasurement* signalMea)
{
        ...
        dot16Ss->operational)
    {
        ...
        if (dot16Ss->targetBs != NULL)
        {
            ...
            dot16Ss->targetBs->lastMeaTime = getSimTime(node);
    ...
    else
    {
        if (dot16Ss->initInfo.dlSynchronized)
        {
            ...
            dot16Ss->servingBs->lastMeaTime = getSimTime(node);
    ...
    else if (dot16Ss->nbrScanStatus == DOT16e_SS_NBR_SCAN_InScan)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                dot16Ss->nbrScanStatus = DOT16e_SS_NBR_SCAN_None;
                dot16Ss->lastScanTime = getSimTime(node);
        ...
        case MSG_MAC_TimerExpired:
        {
            ...
            switch (timerType)
            {
                ...
                case DOT16_TIMER_FrameEnd:
                {
                    ...
                    {
                        if ((getSimTime(node) - dot16Ss->lastCSPacketRcvd) >=
                                    ...
                                    Message* msg)
{
    ...
    clocktype rxBeginTime = getSimTime(node);
