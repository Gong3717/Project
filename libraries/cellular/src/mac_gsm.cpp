    ...
    MacDataGsm* gsm)
{
    ...
    schMsg->startTimeOfFrame = getSimTime( node );
    ...
    GsmBcchListEntry** retBcchInfo)
{
    ...
    for (i = 0; i < msInfo->numBcchChannels; i++ ){
        ...
        if (bcchInfo->numRecorded >= 5 &&
            ...
            GSM_BCCH_REFRESH_PERIOD ) > getSimTime( node ) ){
    ...
    MacDataGsm* gsm)
{
    ...
    if (msInfo->isCellSelected == FALSE ||
        ...
        if (MacGsmPerformCellSelection( node,
            ...
            msInfo->timeSinceCellSelected = getSimTime( node );
    ...
    GsmBcchListEntry** retBcchEntry)
{
    ...
    for (i = 0; i < msInfo->numBcchChannels; i++ ){
        ...
        if (bcchInfo->channelIndex == channelIndex ){
            ...
            bcchInfo->ber += ber;
            bcchInfo->lastUpdatedAt = getSimTime( node );
    ...
    double          ber)
{
    ...
    msInfo->numAssignedChMeasurements++;
    msInfo->assignedChMeasLastUpdatedAt = getSimTime( node );
    ...
    double          ber)
{
    ...
    channelMeas->lastUpdatedAt = getSimTime( node );
    ...
    if (channelString[i] == '[' && channelString[strLength - 1] == ']' ){
        ...
        while (*next != '\0'){
            ...
            if (IO_IsStringNonNegativeInteger( key ) ){
                controlChannels[i++] = ( short )strtoul( key,NULL,10 );
            }
            else{
                sprintf( errorString,
                    "GSM-CONTROL-CHANNEL has an"
                        "invalid value, %s\n",
                    channelString );
        ...
        sprintf( errorString,
            "GSM-CONTROL-CHANNEL is incorrectly "
                "formatted, %s\n",
            channelString );
        ERROR_ReportWarning( errorString );
    }
    ...
    const  int numNodesInSubnet)
{
    ...
    if (strcmp( varStrValue,"GSM-MS" ) == 0 ){
        ...
        msInfo->numControlChannels = GsmParseChannelList( varStrValue,
        ...
        numControlChannels = GsmParseChannelList( varStrValue,
                                 controlChannels );
                    ...
                    if (msInfo->assignedChannelType == GSM_CHANNEL_SDCCH ){
                        ...
                        if (msInfo->slotNumber ==
                            ...
                            gsm->idleTimerMsg = MacGsmStartTimer( node,
                                ...
                                (
                                getSimTime(
                                node ) - msInfo->currentSlotStartTime ) );
                            // Bug # 3436 fix - end
                        }
                    }
            ...
            case GSM_L3_MAC_HANDOVER:
                {
                    ...
                    MacGsmStartTimer( node,
                        ...
                        startTimeOfNextFrame - getSimTime( node ) );
                    //- 1 * MICRO_SECOND);
