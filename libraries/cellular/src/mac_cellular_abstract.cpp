    ...
    if (!PHY_IsListeningToChannel(node, phyNumber, channelIndex))
    {
        if (DEBUG_CHANNEL)
        {
            printf("at %015" TYPES_64BITFMT "d Node%u starts listening"
                   "to channel %d\n",
                    getSimTime(node), node->nodeId, channelIndex);
            fflush(stdout);
        }
                                      ...
                                      Message *msg)
{
    ...
    if(DEBUG_PROGRESS)
    {
        printf("at %015" TYPES_64BITFMT "d node %d TCH trans to dest %d:"
               ...
               getSimTime(node), node->nodeId,  headerPtr->destId,
                                                ...
                                                short  *sectorId)
{
    ...
    *sectorId = CELLULAR_ABSTRACT_INVALID_SECTOR_ID;
    ...
    if (DEBUG_MAC)
    {
        printf("The BS to MS angle is %d numSector is %d so the "
               "sectorId is %d\n",
                                          ...
                                          Message *msg)
{
    ...
    if (macCellularAbstractData->downlinkMeasurement == NULL)
    {
        ...
        newMeas->measurementTime = TIME_getSimTime(node);
    ...
    else
    {
        ...
        if (found == TRUE)
        {
                ...
                (currentMeas->monitoredSectorId != sectorId))
            {
                ...
                currentMeas->measurementTime = TIME_getSimTime(node);
                             ...
                             const NodeInput* nodeInput)
{
    ...
    if (macCellularData->nodeType == CELLULAR_BS)
    {
        ...
        if (!retVal || macData->currentDLChannel < 0 ||
            macData->currentDLChannel >= node->numberChannels)
        {
            ERROR_ReportError("Wrong downlink control channel!");
        }
        ...
        if (!retVal || macData->currentULChannel < 0 ||
            macData->currentULChannel >= node->numberChannels)
        {
            ERROR_ReportError("Wrong uplink control channel!");
        }
...
void MacCellularAbstractReceiveTCH(
    ...
    else
    {
        ...
        MESSAGE_Send(node,
                     msg,
                     msg->eventTime - node->partitionData->theCurrentTime);
#endif
    ...
    clocktype delay)
{
    ...
    splayNode->timeValue = getSimTime(node) + delay;
    ...
    if (newTime)
    {
        ...
        newMsg->eventTime = getSimTime(node) + delay;
    ...
    int interfaceIndex)
{
    ...
    while (tchSplayTree->timeValue == getSimTime(node))
    {
