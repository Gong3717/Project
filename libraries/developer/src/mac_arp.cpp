                                ...
                                int type)
{
    ...
    switch (type)
    {
        case DISCARD:
        {
            ...
            clocktype curr_time = TIME_getSimTime(node);
             ...
             int interfaceIndex)
{
    ...
    {
        char clockStr[MAX_STRING_LENGTH] = {0};
        TIME_PrintClockInSecond(TIME_getSimTime(node), clockStr);
        char addrstr[MAX_STRING_LENGTH] = {0};
         ...
         int interfaceIndex)
{
    ...
    {
        arpData->stats.arppacketdropped++;
         ...
         BOOL entryType)
{
   ...
   else
   {
        ...
        currentTime = TIME_getSimTime(node);
   ...
   arpData->stats.totalCacheEntryCreated += 1;

   if (ARP_CACHE_DEBUG)
   {
        char clock[MAX_STRING_LENGTH] = {0};
        TIME_PrintClockInSecond(TIME_getSimTime(node), clock);
         ...
         clocktype currentTime)
{
    ...
    else
    {
        ...
        arpData->stats.totalCacheEntryUpdated += 1;
         ...
         int interfaceIndex)
{
   ...
   arpData->stats.totalReplyRecvd += 1;
}
         ...
         int interfaceIndex)
{
    ...
    if (packet->hardType == hwType)
    {
        ...
        if (ArpIsSupportedProtocol(packet->protoType))
        {
            ...
            clocktype currentTime = TIME_getSimTime(node);
            margeFlag = FALSE;
         ...
         int networkType)
{
    ...
    if (arpTTableElement
          ...
          && (arpTTableElement->expireTime <= currentTime))
     {
        ...
        // Update ARP cache stats
        arpData->stats.totalCacheEntryAgedout += 1;
        arpData->stats.totalCacheEntryDeleted += 1;
    }
        ...
        if (ArpIsEnable (node, i))
        {
            ...
            while (listItem)
            {
                    ...
                    (currentTime - db->sentTime) >= SECOND)
                {
                    ...
                    {
                        char clockStr[MAX_STRING_LENGTH] = {0};
                        TIME_PrintClockInSecond(TIME_getSimTime(node),
                                                clockStr);
                        char addrstr[MAX_STRING_LENGTH] = {0};
                        ...
                        (currentTime - db->sentTime) >= SECOND)
               {
                   ...
                   db->RetryCount--;
                   db->sentTime = TIME_getSimTime(node);
                   need_retry = TRUE;
                   listItem = listItem->next;
               }
               else
...
void ArpFinalize(Node* node)
{
    ...
    tempCacheDeleted = arpData->stats.totalCacheEntryDeleted;
    ...
    arpData->stats.totalCacheEntryDeleted =  tempCacheDeleted;

    if (arpData->isArpStatEnable)
    {

        sprintf(buf, "Request Sent = %d", arpData->stats.totalReqSend);
        IO_PrintStat(
            node,
            "Network",
            "ARP",
        ...
        sprintf(buf, "Request Received = %d", arpData->stats.totalReqRecvd);
        IO_PrintStat(
            node,
            "Network",
            "ARP",
        ...
        sprintf(buf, "Reply Sent = %d", arpData->stats.totalReplySend);
        IO_PrintStat(
            node,
            "Network",
            "ARP",
        ...
        sprintf(buf, "Reply Received = %d", arpData->stats.totalReplyRecvd);
        IO_PrintStat(
            node,
            "Network",
            "ARP",
        ...
        sprintf(buf, "Data Packet Discarded = %d",
                                             arpData->stats.numPktDiscarded);
        IO_PrintStat(
            node,
            "Network",
        ...
        sprintf(buf, "Entry Inserted = %d",
            arpData->stats.totalCacheEntryCreated);
        IO_PrintStat(
            node,
            "Network",
            "ARP Cache",
        ...
        sprintf(buf, "Entry Updated = %d",
            arpData->stats.totalCacheEntryUpdated);
        IO_PrintStat(
            node,
            "Network",
            "ARP Cache",
        ...
        sprintf(buf, "Entry Agedout = %d",
            arpData->stats.totalCacheEntryAgedout);
        IO_PrintStat(
            node,
            "Network",
            "ARP Cache",
        ...
        sprintf(buf, "Entry Deleted = %d",
            arpData->stats.totalCacheEntryDeleted);
        IO_PrintStat(
            node,
            "Network",
            "ARP Cache",
         ...
         sprintf(buf, "Gratuitous Request Sent = %d",
            arpData->stats.totalGratReqSend);
        IO_PrintStat(
            node,
            "Network",
            "ARP",
            ...
            buf);
#if 0
        //Statistics for Proxy ARP reply
        sprintf(buf, "Proxy Reply Sent = %d",
            arpData->stats.totalProxyReply);
        IO_PrintStat(
            node,
            "Network",
            "ARP",
        ...
        //Statistics for Gratuitous ARP Request


        /*sprintf(buf, "Entry Flushed = %d",
            arpData->stats.totalCacheEntryFlushed);
        IO_PrintStat(
            node,
            "Network",
            "ARP Cache",
