...
static char rootDirectory[MAX_STRING_LENGTH];
#define MAX_METHOD  3
 ...
 */
void
AppEHttpServerInit(Node *node, Address serverAddr, char *baseDir)
{
...
void AppEHttpServerErrorHandle(Node * node, AppDataEHttpServer * serverPtr, char * path, int eIndex)
{
    ...
    sprintf(buf, "Server: My_Httpd\r\n");
    sprintf(buf, "%sContent-Type: text/html\r\n", buf);
    sprintf(buf, "%s\r\n", buf);
    AppEHttpServerMessageToClient(node, serverPtr, buf, strlen(buf));
    sprintf(buf, "<HEAD><TITLE>404 Not Found</TITLE></HEAD>\r\n");
    sprintf(buf, "%s<BODY><H1>404 Not Found</H1>\r\n", buf);
    sprintf(buf, "%sThe requested URL %s was not found on this server.\r\n", buf, path);
    sprintf(buf, "%s</BODY>\r\n", buf);
    AppEHttpServerMessageToClient(node, serverPtr, buf, strlen(buf));
...
void AppEHttpServerHandleRequest(Node * node, AppDataEHttpServer * serverPtr, char * request, int requestLen)
{
    ...
    strcpy(mBasedir, rootDirectory);
    sprintf(path, "%s%s", mBasedir, serverPtr->reqUri);
    ...
    if((stat(path, &fileStat)!=0) &&(strcmp(serverPtr->reqUri, "/serverinfo.html")) )
    {
        AppEHttpServerErrorHandle(node, serverPtr, path, 1);   // 404 Not found error
    ...
    switch (serverPtr->methodReturn) {
        case GET:   // GET method
            ...
            // send contents
            if( AppEHttpServerFileSend(node, serverPtr, path, fileStat.st_size) == -1)
...
int AppEHttpServerFileSend(Node * node, AppDataEHttpServer * serverPtr, char * toSend, int fileSize)
{
    char *buffer;
    FILE * fp;
    int     i;
...
void AppEHttpServerStatsHtml(Node *node, AppDataEHttpServer * serverPtr)
{
    ...
    sprintf(tempBuf, "<p>Base directory  = %s</p>", rootDirectory);
    //AppEHttpServerMessageToClient(node, serverPtr, buf, strlen(buf));
    strcat(buf, tempBuf);
    bytesToSend += strlen(tempBuf);
 ...
 * RETURN:      void.
 */
void AppEHttpServerHeaderSend(Node * node, AppDataEHttpServer *serverPtr,  char *filename, int keepAlive, int fileSize)
{
    char buf[MAX_STRING_LENGTH*8];
    char filetype[MAX_STRING_LENGTH];
 ...
 */
void
AppEHttpServerMessageToClient(Node *node, AppDataEHttpServer *serverPtr, char * output, int length)
{
