...
static char rootFilesystem[MAX_STRING_LENGTH];
...
{
    AppEFtpServerCommandUnknown(node, ftpServer, "PASV");
}
...
{
    AppEFtpServerCommandUnknown(node, ftpServer, "AUTH");
}
                         ...
                         Address remoteAddr)
{
    ...
    ftpClient = (AppDataEFtpClient *)
                MEM_malloc(sizeof(AppDataEFtpClient));

    memset(ftpClient, 0, sizeof(AppDataEFtpClient));
...
AppLayerEFtpServerData(Node *node, Message *msg)
{
    ...
    switch (msg->eventType)
    {
        ...
        case MSG_APP_FromTransDataSent:
        {
            ...
            serverDataPtr->lastItemSent = getSimTime(node);
        ...
        case MSG_APP_FromTransCloseResult:
        {
            ...
            if (serverDataPtr->sessionIsClosed == FALSE)
            {
                ...
                serverDataPtr->sessionFinish = getSimTime(node);
            }
...
void AppEFtpServerDataPrtFileInfo(Node *node,  AppDataEFtpClient *clientPtr, char * fileName, char * returnStr)
{
    ...
    sprintf(absPath, "%s/%s", rootFilesystem, fileName);

    struct stat fileStat;
    if (stat(absPath, &fileStat) ==0)
    {
        if (S_ISREG(fileStat.st_mode)) //regular files only
        {
            ...
            sprintf(returnStr, "%s   %d qualnet qualnet %7d %s %s", temp, fileStat.st_nlink, fileStat.st_size, buff, fileName);
...
void AppEFtpServerSendFile(Node *node, AppDataEFtpClient *clientPtr)
{
    ...
    ifstream readFile;                                                                 
    readFile.open(clientPtr->fileToSend, ios::in|ios::binary);                                       
...
void AppEFtpServerDataExecuteCommand(Node *node, AppDataEFtpClient *clientPtr)
{
    ...
    if (( !memcmp(clientPtr->cmd, "LIST", 4)) || ( !memcmp(clientPtr->cmd, "NLST", 4)) )
    {
        ...
        int n,i;
        n = scandir(rootFilesystem, &namelist, 0, alphasort);
        if (n < 0)
            perror("scandir");
        else {
            while (n--)
    ...
    if (!memcmp(clientPtr->cmd, "RETR", 4))
    {
        ...
        if (isFileAccessMode) //if File Access is enabled
        {
            sprintf(clientPtr->fileToSend, "%s/%s", rootFilesystem, fileName);
#ifndef _WIN32  // linux/unix
            struct stat fileStat;

            if (stat(clientPtr->fileToSend, &fileStat) ==0)
            {
                //set the item size
                clientPtr->sendPtr = 0;
            ...
            long hFile;
            struct _finddata_t c_file;
            hFile = _findfirst(clientPtr->fileToSend, &c_file);
            if (hFile != -1L)
            {
                clientPtr->sendPtr = 0;
...
int AppEFtpServerAvailableFile(Node *node, char * fileName)
{
    ...
    if (isFileAccessMode) //if File Access is enabled
    {
        ...
        if ((hFile = _findfirst(absPath, &c_file)) == -1L)
        {
            return 0;
        }
        else
