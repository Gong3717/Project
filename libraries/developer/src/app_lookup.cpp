    ...
    TosType tos)
{
    ...
    lookupClient->sessionStart = getSimTime(node) + startTime;
    lookupClient->sessionIsClosed = FALSE;
    lookupClient->sessionLastSent = getSimTime(node);
    lookupClient->sessionLastReceived = getSimTime(node);
    lookupClient->sessionFinish = getSimTime(node);
    ...
    Message *msg)
{
    ...
    switch (msg->eventType)
    {
            ...
            switch (timer->type)
            {
                case APP_TIMER_SEND_PKT:
                {
                    ...
                    if (((clientPtr->requestsToSend > 1) &&
                         (getSimTime(node) + clientPtr->requestInterval
                             < clientPtr->endTime)) ||
        ...
        case MSG_APP_FromTransport:
        {
            ...
            clientPtr->sessionLastReceived = getSimTime(node);
            clientPtr->totalEndToEndDelay += getSimTime(node) - data.txTime;
    ...
    short sourcePort)
{
    ...
    lookupServer->sessionStart = getSimTime(node);
    lookupServer->sessionFinish = getSimTime(node);
    lookupServer->sessionLastReceived = getSimTime(node);
    ...
    Message *msg)
{
    ...
    switch (msg->eventType)
    {
        case MSG_APP_FromTransport:
        {
            ...
            {
                char clockStr[24];
                ctoa((getSimTime(node) + data.replyDelay), clockStr);
