                               ...
                               int intIndex)
{
    ...
    IpInterfaceInfoType* borroewdinterfaceInfo = ip->interfaceInfo[intIndex];
    ...
    if (tunnelInfo->tunnelType == IPv6_TUNNEL)
    {
        ...
        ip->interfaceInfo[interfaceIndex]->ipAddress =
            borroewdinterfaceInfo->ipAddress;

        ip->interfaceInfo[interfaceIndex]->multicastEnabled =
            borroewdinterfaceInfo->multicastEnabled;
    ...
    else if (tunnelInfo->tunnelType == IPv4_TUNNEL)
    {
        ...
        ipv6InterfaceInfo->globalAddr =
            borroewdinterfaceInfo->ipv6InterfaceInfo->globalAddr;
        ipv6InterfaceInfo->prefixLen = 0;
        ipv6InterfaceInfo->linkLocalAddr =
            borroewdinterfaceInfo->ipv6InterfaceInfo->linkLocalAddr;
        ipv6InterfaceInfo->multicastAddr =
            borroewdinterfaceInfo->ipv6InterfaceInfo->multicastAddr;
        ipv6InterfaceInfo->multicastEnabled =
            borroewdinterfaceInfo->ipv6InterfaceInfo->multicastEnabled;
        ipv6InterfaceInfo->mtu =
            borroewdinterfaceInfo->ipv6InterfaceInfo->mtu;
    }
            ...
            nodeInput->variableNames[i], "IPv6-ENABLE-6to4-TUNNELING") == 0)
        {
            if (strcmp(nodeInput->values[i], "YES") == 0)
            {
                ...
                if (isNodeId)
                {
                    ...
                    sprintf(buf, "IPv6-ENABLE-6to4-TUNNELING parameter must"
                        "have an IP address as its qualifier.\n");
                ...
                if (node->nodeId == nodeId)
                {
                    ...
                    SetIPv4AddressInfo(
                        &tunneldata->tunnelStartAddress, InterfaceAddress);
                    SetIPv4AddressInfo(
                        &tunneldata->tunnelEndAddress, ANY_ADDRESS);
...
void TunnelSetRoutingProtocolType(Node* node, int interfaceIndex)
{
    ...
    if (tunneldata ->tunnelType == IPv6_TUNNEL)
    {
        ip->interfaceInfo[interfaceIndex]->routingProtocolType =
                                borroewdinterfaceInfo->routingProtocolType;

        ip->interfaceInfo[interfaceIndex]->multicastProtocolType =
                                borroewdinterfaceInfo->multicastProtocolType;
    }
    else if (tunneldata ->tunnelType == IPv4_TUNNEL)
    {
        ...
        interfaceInfo2 = borroewdinterfaceInfo->ipv6InterfaceInfo;
