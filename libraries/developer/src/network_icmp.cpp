...
static void NetworkIcmpReadHostOrRouter(Node *node,
                     ...
                     const NodeInput *nodeInput)
{
    ...
    NetworkIcmpReadHostOrRouter(node, nodeInput);
...
/*
 * FUNCTION:   NetworkIcmpReadHostOrRouter()
 * PURPOSE:    This function is called from  IcmpInit() function to read
 *             value of the string "ICMP-ROUTER" from input file (*.config)
 *             and determine whether the node is host or router.
 * RETURN:     NULL
 * ASSUMPTION: None
 * PARAMETERS: node,          node which is allocating message
 *             nodeInput,     Structure containing contents of input file.
                                        ...
                                        const NodeInput *nodeInput)
{
    ...
    for (index = 0; index < nodeInput->numLines; index++)
    {
        if (strcmp(nodeInput->variableNames[index], "ICMP-ROUTER-LIST") == 0)
        {
            ...
            strcpy(icmpRouterString, nodeInput->values[index]);
            p = icmpRouterString;
            p = strchr(p, '{');
            ...
            if (p == NULL)
            {
                ...
                sprintf(str, "Could not find '{' character:\n in %s\n",
                                                    icmpRouterString);
            ...
            else
            {
                ...
                {
                    char str[MAX_STRING_LENGTH];
                    sprintf(str, "Can't find nodeId list,"
                        " e.g. { 1, 2, ... }:in \n  %s\n",icmpRouterString);
                    ERROR_Assert(FALSE, str);
                }
                else
