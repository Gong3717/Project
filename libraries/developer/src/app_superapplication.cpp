...
static void SuperApplicationServerScheduleNextRepPkt_UDP(Node* node,SuperApplicationData* serverPtr,clocktype interval,Message* msg) {
    ...
    if (SUPERAPPLICATION_DEBUG_TIMER_SET_SERVER)
    {
        ...
        TIME_PrintClockInSecond((interval + getSimTime(node)), clockStr, node);
...
static void SuperApplicationClientSendData_TCP(Node* node,SuperApplicationData* clientPtr,Message* msg) {
        ...
        (clientPtr->totalReqPacketsToSend > 0) )
    {
        ...
        {
            clientPtr->stats.appStats->SessionFinish(node);
        }
    }
    ...
    Message * tempMsg = APP_TcpSendData(
        node,
        ...
        clientPtr->sessionId,
        reqSize,
        clientPtr->requiredTput,
        clientPtr->requiredDelay,
        dataRate
#ifdef ADDON_DB
        ,
        &appParam,
    ...
    // Statistics update
    clientPtr->lastReqFragSentTime = getSimTime(node);
    ...
    if (clientPtr->stats.numReqPacketsSent == 1)
        clientPtr->firstReqFragSentTime = getSimTime(node);
    ...
    //clientPtr->packetSentTime = getSimTime(node);
...
static void SuperApplicationClientSendData_UDP(Node* node,SuperApplicationData* clientPtr,Message* msg) {
    ...
    if ((clientPtr->isVoice) && (clientPtr->isJustMyTurnToTalk))
    {
        ...
        clientPtr->firstTalkTime = getSimTime(node);
    }
    ...
    if (((clientPtr->stats.numReqPacketsSent + 1 == 
        ...
        // Session is over
        ((clientPtr->sessionFinish != clientPtr->sessionStart)
         && (getSimTime(node) + reqInterval >= clientPtr->sessionFinish)))
    {
        data.isLastPacket = TRUE;
        if (!clientPtr->processReply && node->appData.appStats)
        {
    ...
    while (reqSize > 0)
    {
        ...
        if (clientPtr->isVoice == FALSE)
        {
            tempMsg = APP_UdpSendNewHeaderVirtualDataWithPriority(
                node,
                clientPtr->clientAddr,
        ...
        if (node->appData.appStats)
        {
            ...
            if (!(clientPtr->stats.appStats->IsSessionStarted(STAT_Unicast))
                &&
                !(clientPtr->stats.appStats->IsSessionStarted(STAT_Multicast)))
            {
                clientPtr->firstReqFragSentTime =
                    clientPtr->lastReqFragSentTime;
                if (NetworkIpIsMulticastAddress(node,clientPtr->serverAddr))
                {
                    clientPtr->stats.appStats->SessionStart(node,STAT_Multicast);
                }
                else
                {
                    clientPtr->stats.appStats->SessionStart(node,STAT_Unicast);
                }
            }
            if (NetworkIpIsMulticastAddress(node,clientPtr->serverAddr))
            {
                clientPtr->stats.appStats->AddFragmentSentDataPoints(node,
                                                            fragSize,
                                                            STAT_Multicast);
                clientPtr->stats.appStats->AddTiming(node,
                                                     tempMsg,
                                                     fragSize,
                                                     0);
                if (data.fragNo == 0)
    ...
    clientPtr->lastReqPacketSentTime = getSimTime(node);
         ...
         && getSimTime(node) + reqInterval >= clientPtr->sessionFinish))
    {
        ...
        {
            char clockStr[24];
            TIME_PrintClockInSecond(reqInterval+getSimTime(node), clockStr, node);
...
static void SuperApplicationServerSendData_UDP(Node* node,SuperApplicationData* serverPtr,Message* msg) {
    ...
    if ((serverPtr->isVoice) && (serverPtr->isJustMyTurnToTalk)){
        ...
        serverPtr->firstTalkTime = getSimTime(node);
    }
    ...
    while (repSize > 0)
    {
        ...
        if (serverPtr->isVoice == FALSE) {
            tempMsg = APP_UdpSendNewHeaderVirtualDataWithPriority(
                ...
                0,
                TRACE_SUPERAPPLICATION,
                serverPtr->applicationName,
                &data,
                serverPtr->isMdpEnabled,
        ...
        serverPtr->lastRepFragSentTime = getSimTime(node);
...
static void SuperApplicationScheduleNextReply_TCP(Node* node,SuperApplicationData* serverPtr, clocktype delay) {
    ...
    if (SUPERAPPLICATION_DEBUG_TIMER_SET_SERVER)
    {
        ...
        printf("    Reply for  : %u req seq no \n",
               replyPacketTimer->reqSeqNo);
        TIME_PrintClockInSecond((delay + getSimTime(node)), clockStr, node);
        printf("    Alarm time :  %s sec\n", clockStr);
...
static void SuperApplicationClientReceivePacket_UDP(Node* node,Message* msg,SuperApplicationData* clientPtr) {
    ...
    clientPtr->totalEndToEndDelayForRep += (double)
        (getSimTime(node) - data->txTime) / SECOND;
    // Calculate jitter
    ...
    clientPtr->lastRepFragRcvTime = getSimTime(node);
            ...
            || data->isMdpEnabled)
    {
        ...
        if (clientPtr->repFragNo == data->totFrag - 1)
        {
            ...
            clocktype packetDelay = getSimTime(node) - data->txTime;
            ...
            clientPtr->lastRepPacketRcvTime = getSimTime(node);
            if (clientPtr->stats.numRepPacketsRcvd == 1) {
                   ...
                   while (it != clientPtr->fragCache.end())
                   {
                       ...
                       if (data->seqNo > clientPtr->repSeqNo)
                       {
                           ...
                           else if (clientPtr->prevMsgRecvdComplete == FALSE)
                           {
                               ...
                               HandleStatsDBAppDropEventsInsertion(node,fragInfo.fragMsg,
                                   "AppDrop",fragInfo.fragDelay, fragInfo.fragSize, "Not Enough Fragments Received For Reassembly");
               ...
               else
               {
                   if (clientPtr->prevMsg != NULL)
                   {
                       if (data->seqNo > clientPtr->repSeqNo)
                       {
                           ...
                           {
                               HandleStatsDBAppDropEventsInsertion(node,clientPtr->prevMsg,
                                   "AppDrop",-1, clientPtr->prevMsgSize, "Not Enough Fragments Received For Reassembly");
                           }
                       }
...
static void SuperApplicationServerProcessFragRcvd_TCP(Node* node,SuperApplicationData* serverPtr,SuperApplicationTCPDataPacket *data) {
    if (serverPtr->stats.numReqFragsRcvd == 0)
    {
        ...
        NodeInput appInput;
        BOOL retVal;
        serverPtr->firstReqFragRcvTime = getSimTime(node);
    ...
    serverPtr->lastReqFragRcvTime = getSimTime(node);
...
// Make sure whether the below code section is really required for ADDON_DB
            ...
            clocktype delay = getSimTime(node) - header->txTime;
#ifdef ADDON_BOEINGFCS
    ...
    while (totalBytesRead > 0 && (completePacketRvd == TRUE))
    {
        ...
        if (totalBytesRead >= header->totalPacketBytes)
        {
            ...
            clocktype packetDelay = getSimTime(node) - header->txTime;
            ...
            packetDelay = getSimTime(node) - sent;
#endif
#ifdef ADDON_DB
            StatsDb* db = node->partitionData->statsDb;
            if (db != NULL)
            {
                ...
                {
                    packetDelay = getSimTime(node) - appParamInfo->m_PktCreationTime;
                }
            }
#endif
            ...
            if (serverPtr->stats.numReqPacketBytesRcvd == 0) {
                serverPtr->firstReqPacketRcvTime = getSimTime(node);
...
// Make sure whether the below code section is really required for ADDON_DB
            ...
            serverPtr->lastReqPacketRcvTime = getSimTime(node);
...
#ifdef ADDON_BOEINGFCS
            if (getSimTime(node) - header->txTime > 0
...
static void SuperApplicationServerReceivePacket_UDP(Node* node,Message* msg,SuperApplicationData* serverPtr) {
    ...
    clocktype packetDelay = getSimTime(node) - serverPtr->packetSentTime;
    ...
    delay = getSimTime(node) - sent;
    if (serverPtr->fragNo == 0) {
        serverPtr->packetSentTime = sent;
    }
    packetDelay = getSimTime(node) - serverPtr->packetSentTime;
#endif
    ...
    {
        clocktype curr = getSimTime(node);
        serverPtr->maxEndToEndDelayForReq = MAX(delay , serverPtr->maxEndToEndDelayForReq);
        serverPtr->minEndToEndDelayForReq = MIN(delay , serverPtr->minEndToEndDelayForReq);
        serverPtr->totalEndToEndDelayForReq += (double)delay / SECOND;
    ...
    else
    {
        ...
        serverPtr->lastReqFragRcvTimeDP = getSimTime(node);
    }
        ...
        || data->isMdpEnabled)
    {
        ...
        if (serverPtr->fragNo == data->totFrag - 1)
        {
                ...
                if (node->partitionData->statsDb->statsAppEvents->recordFragment)
                {
                    ...
                    while (it != serverPtr->fragCache.end())
                    {
                        ...
                        if (data->seqNo > serverPtr->seqNo)
                        {
                            ...
                            else if (serverPtr->prevMsgRecvdComplete == FALSE)
                            {
                ...
                else if (!node->partitionData->statsDb->statsAppEvents->recordFragment)
                {
                    if (serverPtr->prevMsg != NULL)
                    {
                        if (data->seqNo > serverPtr->seqNo)
                        {
                            ...
                            {
                                HandleStatsDBAppDropEventsInsertion(node,serverPtr->prevMsg,
                                    "AppDrop",-1, serverPtr->prevMsgSize, "Not Enough Fragments Received For Reassembly");
                            }
                        }
            ...
            serverPtr->fragNo = 0;
            // Subtract the fragments received in order
            if (data->fragNo > serverPtr->fragNo)
            {
            ...
            else
            {
                ...
                serverPtr->lastReqPacketRcvTimeDP = getSimTime(node);
            ...
            packetDelay = getSimTime(node) - data->txTime;
            serverPtr->totalEndToEndDelayForReqPacket +=
                (double)packetDelay / SECOND;
            serverPtr->lastReqPacketRcvTime = getSimTime(node);
            if ((Float64)(serverPtr->stats.numReqPacketBytesRcvd) == 0) {
...
static void enterSuperApplicationClient_ConnectionClosed_TCP(Node* node,
    ...
    if (dataPtr->isClosed == FALSE)
    {
        ...
        dataPtr->sessionFinish = getSimTime(node);
    }
...
static void enterSuperApplicationServer_ConnectionClosed_TCP(Node* node,
    ...
    if (node->appData.appStats)
    {
        ...
        {
            dataPtr->stats.appStats->SessionFinish(node);
        }
    }
}
                           ...
                           )
{
    ...
    {
        //randomDuration.setSeed(node->globalSeed);
        duration = randomDuration.getRandomNumber() + getSimTime(node);
        if (duration == 0){
            clientPtr->sessionFinish = clientPtr->sessionStart;
        }else{
            clientPtr->sessionFinish = clientPtr->sessionStart
                                       + randomDuration.getRandomNumber() + getSimTime(node);
        }
    }
...
static void SuperApplicationInitStats(Node* node, SuperApplicationStatsType *stats) {
    ...
    if (node->guiOption) {
        ...
        stats->numRepFragsRcvdId = GUI_DefineMetric("Total number of reply fragments received", node->nodeId, GUI_APP_LAYER, 0, GUI_UNSIGNED_TYPE,GUI_CUMULATIVE_METRIC);
        ...
        stats->numReqFragsRcvdId = GUI_DefineMetric("Total number of request fragments received", node->nodeId, GUI_APP_LAYER, 0, GUI_UNSIGNED_TYPE,GUI_CUMULATIVE_METRIC);
        stats->numReqFragBytesRcvdId = GUI_DefineMetric("Total req fragment bytes received", node->nodeId, GUI_APP_LAYER, 0, GUI_UNSIGNED_TYPE,GUI_CUMULATIVE_METRIC);
        ...
        stats->numSlowPacketsId = GUI_DefineMetric("Total packets with real time > sim time", node->nodeId, GUI_APP_LAYER, 0, GUI_UNSIGNED_TYPE,GUI_CUMULATIVE_METRIC);
...
void SuperApplicationRunTimeStat(Node* node, SuperApplicationData* dataPtr) {
    //clocktype now = getSimTime(node);
    clocktype now = getSimTime(node) + getSimStartTime(node);
    if (node->guiOption) {
...
SuperApplicationInit(Node* node,
    ...
    if (isServer)
    {
        ...
        {
            dataPtr->connectionId = -1;
            // Perform init function for TCP server
...
void SuperApplicationHandleSourceInputString(
    ...
    if (strcmp(keyword, "DESTINATIONS") != 0) {
        fprintf(stderr,
                "HAVE TO SPECIFIES BOTH SOURCES AND DESTINATIONS"
                "SUPER-APPLICATION Client Server       (%d)\n", i+1);
        ERROR_ReportError("Illegal configurations");
    }
    ...
    Message* msg)
{
    ...
    if (serverPtr->stats.numRepPacketsSent == 1){
        serverPtr->firstRepFragSentTime = getSimTime(node);
    }
    if (serverPtr->lastRepFragSentTime > serverPtr->firstRepFragSentTime){
    ...
    SuperApplicationData* clientPtr)
{
    ...
    while (totalFragBytesRead > 0 && (completeFragmentRvd == TRUE))
    {
        ...
        header = (SuperApplicationTCPDataPacket *)MESSAGE_ReturnInfo(msg,
                                       INFO_TYPE_SuperAppTCPData,countInfo);
        delay = getSimTime(node) - header->txTime;
...
// Make sure whether the below code section is really required for ADDON_DB
    ...
    while (totalBytesRead > 0 && (completePacketRvd == TRUE))
    {
        ...
        if (totalBytesRead >= header->totalPacketBytes)
        {
            ...
            if (header->isLastPacket) {
                APP_TcpCloseConnection(node, clientPtr->connectionId);
                if (node->appData.appStats)
                {
                    if (!clientPtr->stats.appStats->IsSessionFinished(STAT_Unicast))
                    {
...
static void SuperApplicationClientProcessFragRcvd_TCP(Node* node, SuperApplicationData* clientPtr){
    if (clientPtr->stats.numRepFragsRcvd == 0) {
        // First fragment received. Init server
        clientPtr->firstRepFragRcvTime = getSimTime(node);
    }
    clientPtr->stats.numRepFragsRcvd++;
    clientPtr->lastRepFragRcvTime = getSimTime(node);
    if (clientPtr->lastRepFragRcvTime > clientPtr->firstRepFragRcvTime) {
...
BOOL SuperAppVoiceStartTalking (Node* node, SuperApplicationData* dataPtr, char* dataType, clocktype reqInterval){
    ...
    if (((getSimTime(node) - dataPtr->firstTalkTime +
    ...
    else if (((getSimTime(node) - dataPtr->firstTalkTime +
                 reqInterval) >= dataPtr->talkTime) &&
              (((getSimTime(node) +
                   reqInterval) < dataPtr->sessionFinish) ||
                (dataPtr->sessionFinish == dataPtr->sessionStart)))
...
void SuperApp_ChangeTputStats(Node* node, BOOL before, BOOL after) {
    ...
    if (after) {
        ...
        for (; appList != NULL; appList = appList->appNext) {
            if (appList->appType == APP_SUPERAPPLICATION_CLIENT) {
                dataPtr = (SuperApplicationData*) appList->appDetail;
                dataPtr->firstReqFragSentTime = getSimTime(node);
                dataPtr->lastReqFragSentTime  = getSimTime(node);
                if (dataPtr->processReply == TRUE) {
                    dataPtr->firstRepFragRcvTime = getSimTime(node);
                    dataPtr->lastRepFragRcvTime  = getSimTime(node);
                }
            } else if (appList->appType == APP_SUPERAPPLICATION_SERVER) {
                dataPtr = (SuperApplicationData*) appList->appDetail;
                dataPtr->firstReqFragRcvTime = getSimTime(node);
                dataPtr->lastReqFragRcvTime = getSimTime(node);
                if (dataPtr->processReply == TRUE) {
                    dataPtr->firstRepFragSentTime = getSimTime(node);
                    dataPtr->lastRepFragSentTime  = getSimTime(node);
                }
            }
        }
    }
       ...
       if (appList->appType == APP_SUPERAPPLICATION_SERVER)
        {
          if (dataPtr->serverAddr == groupAddress)
            {
              dataPtr->groupLeavingTime = getSimTime(node);
            }
        }
    }
}
    ...
    for (; appList != NULL; appList = appList->appNext)
    {
        if (appList->appType == APP_SUPERAPPLICATION_SERVER)
        {
            ...
            if (serverPtr->serverAddr == groupAddress)
            {
                ...
                {
                    serverPtr->timeSpentOutofGroup += getSimTime(node) -
