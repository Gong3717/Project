...
int EcnRioQueue::replicate(Queue* oldQueue)
{
    ...
    if (isCollectStats)
    {
        ...
        stats->numPacketsQueued = (int)Queue::stats->GetPacketsEnqueued().GetValue(getSimTime(node));
        stats->numPacketsDequeued = (int)Queue::stats->GetPacketsDequeued().GetValue(getSimTime(node));
        stats->numPacketsDropped = (int)Queue::stats->GetPacketsDropped().GetValue(getSimTime(node));
        stats->numBytesQueued = (int)Queue::stats->GetBytesEnqueued().GetValue(getSimTime(node));
        stats->numBytesDequeued = (int)Queue::stats->GetBytesDequeued().GetValue(getSimTime(node));;
        stats->numBytesDropped = (int)Queue::stats->GetBytesDropped().GetValue(getSimTime(node));;
    }
