    ...
    if (Ipv6IsForwardingEnabled(ip->ipv6))
    {
        ...
        else
        {
            ...
            ip->ipv6->randomNum.setDistributionUniform(
                        MIN_RTR_ADVERT_INTERVAL, MAX_RTR_ADVERT_INTERVAL);
...
//GuiStart
    if (node->guiOption == TRUE)
    {
        ...
        if (isFound)
        {
            ...
            GUI_Receive(lastHopNodeId, node->nodeId, GUI_NETWORK_LAYER,
                        GUI_DEFAULT_DATA_TYPE, interfaceIndex1,
                        interfaceIndex,
                        getSimTime(node));
        }
    }
...
void Ipv6DeleteDestination(Node* node)
{
    ...
    while (temp != NULL)
    {
        if (temp->ro->ro_rt.rt_node)
        {
            ...
            if ((getSimTime(node) - temp->ro->ro_rt.rt_expire) >= 0
...
void Ipv6AddToMulticastGroupList(Node* node, in6_addr groupAddress)
{
    ...
    ListInsert(node, list, getSimTime(node), entry);
}
...
int Ipv6IsValidGetMulticastScope(Node* node, in6_addr multiAddr)
{
    ...
    switch (scope)
    {
        ...
        {
            fprintf(stderr,
                    "MCBR: destination address scope belongs"
                    " to reserved range\n");
            break;
        }
        case 0x06:
