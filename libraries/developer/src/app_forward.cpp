    ...
    BOOL isCompleteMsg)
{
    ...
    if (!isTcp)
    {
        ...
        delay = getSimTime(node) - udpFragData->packetCreationTime;
    ...
    else
    {
        ...
        delay = getSimTime(node) - tcpHeader->packetCreationTime;
    ...
    Message* msg)
{
    ...
    {
        // if ((*timeOutIt)->timeout < getSimTime(node))
        if (forward->timeOutInfo.at(i)->timeout < getSimTime(node))
        {
            // retireve the data.
    ...
    if (udpFragData->uniqueFragNo > forward->udpUniqueFragNoRcvd)
    {
        ...
        if (forward->udpStats->numUdpFragsReceived == 1)
        {
            forward->udpStats->udpServerSessionStart = getSimTime(node);
        }

        forward->udpStats->udpServerSessionFinish = getSimTime(node);
        // New code.
        forward->tosStats[udpFragData->tos]->totalPriorityDelay += getSimTime(node) - msg->packetCreationTime;
    ...
    Message* msg)
{
    ...
    {
        forward->tcpStats->firstTcpFragRecdTime = getSimTime(node);
    }

    forward->tcpStats->lastTcpFragRecdTime = getSimTime(node);
    ...
    AppDataForward *forward)
{
            ...
            && !forward->waitingForDataSent)
    {
        ...
        {
            // first Packet
            forward->tcpStats->firstTcpPacketSendTime = getSimTime(node);
        }
        forward->tcpStats->lastTcpPacketSendTime = getSimTime(node);
        // STATS DB CODE
#ifdef ADDON_DB
        ...
        appParam.m_TotalMsgSize = bufferedRequest->realDataSize +
                                  bufferedRequest->virtualSize;
        appParam.SetMsgSize(bufferedRequest->realDataSize +
                            bufferedRequest->virtualSize);
        ...
        if (node->appData.appStats)
        {
            int bytesSent = bufferedRequest->realDataSize + bufferedRequest->virtualSize;
            forward->tcpStats->tcpAppStats->AddFragmentSentDataPoints(
                node,
                bytesSent,
                STAT_Unicast);
    ...
    BOOL isUdpMulticast = FALSE)
{
    ...
    if (realDataSize > 0)
    {
        ERROR_Assert(headerData != NULL, "header should not be NULL");
        memcpy(MESSAGE_ReturnPacket(msg), headerData, realDataSize);
    }
    ...
    udpFragInfo->udpTimeout = getSimTime(srcNode) + DEFAULT_UDP_TIMEOUT;
    ...
    udpFragInfo->packetCreationTime = getSimTime(srcNode);
    ...
    int numNodesInMulticastGroup)
{
    ...
    Int32 sequenceNum = 0;
    Int32 numFrags = 0;
    Int32 messageSize = actualMsgSize + virtualMsgSize;
    ...
    if (iface->type == EXTERNAL_SOCKET)
    {
        ...
        SocketInterface_GetMessageIds(data, messageSize, &msgId1, &msgId2);
        appParam.SetSocketInterfaceMsgId(msgId1, msgId2);
    }
#endif // SOCKET_INTERFACE
    ...
    while (messageSize > 0)
    {
        ...
        if (sequenceNum == 0)
        {
            // The whole code assumes that header size is smaller than fragment size.
            ERROR_Assert(actualMsgSize <= udpFragmentSize,
                         "The header size must be smaller than UDP"
                         " fragment unit size");

            // First Fragment, send header.
            if (!sendUdpDataMsg->isVirtual)
            {
                // Send using virtual data function, but no virtual data will be
                // sent
    ...
    EXTERNAL_ForwardSendTcpData* sendTcpDataMsg)
{
    ...
    case FORWARD_TcpConnected:
        ...
        else
        {
            ...
            appParam.m_TotalMsgSize = realDataSize + virtualDataSize;
            appParam.SetMsgSize(realDataSize +
                                virtualDataSize);
            ...
            // TCP Stats
            int bytesSent = realDataSize + virtualDataSize;
            if (nodeA->appData.appStats)
           {
            ...
            {
                // first Packet
                forward->tcpStats->firstTcpPacketSendTime = getSimTime(nodeA);
            }
            forward->tcpStats->lastTcpPacketSendTime = getSimTime(nodeA);
...
AppLayerForward(Node *node, Message *msg)
{
    ...
    case MSG_EXTERNAL_ForwardSendUdpData:
    {
        ...
        forward->tosStats[sendData->priority]->numPriorityBytesSent += MESSAGE_ReturnPacketSize(msg);
    ...
    case MSG_APP_FromTransOpenResult:
    {
        ...
        if (openResult->connectionId < 0)
        {
            ...
            if (forward->requestBuffer)
            {
                ...
                appParam.m_fragEnabled = FALSE;
                appParam.SetPriority(forward->tos);
                appParam.SetMsgSize(forward->requestBuffer->realDataSize +
                    forward->requestBuffer->virtualSize);
                appParam.m_TotalMsgSize = forward->requestBuffer->realDataSize +
                                          forward->requestBuffer->virtualSize;
                appParam.SetMessageFailure("TCP Connection Failure") ;
...

{
    ...
    ERROR_Assert(tcpHeaderInfo != NULL, "Unable to add an info field!");
    tcpHeaderInfo->packetCreationTime = getSimTime(node);
    tcpHeaderInfo->packetSize = MESSAGE_ReturnPacketSize(msg);
