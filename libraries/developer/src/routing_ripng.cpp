...
//add your code here:
    ...
    else
    {
        ...
        ERROR_Assert(*timePtr >= getSimTime(node), "RIPng route timer in "
            "the past");
...
//add your code here:
    ...
    else
    {
        ...
        COPY_ADDR6(incomingInterfacePrefix, sourceGlobalAddress);
        sourceGlobalAddress.s6_addr32[2] =
            sourceAddress.s6_addr32[2];
        sourceGlobalAddress.s6_addr32[3] =
            sourceAddress.s6_addr32[3];
    }
    ...
    if (RIPNG_DEBUG)
    {
        ...
        TIME_PrintClockInSecond(getSimTime(node), TimeBuf);
                ...
                (match == TRUE
                 ...
                 || (routePtr->timeoutTime != 0
                    //&& routePtr->nextHop != sourceAddress
                        ...
                        && dist_metric == routePtr->metric
                        && getSimTime(node) >= routePtr->timeoutTime
                                              - RIPNG_ROUTE_TIMEOUT_INTERVAL
                                              / 2))
            {
...
//add your code here:
    ...
    // Get Link-Local Address of the Incoming Interface Index
    if (NetworkIpIsWiredNetwork(node, interfaceIndex))
    {
        // Get Link-Local Address of the Incoming Interface Index
        Ipv6GetLinkLocalAddress(
...
static void enterRIPngHandleRouteTimerAlarm(Node* node,
    ...
    if (*timePtr != 0)
    {
       ...
       {
            // getSimTime(node) matches route alarm time,
    ...
    if (node->appData.routingVar == NULL)
    {
       ...
       // Set pointer in node data structure to RIPng state.
        ...
        for (i = 0; i < node->numberInterfaces; i++)
        {
            ...
            {
                // Get link-local interface address from the interface index
                Ipv6GetLinkLocalAddress(
                    node,
                    i,
                    &interfaceAddress);
