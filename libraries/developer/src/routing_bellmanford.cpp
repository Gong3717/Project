...
RoutingBellmanfordInit(Node *node)
{
    ...
    bellmanford->nextPeriodicUpdateTime = getSimTime(node) + randomDelay;
...
HandlePeriodicUpdateAlarm(Node *node)
{
    ...
    // Schedule next periodic update. (getSimTime(node) + 10 s + 0-100 ms)
    ...
    bellmanford->nextPeriodicUpdateTime = getSimTime(node) + delay;
}
...
HandleCheckRouteTimeoutAlarm(Node *node)
{
    ...
    //  dont time out.
        ...
        if (bellmanford->routeTable[i].localRoute == FALSE
            ...
            && getSimTime(node) - bellmanford->routeTable[i].refreshTime
               >= TIMEOUT_DELAY)
        {
            ...
            bellmanford->routeTable[i].distance    = BELLMANFORD_INFINITY;
            bellmanford->routeTable[i].refreshTime = getSimTime(node);
...
ScheduleTriggeredUpdate(Node *node)
{
    ...
    if (bellmanford->triggeredUpdateScheduled == FALSE
        && bellmanford->nextPeriodicUpdateTime
           > getSimTime(node) + TRIGGERED_UPDATE_DELAY + TRIGGERED_UPDATE_JITTER)
    {
        Message *newMsg;

        // Schedule triggered update. (getSimTime(node) + 0-100 ms)
    ...
    AdvertisedRoute *neighborRowPtr)
{
    ...
    for (i = 0; i < numAdvertisedRoutes; i++)
    {
            ...
            || neighborAddress == rowPtr->nextHop)
        {
            ...
            rowPtr->refreshTime = getSimTime(node);
