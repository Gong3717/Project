...
MessengerSenseChannel(Node *node, MessengerState *messenger)
{
    ...
    if ((messenger->messageLastRecvTime == 0) ||
        ((TIME_getSimTime(node) - messenger->messageLastRecvTime)
...
{
    messenger->messageLastRecvTime = TIME_getSimTime(node);
    ...
    if (messenger->messageLastSentTime != 0)
    {
                ...
                < HLA_VOICE_PACKET_BACKOFF_TIME)
        {
            ...
            if (DEBUG_STATE)
            {
                ...
                TIME_PrintClockInSecond(TIME_getSimTime(node), time);
    ...
    while (queuePtr->packetsInQueue())
    {
        ...
        if (queueMsg != NULL)
        {
            ...
            if (TEST_VOICE_APP)
            {
                ...
                TIME_PrintClockInSecond(TIME_getSimTime(node), SentAt);
    ...
    MessengerMessageData msgData)
{
    ...
    msgData.pktHdr.timeStamp = TIME_getSimTime(node);
    ...
    {
        sendEntry->messageStartTime = TIME_getSimTime(node);
    }

    msgData.pktHdr.pktNum++;

    sendEntry->pktHdr.pktNum = msgData.pktHdr.pktNum;
    sendEntry->endTime = TIME_getSimTime(node);
    ...
    MessengerMessageData msgData)
{
    ...
    if (msgData.pktHdr.appType == VOICE_PACKET)
    {
        ...
        msgFrags = APP_UdpCreateNewHeaderVirtualDataWithPriority(
                    node,
                    APP_MESSENGER,
                    ANY_IP,
                    APP_MESSENGER,
        ...
        if (TEST_VOICE_APP)
        {
            ...
            TIME_PrintClockInSecond(TIME_getSimTime(node), SpeakedAt);
    ...
    const MessengerMessageData msgData)
{
    ...
    {
        SendFragmentTcp(node, messenger, msgData, TIME_getSimTime(node));
    }
    ...
    MessengerPktBufferEntry *recvEntry = NULL)
{
    if (!recvEntry)
    {
        recvEntry = CreateOrReturnRecvEntry(
                        node, messenger, pktHdr, TIME_getSimTime(node));
    }
    recvEntry->pktDelays[pktHdr.pktNum]
        = TIME_getSimTime(node) - pktHdr.timeStamp;
    ...
    switch (recvEntry->receiveState)
    {
        ...
        case VIRTUAL_PAYLOAD:
        {
            if (*packetBytesRemaining >= recvEntry->receiveBytesRemaining)
            {
                ...
                {
                    char buf[MAX_CLOCK_STRING_LENGTH];
                    TIME_PrintClockInSecond(TIME_getSimTime(node),buf);
                ...
                recvEntry->msgData.endTime = TIME_getSimTime(node);

                ERROR_Assert(recvEntry->msgData.pktHdr.messageResultFunc,
                             "Result Function Not Registered");

                recvEntry->msgData.pktHdr.messageResultFunc(
                    node, resultMsg, TRUE);
    ...
    switch(msg->eventType) {
        ...
        case MSG_APP_FromMac:
            ...
            if (DEBUG)
            {
                ...
                TIME_PrintClockInSecond(
                    TIME_getSimTime(node) - pktHdr->timeStamp, buf);
                ...
                pktHdr->srcAddr) != INVALID_MAPPING)
            {
                ...
                if (DEBUG)
                {
                    printf("#%d: sender side timer at %f\n",
                           node->nodeId,
                           ((double)TIME_getSimTime(node)) / SECOND);
                }
                MESSAGE_Free(node, msg);
            }
            else
        ...
        case MSG_APP_ChannelIsBusy: {
            ...
            if ((messenger->nextChannelSenseTime == 0) ||
                (messenger->nextChannelSenseTime <= TIME_getSimTime(node)))
            {
                if (messenger->numCollisionDetected)
                {
        ...
        case MSG_APP_FromTransOpenResult: {
            ...
            else
            {
                ...
                sendEntry->pktHdr.timeStamp = TIME_getSimTime(node);
