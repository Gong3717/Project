...
//**
// FUNCTION   :: REDScheduler::updateAverageLength
// LAYER      ::
// PURPOSE    :: This function is used to update the moving
//               average of the average sum of queue lengths 
//               of the scheduler via exponential smoothing
// PARAMETERS ::
// + newLength : int : New sum of length of queues
// RETURN     :: void : Null
// **/

void
REDScheduler::updateAverageLength(int newLength)
{
    averageLength = (int)(((1.0 - params->smoothingFactor) * averageLength)
                      + (params->smoothingFactor * newLength));
}

//**
// FUNCTION   :: REDScheduler::insert
// LAYER      ::
    ...
    )
{
    ...
    updateAverageLength(bytesInQueue(ALL_PRIORITIES
#ifdef ADDON_BOEINGFCS
                                    ,TRUE
#endif
                                     ) + 
    ...
    // Drop packet if RED limit is exceeded
    if ((params->maxLength != 0) && (averageLength > params->maxLength))    
    {
        drop = TRUE;
    }
    else if ((params->minLength != 0) && (averageLength > params->minLength))
    {
        // Recalculate clim as a quarter way between average length and 
        // the maximum RED limit
        if (clim <= 0)
        {
            clim = averageLength + ((params->maxLength - averageLength)/4);
        }
        // Drop packet if average sum of queue length exceeds clim
        if (averageLength > clim)
        {
            clim = 0;
            drop = TRUE;
        }
    ...
    )
{
    ...
    {
        // Recalculate clim as a quarter way between average length and 
        // the maximum RED limitif (clim <= 0)
        {
            clim = averageLength + ((params->maxLength - averageLength)/4);
        }
    ...
    else
    {
        *QueueIsFull = TRUE;
#ifdef ADDON_BOEINGFCS
        RoutingCesMalsrSignalSchedulerCongestion(node,
                                                 interfaceIndex,
                                                 msg,
                                                 params->minLength,
                                                 averageLength);
...
//**
// FUNCTION   :: REDScheduler::retrieve
// LAYER      ::
...
//**
// FUNCTION   :: REDScheduler::addQueue
// LAYER      ::
    ...
    const double weight)
{
    if ((numQueues == 0) || (numQueues == maxQueues))
    {
        ...
        REDStat* newStats
            = new REDStat[maxQueues + DEFAULT_QUEUE_COUNT];
        ...
        if (queueData != NULL)
        {
            ...
            memcpy(newStats, stats, sizeof(REDStat) * maxQueues);
            delete []stats;
        }
    ...
    if (priority == ALL_PRIORITIES)
    {
        ...
        memset(&stats[index], 0, sizeof(REDStat));
        ...
        for (i = 0; i < numQueues; i++)
        {
            if (queueData[i].priority > priority)
            {
                ...
                memcpy(newStats, &stats[i],
                        sizeof(REDStat) * afterEntries);

                memcpy(&stats[i+1], newStats,
                        sizeof(REDStat) * afterEntries);

                memset(&stats[i], 0, sizeof(REDStat));
...
//**
// FUNCTION   :: REDScheduler::removeQueue
// LAYER      ::
...
void REDScheduler::removeQueue(const int priority)
{
    int i;
    for (i = 0; i < numQueues; i++)
    {
...
//**
// FUNCTION   :: REDScheduler::swapQueue
// LAYER      ::
...
//**
// FUNCTION   :: REDScheduler::finalize
// LAYER      ::
    ...
    const char* splStatStr)
{
    ...
    char schedType[MAX_STRING_LENGTH] = "RedStrictPrio";
    ...
    else
    {
        ...
        sprintf(schedType, "%s RED Strict ", invokingProtocol);
        sprintf(intfIndexStr, "%d", interfaceIndex);
    }

    if (splStatStr)
    {
        sprintf(schedType, "  RED Strict %s", splStatStr);
    }
...
//---------------------------------------------------------------------------
// REDScheduler Class Constructor
//---------------------------------------------------------------------------

//**
// FUNCTION   :: REDScheduler::REDScheduler
// LAYER      ::
// PURPOSE    :: Constractor of REDScheduler class to initialize
    ...
    REDParams *rParams)
{
    ...
    //RED specific data
    params = (REDParams*) MEM_malloc(sizeof(REDParams));
    memcpy(params,rParams,sizeof(REDParams));
    averageLength = 0;
    clim = 0;
