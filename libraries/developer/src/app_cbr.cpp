        ...
        case MSG_APP_TimerExpired:
        {
            ...
            switch (timer->type)
            {
                case APP_TIMER_SEND_PKT:
                {
                    ...
                    if (node->appData.appStats)
                    {
                        if (clientPtr->stats->GetMessagesSent().GetValue(getSimTime(node)) == 0)
                        {                         
    ...
    switch (msg->eventType)
    {
        case MSG_APP_FromTransport:
        {
...
#endif
            {
...
#ifdef DEBUG
                {
                    ...
                    TIME_PrintClockInSecond(
                        serverPtr->stats->GetTotalDelay().GetValue(getSimTime(node)), clockStr);
            ...
            if (db != NULL)
            {
                ...
                if (node->appData.appStats)
                {
                    if (serverPtr->stats->GetMessagesReceived().GetValue(getSimTime(node)) == 1)
                    ...
                    SequenceNumber::Status seqStatus =
                        APP_ReportStatsDbReceiveEvent(
                            ...
                            data.seqNo,
                            delay,
                            serverPtr->stats->GetJitter().GetValue(getSimTime(node)),
                            MESSAGE_ReturnPacketSize(msg),
                            serverPtr->stats->GetMessagesReceived().GetValue(getSimTime(node)),
                            msgStatus);
