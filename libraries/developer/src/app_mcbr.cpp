...
AppLayerMCbrClient(Node *node, Message *msg)
{
           ...
           if (clientPtr == NULL)
            ...
            switch (timer->type)
            {
                case APP_TIMER_SEND_PKT:
                {
                    ...
                    {
                        char clockStr[24];
                        ctoa(getSimTime(node)+getSimStartTime(node),
                                                              clockStr);
    ...
    appParam.m_fragEnabled = FALSE;
                    ...
                    Message* msg = APP_UdpSendNewDataWithPriority(
                        node,
                        APP_MCBR_SERVER,
                        clientPtr->localAddr,
                    ...
                    if (node->appData.appStats)
                    {
                        if (clientPtr->stats->GetMessagesSent(STAT_Multicast).GetValue(getSimTime(node)) == 0)
                        {
                            clientPtr->stats->SessionStart(node,STAT_Multicast);
                        }
                         ...
                         char* appName)
{
    ...
    mcbrClient->sessionStart = getSimTime(node) + startTime;
    mcbrClient->sessionIsClosed = FALSE;
    mcbrClient->sessionLastSent = getSimTime(node);
    mcbrClient->sessionFinish = getSimTime(node);
        ...
        case MSG_APP_FromTransport:
        {
            ...
            if (data.seqNo >= serverPtr->seqNo || data.isMdpEnabled)
            {
                if (node->appData.appStats)
                {
                    ...
                    {
                        char clockStr[24];
                        ctoa(serverPtr->stats->GetTotalDelay(STAT_Multicast).GetValue(getSimTime(node)), clockStr);
            ...
            if (db != NULL && node->appData.appStats)
            {
                ...
                delay = getSimTime(node) - data.txTime;
                SequenceNumber::Status seqStatus =
                    APP_ReportStatsDbReceiveEvent(
                        ...
                        data.seqNo,
                        delay,
                        serverPtr->stats->GetJitter(STAT_Multicast).GetValue(getSimTime(node)),
                        MESSAGE_ReturnPacketSize(msg),
                        serverPtr->stats->GetMessagesReceived(STAT_Multicast).GetValue(getSimTime(node)),
                        msgStatus);
                           ...
                           int sourcePort)
{
    ...
    mcbrServer->sessionStart = getSimTime(node);
    mcbrServer->sessionFinish = getSimTime(node);
    mcbrServer->sessionLastReceived = getSimTime(node);
