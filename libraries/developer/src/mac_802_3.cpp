    ...
    FILE* fp)
{
    ...
    TIME_PrintClockInSecond(getSimTime(node), simTime);
    ...
    MacData802_3* mac802_3)
{
    if (mac802_3->stationState == IDLE_STATE)
    {
        ...
        mac802_3->interFrameGapStartTime = getSimTime(node);
    ...
    Message* msg)
{
    ...
    switch (msg->eventType)
    {
        case MSG_MAC_StartTransmission:
        {
            ...
            {
                clocktype presentTime = getSimTime(node);
    ...
    Int32 interfaceIndex)
{
    ...
    // Check for TRACE_802_3 headers
    Int32 controlSize = 0;
    Int32 i;
    for (i = 0; i < msg->numberOfHeaders; i++)
    {
        if (msg->headerProtocols[i] == TRACE_802_3)
        {
    ...
    Int32 interfaceIndex)
{
    ...
    UInt16 lengthOfPacket = Mac802_3GetLengthOfPacket(msg);

    if (lengthOfPacket == MAC802_3_VLAN_TAG_TYPE)
    {
        controlSize = controlSize + sizeof(MacHeaderVlanTag);
    }
    ...
    if (!isMyFrame && !isAnyFrame && !MAC_IsASwitch(node))
    {
        return;
    }
