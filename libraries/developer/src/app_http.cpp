...
const int HttpPrimaryReplyDistLength =  1133 ;
const int HttpConsecutiveDocsDistLength =  21 ;
const int HttpFilesPerDocumentDistLength =  27 ;
const int HttpPrimaryRequestDistLength =  305 ;
const int HttpSecondaryReplyDistLength =  1848 ;
const int HttpSecondaryRequestDistLength =  286 ;
const int HttpThinkTimeDistLength =  1649 ;
 ...
 */
void
AppLayerHttpClient(Node *node, Message *msg)
{
    ...
    switch (msg->eventType)
    {
        case MSG_APP_FromTransOpenResult:
        {
            ...
            else
            {
                ...
                clientPtr = AppHttpClientUpdateHttpClient(node, openResult);
        ...
        case MSG_APP_FromTransDataSent:
        {
            ...
            clientPtr = AppHttpClientGetHttpClient(node,
            ...
            {
                clientPtr->appStats->AddFragmentSentDataPoints(node,
                                                       dataSent->length);
            }
                ...
                (clientPtr->state == XMIT_SECONDARY_REQUEST))
            {
                ...
                if (!clientPtr->sessionIsClosed)
                {
                    ...
                    if (node->appData.appStats)
                    {
                        clientPtr->appStats->AddMessageSentDataPoints(
                            node,
                            msg,
                            0,
                            sendSize,
        ...
        case MSG_APP_FromTransDataReceived:
        {
            ...
            /* Drop the previous connection packet if a new connection
            /* has been established and an old connection packet is received*/
            /* i.e drop the packet in case of connectionId mismatch*/
            if (clientPtr == NULL)
            {
                if (AppHttpClientCheckLatePacket(node,
                                                 dataRecvd->connectionId))
                {
                    break;
                }
            ...
            if (node->appData.appStats)
            {
                clientPtr->appStats->AddFragmentReceivedDataPoints(
                    node,
                    msg,
            ...
            {
                clientPtr->lastReceiveTime = getSimTime(node);
            }
            else if (msg->packet[msg->packetSize - 1] == 'd')
            {
                ...
                clientPtr->lastReceiveTime = getSimTime(node);
                AppHttpClientProcessReplyPacket(node, clientPtr);
            }
            else
            {
                assert(FALSE);
        ...
        case MSG_APP_FromTransCloseResult:
        {
            ...
            if (node->appData.appStats)
            {
                if (!clientPtr->appStats->IsSessionFinished(STAT_Unicast))
                {
                    clientPtr->appStats->SessionFinish(node);
                }
            }
        ...
        case MSG_APP_TimerExpired:
        {
            ...
            AppDataHttpClient *tmpHttpClient = NULL;
            AppDataHttpClient *httpClient = NULL;
            HttpClientTimer *timer;

            timer = (HttpClientTimer *) MESSAGE_ReturnInfo(msg);

            for (; appList != NULL; appList = appList->appNext)
            {
                if (appList->appType == APP_HTTP_CLIENT)
                {
                    tmpHttpClient = (AppDataHttpClient *) appList->appDetail;

#ifdef DEBUG
                    printf("HTTP Client: Node %ld comparing uniqueId "
                           "%ld with %ld\n",
                           node->nodeId,
                           tmpHttpClient->uniqueId,
                           timer->clientId);
#endif /* DEBUG */
            ...
            if (timer->timerType == THINK_TIMER)
            {
                ...
                AppHttpClientProcessDoneThinking(node, httpClient);
            }
            else
            {
#ifdef DEBUG
                ...
                AppHttpClientProcessWaitReplyTimer(node, httpClient);
            }
            break;
        }
        default:
    ...
    clocktype thinkTime)
{
    ...
    MESSAGE_InfoAlloc(node, newMsg, sizeof(HttpClientTimer));
    timer = (HttpClientTimer *) MESSAGE_ReturnInfo(newMsg);

    timer->clientId = clientPtr->uniqueId;
    timer->timerType = THINK_TIMER;

    MESSAGE_Send(node, newMsg, thinkTime);
}
    ...
    HttpClientTimerType timerType)
{
    ...
    MESSAGE_Send(node, newMsg, clientPtr->threshhold);
                                 ...
                                 AppDataHttpClient *clientPtr)
{
    ...
    {
        Address serverAddr = AppHttpClientSelectNewServer(clientPtr);
        ...
        if (!Address_IsSameAddress(&serverAddr, &clientPtr->remoteAddr))
        {
            clocktype sum;
            ...
            if (!clientPtr->sessionIsClosed)
            {
                ...
                if (node->appData.appStats)
                {
                    clientPtr->appStats->AddMessageSentDataPoints(
                            ...
                            msg,
                            0,
                            clientPtr->stats.itemRequestBytes,
                            0);
                }
                                   ...
                                   AppDataHttpClient *clientPtr)
{
    if (clientPtr->state == IDLE)
        return;
             ...
             (clientPtr->state == WAIT_SECONDARY_RESPONSE))
    {
        if (clientPtr->lastReceiveTime <
            (getSimTime(node) - clientPtr->threshhold))
        {
            clocktype sum;
            ...
            clientPtr->numSessions++;
            clientPtr->avgSessionLength = sum / clientPtr->numSessions;
            clientPtr->sessionStart = getSimTime(node);
                                ...
                                AppDataHttpClient *clientPtr)
{
    ...
    else
    {
        ...
        clocktype thinkTime = (clocktype) (thinkTimeDouble * SECOND);
        clocktype currentPageRequestTime;
        char buf[MAX_CLOCK_STRING_LENGTH];

        clientPtr->numPages++;
        currentPageRequestTime =
            (getSimTime(node) - clientPtr->primaryRequestSendTime);
        clientPtr->cumulativePageRequestTimes += currentPageRequestTime;
        if (currentPageRequestTime > clientPtr->longestPageRequestTime)
        {
            clientPtr->longestPageRequestTime = currentPageRequestTime;
        }
...
AppHttpClientConsecutiveDocumentRetrievals(AppDataHttpClient *clientPtr)
{
    ...
    double u = RANDOM_erand(clientPtr->seed);
    float value;
    ...
    if (midpoint < 0)
    {
        value = (float) HttpConsecutiveDocsDistTable[0].value;
    }
    else
    {
        value = (float)
                DoubleDistEmpiricalIntegralInterpolate(
                    ...
                    HttpConsecutiveDocsDistTable[midpoint + 1].value,
                    u);
    }
...
AppHttpClientDetermineItemCount(AppDataHttpClient *clientPtr)
{
    ...
    if (midpoint < 0)
    {
        value = (float) HttpFilesPerDocumentDistTable[0].value;
    }
    else
    {
        value = (float)
                DoubleDistEmpiricalIntegralInterpolate(
                    ...
                    HttpFilesPerDocumentDistTable[midpoint + 1].value,
                    u);
    }
    ...
    AppDataHttpClient *clientPtr)
{
    ...
    if (midpoint < 0)
    {
        value = (float) HttpSecondaryRequestDistTable[0].value;
    }
    else
    {
        value = (float)
                DoubleDistEmpiricalIntegralInterpolate(
                    ...
                    HttpSecondaryRequestDistTable[midpoint + 1].value,
                    u);
    }
    ...
    AppDataHttpClient *clientPtr)
{
    ...
    if (midpoint < 0)
    {
        value = (float) HttpPrimaryRequestDistTable[0].value;
    }
    else
    {
        value = (float)
                DoubleDistEmpiricalIntegralInterpolate(
                    ...
                    HttpPrimaryRequestDistTable[midpoint + 1].value,
                    u);
    }
    ...
    Int32 primaryRequestLength)
{
    ...
    sendSize = MIN(primaryRequestLength, APP_MAX_DATA_SIZE);
    payload = (char *)MEM_malloc(sendSize);
    memset(payload,0,sendSize);
    primaryRequestLength -= sendSize;
    clientPtr->stats.itemRequestBytes = primaryRequestLength;
    clientPtr->primaryRequestSendTime = getSimTime(node);
    ...
    if (!clientPtr->sessionIsClosed)
    {
        ...
        if (node->appData.appStats)
        {
            clientPtr->appStats->AddMessageSentDataPoints(node,
                                                          msg,
                                                          0,
                                                          sendSize,
                                                          0);
    ...
    Int32 secondaryRequestLength)
{
    ...
    sendSize = MIN(secondaryRequestLength, APP_MAX_DATA_SIZE);
    payload = (char *)MEM_malloc(sendSize);
    memset(payload,0,sendSize);
    ...
    char* appName)
{
    ...
    clientPtr = AppHttpClientNewHttpClient(node, appName);

    if (clientPtr == NULL)
    {
        printf("HTTP Client: Node %d cannot allocate "
               "new http client\n", node->nodeId);
    ...
    {
        IO_ConvertIpAddressToString(&clientPtr->servers[i], addrStr);
        printf(" %s ", addrStr);
    }
    printf("\n");
    }
    ...
    clientPtr->localAddr = clientAddr;
    ...
    if (node->appData.appStats)
    {
        clientPtr->appStats = new STAT_AppStatistics(
                                        node,
                                        "httpClient",
                                        STAT_Unicast,
                                        STAT_AppSenderReceiver,
                                        clientPtr->applicationName->c_str());
        clientPtr->appStats->EnableAutoRefragment();

        clientPtr->appStats->Initialize(
           node,
           clientAddr,
           serverAddr,
    ...
    printf("    Request %d documents on server address %s\n",
        clientPtr->documentsOnCurrentServer, addrStr);
    }
#endif /* DEBUG */
...
void //inline//
AppHttpClientPrintStats(Node *node, AppDataHttpClient *clientPtr)
{
    char buf[MAX_STRING_LENGTH];
    char clockStr[MAX_STRING_LENGTH];
    char BytesRecvdPerConnectionStr[MAX_STRING_LENGTH];
    char BytesSentPerConnectionStr[MAX_STRING_LENGTH];
    Int64 numBytesRecvd = 0;
    Int64 numBytesSent = 0;
    if (!clientPtr->appStats->IsSessionFinished(STAT_Unicast))
    {
        clientPtr->appStats->SessionFinish(node);
    }
    ...
    TIME_PrintClockInSecond(clientPtr->avgSessionLength, clockStr);
    sprintf(buf, "Average Connection Length (s) = %s", clockStr);
    IO_PrintStat(
        node,
        "Application",
        "HTTP Client",
    ...
    sprintf(buf, "Average Number of Pages per Connection = %d",
    ...
    numBytesRecvd =(Int64)clientPtr->appStats->GetDataReceived(STAT_Unicast).
                                                GetValue(getSimTime(node));
    numBytesSent = (Int64)clientPtr->appStats->GetDataSent(STAT_Unicast).
                                                GetValue(getSimTime(node));
    ctoa((numBytesRecvd / (clocktype) clientPtr->numSessions),
        BytesRecvdPerConnectionStr);
    sprintf(buf, "Average Number of Bytes Received per Connection = %s",
        BytesRecvdPerConnectionStr);
    IO_PrintStat(
        node,
        "Application",
    ...
    ctoa((numBytesSent / (clocktype) clientPtr->numSessions),
        BytesSentPerConnectionStr);
    sprintf(buf, "Average Number of Bytes Sent per Connection = %s",
        BytesSentPerConnectionStr);
    IO_PrintStat(
        node,
        "Application",
    ...
    else
    {
        TIME_PrintClockInSecond(
               (clientPtr->cumulativePageRequestTimes / clientPtr->numPages),
               clockStr);
    }

    sprintf(buf, "Average Page Request Time (s) = %s", clockStr);
    IO_PrintStat(
        node,
        "Application",
        "HTTP Client",
    ...
    TIME_PrintClockInSecond(clientPtr->longestPageRequestTime, clockStr);
    sprintf(buf, "Longest Page Request Time (s) = %s", clockStr);
    IO_PrintStat(
        node,
        "Application",
        "HTTP Client",
 ...
 */
void
AppHttpClientFinalize(Node *node, AppInfo* appInfo)
{
    AppDataHttpClient *clientPtr = (AppDataHttpClient*)appInfo->appDetail;
    ...
    if (node->appData.appStats == TRUE)
    {
        ...
        AppHttpClientPrintStats(node, clientPtr);
        // Print stats in .stat file using Stat APIs
        if (clientPtr->appStats)
        {
            clientPtr->appStats->Print(
                node,
                "Application",
                "HTTP Client",
                ANY_ADDRESS,
 ...
 */
AppDataHttpClient * //inline//
AppHttpClientGetHttpClient(Node *node, int connId)
{
    // Get list of apps based on node address and connId
    Address addr;
    SetIPv4AddressInfo(&addr, node->nodeId);
    AppInfo* appList = node->appData.appMap->getAppInfo(addr, connId);
    AppDataHttpClient *httpClient;

    for (; appList != NULL; appList = appList->appNext)
    {
        if (appList->appType == APP_HTTP_CLIENT)
        {
            httpClient = (AppDataHttpClient *) appList->appDetail;
 ...
 */
AppDataHttpClient * //inline//
AppHttpClientUpdateHttpClient(Node *node,
                            TransportToAppOpenResult *openResult)
{
    AppInfo *appList = node->appData.appPtr;
    AppDataHttpClient *tmpHttpClient = NULL;
    ...
    httpClient->sessionStart = getSimTime(node);
    httpClient->sessionFinish = getSimTime(node);
    httpClient->sessionIsClosed = FALSE;
    if (node->appData.appStats)
    {
        if (!httpClient->appStats->IsSessionStarted())
        {
             httpClient->appStats->SessionStart(node);
        }
    }
    Address addr;
    SetIPv4AddressInfo(&addr, node->nodeId);
    // Push app detail onto AppMap
    node->appData.appMap->pushDetail(addr,
    ...
    IO_ConvertIpAddressToString(&httpClient->localAddr, addrStr);
    printf("    localAddr = %s\n", addrStr);
    IO_ConvertIpAddressToString(&httpClient->remoteAddr, addrStr);
    printf("    remoteAddr = %s\n", addrStr);
#endif /* DEBUG */
 ...
 */
AppDataHttpClient * //inline//
AppHttpClientNewHttpClient(Node *node, char* appName)
{
    AppDataHttpClient *httpClient;

    httpClient = (AppDataHttpClient *)
                 MEM_malloc(sizeof(AppDataHttpClient));
    memset(httpClient, 0, sizeof(AppDataHttpClient));

    httpClient->connectionId = -1;
    httpClient->uniqueId = node->appData.uniqueId++;

    // ->localAddr,
    // ->remoteAddr initialized later.

    httpClient->itemSizeLeft = 0;
    httpClient->numSessions = 0;
    httpClient->numPages = 0;
    httpClient->avgSessionLength = 0;
    httpClient->cumulativePageRequestTimes = 0;
    httpClient->primaryRequestSendTime = 0;
    httpClient->longestPageRequestTime = 0;
    httpClient->sessionStart = -1;
    httpClient->appStats = NULL;

    if (appName)
    {
        httpClient->applicationName = new std::string(appName);
    }
    else
    {
        httpClient->applicationName = new std::string();
    }
#ifdef DEBUG
    ...
    APP_RegisterNewApp(node, APP_HTTP_CLIENT, httpClient);
 ...
 */
void
AppLayerHttpServer(Node *node, Message *msg)
{
    char buf[MAX_STRING_LENGTH];
    ...
    switch (msg->eventType)
    {
        ...
        case MSG_APP_FromTransOpenResult:
        {
            ...
            {
                AppDataHttpServer* serverPtr;
                serverPtr = AppHttpServerGetExistingHttpServer(node,
                                                               openResult);
                if (serverPtr != NULL)
                {
                    serverPtr->connectionId = openResult->connectionId;
                    serverPtr->sessionIsClosed = FALSE;
                    RANDOM_SetSeed(serverPtr->seed,
                                   node->globalSeed,
                                   node->nodeId,
                                   APP_HTTP_SERVER,
                ...
                {
                    serverPtr = AppHttpServerNewHttpServer(node, openResult);
                    assert(serverPtr != NULL);

                    if (node->appData.appStats)
                    {
                        serverPtr->appStats->Initialize(
                            node,
                            serverPtr->remoteAddr,
                            serverPtr->localAddr,
                            (STAT_SessionIdType)openResult->clientUniqueId);

                        if (!serverPtr->appStats->IsSessionStarted())
                        {
                            serverPtr->appStats->SessionStart(node);
                        }
                    }
                }
            }
        ...
        case MSG_APP_FromTransDataSent:
        {
            ...
            serverPtr = AppHttpServerGetHttpServer(node,
                                     dataSent->connectionId);
            ...
            if (node->appData.appStats)
            {
                serverPtr->appStats->AddFragmentSentDataPoints(node,
                                                               dataSent->length);
            }
            if (serverPtr->bytesRemaining > 0)
            {
                ...
                if (!serverPtr->sessionIsClosed)
                {
                    ...
                    if (node->appData.appStats)
                    {
                        serverPtr->appStats->AddMessageSentDataPoints(node,
                                                                      msg,
                                                                      0,
                                                                      sendSize,
                                                                      0);
        ...
        case MSG_APP_FromTransDataReceived:
        {
            ...
            {
                // Drop the packet for an already closed connection.
                break;
            }

            if (node->appData.appStats)
            {
                serverPtr->appStats->AddFragmentReceivedDataPoints(
                    node,
                    msg,
                    MESSAGE_ReturnPacketSize(msg));
            }
            ...
            else if (packet[msg->packetSize - 1] == 'p')
            {
                Int32 primaryReplyLength =
                    AppHttpServerDeterminePrimaryReplyLength(node, serverPtr);

                serverPtr->pagesSent++;
                /* HTTP Primary Request  completely received,
                   now send Primary Reply */
                AppHttpServerSendPrimaryReply(node, serverPtr,
                        primaryReplyLength);
            }
            else if (packet[msg->packetSize - 1] == 's')
            {
                Int32 secondaryReplyLength =
                    AppHttpServerDetermineSecondaryReplyLength(node, serverPtr);

                /* HTTP Secondary Request  completely received,
                   now send Secondary Reply */
                AppHttpServerSendSecondaryReply(node, serverPtr,
            ...
            else if (packet[msg->packetSize - 1] == 'c')
            {
                ...
                if (node->appData.appStats)
                {
                    if (!serverPtr->appStats->IsSessionFinished(STAT_Unicast))
                    {
                        serverPtr->appStats->SessionFinish(node);
                    }
                }
 ...
 */
void
AppHttpServerInit(Node *node, Address serverAddr)
{
    APP_TcpServerListen(
        node,
        APP_HTTP_SERVER,
 ...
 */
void //inline//
AppHttpServerPrintStats(Node *node, AppDataHttpServer *serverPtr)
{
 ...
 */
void
AppHttpServerFinalize(Node *node, AppInfo* appInfo)
{
    AppDataHttpServer *serverPtr = (AppDataHttpServer*)appInfo->appDetail;
    ...
    if (node->appData.appStats == TRUE)
    {
        ...
        AppHttpServerPrintStats(node, serverPtr);
        // Print stats in .stat file using Stat APIs
        if (serverPtr->appStats)
        {
            serverPtr->appStats->Print(
                node,
                "Application",
                "HTTP Server",
                ANY_ADDRESS,
 ...
 *              NULL if nothing found.
 */
AppDataHttpServer * //inline//
AppHttpServerGetHttpServer(Node *node, int connId)
{
    AppInfo *appList = node->appData.appPtr;
    AppDataHttpServer *httpServer;

    for (; appList != NULL; appList = appList->appNext)
    {
        if (appList->appType == APP_HTTP_SERVER)
        {
            httpServer = (AppDataHttpServer *) appList->appDetail;
 ...
 */
AppDataHttpServer * //inline//
AppHttpServerNewHttpServer(Node *node,
                           TransportToAppOpenResult *openResult)
{
    AppDataHttpServer *httpServer;

    httpServer = (AppDataHttpServer *)
                 MEM_malloc(sizeof(AppDataHttpServer));

    /*
     * fill in connection id, etc.
     */
    httpServer->clientUniqueId = openResult->clientUniqueId;
    ...
    httpServer->sessionStart = getSimTime(node);
    httpServer->sessionFinish = getSimTime(node);
    ...
    if (node->appData.appStats)
    {
        httpServer->appStats = new STAT_AppStatistics(
            node,
            "httpServer",
            STAT_Unicast,
            STAT_AppSenderReceiver);
        httpServer->appStats->EnableAutoRefragment();
    }
    ...
    APP_RegisterNewApp(node, APP_HTTP_SERVER, httpServer);
    // Push app detail onto the AppMap
    node->appData.appMap->pushDetail(httpServer->remoteAddr,
                                     httpServer->clientUniqueId,
                                     APP_HTTP_SERVER,
                                     httpServer);
...
AppHttpClientCheckLatePacket(Node *node, Int32 connId)
{
    AppInfo *appList = node->appData.appPtr;
    AppDataHttpClient *httpClient;
    /* Drop the previous connection packet if a new connection has
    /*been established*/
    /* i.e drop the packet in case of connectionId mismatch*/
    BOOL latePacket = FALSE;
    for (; appList != NULL; appList = appList->appNext)
    {
        if (appList->appType == APP_HTTP_CLIENT)
        {
...
AppHttpServerGetExistingHttpServer(Node* node,
                                   TransportToAppOpenResult* openResult)
{
    Address clientAddr = openResult->remoteAddr;
    // Get list of apps based on client address and clientUniqueId
    AppInfo* appList = node->appData.appMap->
        getAppInfo(clientAddr,openResult->clientUniqueId);
    AppDataHttpServer* httpServer;
