...
static void enterVbrClientTimerExpired(Node* node,
    ...
    if (node->appData.appStats)
    {
        if (dataPtr->appStats->GetDataSent().GetValue(getSimTime(node)))
        {
    ...
    // send UDP data
    Message * msgVbr = APP_UdpSendNewHeaderVirtualDataWithPriority(
        node,
        APP_VBR_SERVER,
        dataPtr->clientAddr,
...
static void enterVbrServerReceivesData(Node* node,
    ...
    {
        dataPtr->sessionStart = getSimTime(node);
    }
...
void VbrRunTimeStat(Node* node, VbrData* dataPtr) {
    ...
    {
        if (node->guiOption) {
            GUI_SendIntegerData(node->nodeId, dataPtr->stats.BytesSentId, dataPtr->appStats->GetDataSent().GetValue(now), now);
            GUI_SendIntegerData(node->nodeId, dataPtr->stats.BytesReceivedId, dataPtr->appStats->GetDataReceived().GetValue(now), now);
        }
    }
}
