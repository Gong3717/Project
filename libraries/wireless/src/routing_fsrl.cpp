...
void FsrlPrintLandmarkTable(Node* node, FsrlData* fisheye)
{
    ...
    TIME_PrintClockInSecond(getSimTime(node), timeInSecond);
                            ...
                            int comingSeq)
{
    ...
    if (fisheye->drifterTable.row == NULL)
    {
        ...
        fisheye->drifterTable.row->lastModified = getSimTime(node);
        ...
        while (!found && tmp != NULL)
        {
            ...
            if (tmp->destAddr == destAddr)
            {
                ...
                if (comingSeq > tmp->sequenceNumber)
                {
                    ...
                    tmp->lastModified = getSimTime(node);
                }
         ...
         FsrlData* fisheye)
{
    ...
    while (tmp != NULL)
    {
        ...
        if ((getSimTime(node) - tmp->lastModified) >
                             ...
                             int interfaceIndex)
{
    ...
    if (fisheye->landmarkTable.row == NULL && distance < FISHEYE_INFINITY)
    {
        ...
        fisheye->landmarkTable.row->timestamp = getSimTime(node);
                              ...
                              int incomingInterface)
{
    ...
    if (row->neighborInfo->list == NULL)
    {
        ...
        tmp->timeLastHeard = getSimTime(node);
        tmp->next = NULL;
    ...
    else
    {
        ...
        while ((neighbor != NULL) && (!found))
        {
            ...
            {
                found = TRUE;
                neighbor->timeLastHeard = getSimTime(node);
...
void FsrlHandleNeighborTimeout(Node* node, FsrlData* fisheye)
{
    ...
    for (int i = 0; i < node->numberInterfaces; i++)
    {
        ...
        while (current != NULL)
        {
            ...
            if ((getSimTime(node) - current->timeLastHeard) >
                               ...
                               const int interfaceIndex)
{
    ...
    switch (MESSAGE_GetEvent(msg))
    {
        ...
        default:
            fprintf(stderr,
                    "FSRL: Unknown MSG type %d!\n",
                    msg->eventType);
            ERROR_Assert(0, "Unknown message type!");
    }
}
