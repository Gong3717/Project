...
static void MacDot11eACInit(Node* node, MacDataDot11* dot11, PhyModel phyModel)
{
    ...
    if (phyModel == PHY802_11a || phyModel == PHY802_11n){
        ...
        if (phyModel == PHY802_11n)
        {
           dot11->ACs[DOT11e_AC_BK].outputBuffer = new OutputBuffer;
           dot11->ACs[DOT11e_AC_BK].isACHasPacket = FALSE;
        }
        ...
        if (phyModel == PHY802_11n)
        {
           dot11->ACs[DOT11e_AC_BE].outputBuffer = new OutputBuffer;
           dot11->ACs[DOT11e_AC_BE].isACHasPacket = FALSE;
        }
        ...
        if (phyModel == PHY802_11n)
        {
           dot11->ACs[DOT11e_AC_VI].outputBuffer = new OutputBuffer;
           dot11->ACs[DOT11e_AC_VI].isACHasPacket = FALSE;
        }
        ...
        if (phyModel == PHY802_11n)
        {
           dot11->ACs[DOT11e_AC_VO].outputBuffer = new OutputBuffer;
           dot11->ACs[DOT11e_AC_VO].isACHasPacket = FALSE;
        }
    ...
    int interfaceIndex)
{
    ...
    sprintf(buf, "Unicast packets sent to channel = %d",
           (int) (stats->GetDataFramesSent(STAT_Unicast).GetValue(getSimTime(node)) + 
           stats->GetControlFramesSent(STAT_Unicast).GetValue(getSimTime(node))));
    IO_PrintStat(node, "MAC", DOT11_MAC_STATS_LABEL, ANY_DEST, interfaceIndex, buf);

    sprintf(buf, "Broadcast packets sent to channel = %d",
           (int) (stats->GetDataFramesSent(STAT_Broadcast).GetValue(getSimTime(node)) + 
           stats->GetControlFramesSent(STAT_Broadcast).GetValue(getSimTime(node))));
    IO_PrintStat(node, "MAC", DOT11_MAC_STATS_LABEL, ANY_DEST, interfaceIndex, buf);

    sprintf(buf, "Unicast packets received clearly = %d",
           (int) (stats->GetDataFramesReceived(STAT_Unicast).GetValue(getSimTime(node)) + 
           stats->GetControlFramesReceived(STAT_Unicast).GetValue(getSimTime(node))));
    IO_PrintStat(node, "MAC", DOT11_MAC_STATS_LABEL, ANY_DEST, interfaceIndex, buf);

    sprintf(buf, "Broadcast packets received clearly = %d",
           (int) (stats->GetDataFramesReceived(STAT_Broadcast).GetValue(getSimTime(node)) + 
           stats->GetControlFramesReceived(STAT_Broadcast).GetValue(getSimTime(node))));
    IO_PrintStat(node, "MAC", DOT11_MAC_STATS_LABEL, ANY_DEST, interfaceIndex, buf);
        ...
        int* networkType)
{
    ...
    int acIndex = 0;
    int acCounter = 0;
    for (acCounter = 0; acCounter < DOT11e_NUMBER_OF_AC;
         acCounter++)
    {
        ...
        MacDataDot11* dot11)
{
    ...
    int lastAcIndex = dot11->currentACIndex;
    if (dot11->isHTEnable)
    {
        MacDot11nSetCurrentAC(dot11);
        if (dot11->ACs[dot11->currentACIndex].frameInfo == NULL)
        {
            setCurrentMsg
                = MacDot11nSetMessageInAC(node, dot11, dot11->currentACIndex);
        }
    }
    else
    {
             ...
             acCounter++)
    {
        ...
        if (dot11->isHTEnable)
        {
            sprintf(buf, "AC[%d] Total Frame Retried = %u",
                      acCounter, dot11->ACs[acCounter].totalNoOfthisTypeFrameRetried);
            IO_PrintStat(node, "MAC", DOT11e_MAC_STATS_LABEL, ANY_DEST,
                         interfaceIndex, buf);
    ...
    Node* node, MacDataDot11* dot11, DOT11_DataRateEntry* rateInfo) 
{
    ...
    if (htInfo ) {
        int maxMcs = std::min(htInfo->maxMcsIdx, Phy802_11nGetNumAtnaElems(
                    ...
                    const MAC_PHY_TxRxVector* rxFrameVector)
{
    ...
    if (Phy802_11nShortGiCapable(node->phyData[dot11->myMacData->phyNumber]) 
        && destHtInfo && destHtInfo->shortGiCapable && txVector->mcs > 7) 
    {
        //Response frames
                      ...
                      const DOT11_NeighborHtInfo* destHtInfo)
{
    if (destHtInfo && destHtInfo->stbcCapable) {
        int nSts = Phy802_11nGetNumAtnaElems(node->phyData[dot11->myMacData->phyNumber]);
...
static
void MacDot11nSetChnlBwdth(Node* node,
                           MacDataDot11* dot11,
                           MAC_PHY_TxRxVector* txVector,
                           DOT11_NeighborHtInfo* htInfo,
                           BOOL isDataFrame)
{
     ...
     if (htInfo!=NULL)
     {
        ...
        if (!(htInfo->isFortyMhzIntolerant)||
             !isDataFrame)
         {
             txVector->chBwdth =  CHBWDTH_20MHZ;
         }
    ...
    const MAC_PHY_TxRxVector* rxFrameVector)
{
    ...
    if (frameType == DOT11_DATA 
        ...
        MacDot11nSetChnlBwdthForDataMgmtFrame(
            node, dot11, &txVector, htInfo, isBroadcast);
    }
    ...
    // Else a control response frame
        ...
        {
            ERROR_Assert(rxFrameVector->mcs < 8,"MCS greater than 8");
        }
        ...
        || frameType == DOT11N_DELAYED_BLOCK_ACK)
    {
         MacDot11nSetMCSIndexForBroadcastFrame(
            node, dot11, frameType, destAddr, &txVector, htInfo);
         MacDot11nSetChnlBwdthForBroadcastFrame(node, dot11, &txVector);
    }

    MacDot11nSetStbc(node, dot11, frameType, &txVector, htInfo);
    MacDot11nSetGiType(node, dot11, frameType, &txVector, htInfo, rxFrameVector);
    return txVector;
}
...
BOOL MacDot11IsACsHasMessage(Node* node, MacDataDot11* dot11)
{
    ...
    else
    {
            ...
            acCounter++)
        {
            if (dot11->ACs[acCounter].frameInfo != NULL)
            {
                    if (dot11->ACs[acCounter].frameInfo->msg !=
                        NULL)
                    {
                        return TRUE;
                    }
...
// + eventType      : MacDBEventType : Db event type
// + macParam       : StatsDBMacEventParam & : Reference to
//                    StatsDBMacEventParam structure
// + isMyFrame      : BOOL & : Reference to isMyFrame
// + isAnyFrame     : BOOL & : Reference to isAnyFrame
// RETURN    :: NULL
// **/
void MacDot11GetPacketProperty(Node *node, Message *msg,
