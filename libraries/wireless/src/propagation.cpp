        ...
        case ITM:
        {
            ...
            if (((terrainData->getCoordinateSystem() == CARTESIAN) &&
                 (pathProfile->fromPosition.cartesian.y >=
                  pathProfile->toPosition.cartesian.y)) || // LATLONALT
                             ...
                             &(pathProfile->toPosition))))
                    {
                        ...
                        else
                        {
                            ...
                            RANDOM_SetSeed(oparSeed,
                                           ...
                                           (int) (node->partitionData->theCurrentTime >> 8));
         ...
         PropPathProfile* pathProfile)
{
    ...
    if (txNode->partitionData->isAgiInterfaceEnabled)
    {
        ...
        req.time_nanoseconds = getSimTime(txNode);
        req.xmtrId = xmtrId;
        req.rcvrId = rcvrId;
...
void PROP_AddNodeToList(Node *node, int channelIndex)
{
    ...
    if (nodeInList == FALSE) {
        ...
        oldProfile = node->propData[channelIndex].pathProfile;
        if (oldProfile != NULL) {
            MEM_free(oldProfile);
        }
        ...
        newProfile = (PropPathProfile*)MEM_malloc(newPathProfileSize * sizeof(PropPathProfile));
        memset(newProfile, 0, (newPathProfileSize * sizeof(PropPathProfile)));
        ...
        for (i = 0; i < propChannel->numNodes; i++) {
            propChannel->nodeList[i]->propData[channelIndex].pathProfile[switchIndex].propDelay = 0;
            propChannel->nodeList[i]->propData[channelIndex].pathProfile[switchIndex].distance = 0.0;
            propChannel->nodeList[i]->propData[channelIndex].pathProfile[switchIndex].txDOA.azimuth = 0;
            propChannel->nodeList[i]->propData[channelIndex].pathProfile[switchIndex].txDOA.elevation = 0;
            propChannel->nodeList[i]->propData[channelIndex].pathProfile[switchIndex].rxDOA.azimuth = 0;
            propChannel->nodeList[i]->propData[channelIndex].pathProfile[switchIndex].rxDOA.elevation = 0;
            propChannel->nodeList[i]->propData[channelIndex].pathProfile[switchIndex].pathloss_dB = NEGATIVE_PATHLOSS_dB;
            propChannel->nodeList[i]->propData[channelIndex].pathProfile[switchIndex].sequenceNum = -1;
        }

        // update switched node's list
        for (i = 0; i < switchIndex; i++) {
            newProfile[i].propDelay = 0;
            newProfile[i].distance = 0.0;
            newProfile[i].txDOA.azimuth = 0;
            newProfile[i].txDOA.elevation = 0;
            newProfile[i].rxDOA.azimuth = 0;
            newProfile[i].rxDOA.elevation = 0;
            newProfile[i].pathloss_dB = NEGATIVE_PATHLOSS_dB;
            newProfile[i].sequenceNum = -1;
        }
    }
}
