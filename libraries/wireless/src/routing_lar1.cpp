                                      ...
                                      Lar1Data *lar1)
{
    ...
    {
        if (head->lifetime < getSimTime(node))
        {
                                  ...
                                  int seqNum)
{
    ...
    entry->seqNum = seqNum;
    entry->lifetime = getSimTime(node) + LAR1_REQUEST_SEEN_LIFETIME;
                                 ...
                                 NodeAddress destAddr)
{
    ...
    if (!found){
        ...
        radius = velocity * (double)((getSimTime(node) - destinationTime) / SECOND);
#ifdef DEBUG
