                         ...
                         OdmrpData* odmrp)
{
    ...
    {
        if ((forwardTable[i].mcastAddr == mcastAddr)
            && (getSimTime(node) - forwardTable[i].timestamp
            >= odmrp->expireTime))
        {
            return TRUE;
        }
                       ...
                       DataBuffer* buffer)
{
    ...
    for (i = 0; i < numEntry; i++)
    {
        if (forwardTable[i].mcastAddr == mcastAddr)
        {
            forwardTable[i].timestamp = getSimTime(node);
            break;
        }
    }
                    ...
                    DataBuffer* fgFlag)
{
    ...
    forwardTable.timestamp = getSimTime(node);
                  ...
                  OdmrpData* odmrp)
{
    ...
    ssTable.minExpireTime = getSimTime(node) + odmrp->refreshTime;
    ssTable.lastSent = getSimTime(node);
                                ...
                                DataBuffer* buffer)
{
    ...
    for (i = 0; i < numEntry; i++)
    {
        ...
        {
            memSrcTable[i].timestamp = getSimTime(node);
            return TRUE;
        }
    }
    return FALSE;
                             ...
                             OdmrpMtNode* memberTable)
{
    ...
    {
        ++(memberTable->size);
        memberTable->queryLastReceived = getSimTime(node);
        memSrcTable.srcAddr = srcAddr;
        memSrcTable.timestamp = getSimTime(node);
                             ...
                             DataBuffer* buffer)
{
    ...
    for (i = 0; i < numEntry; i++)
    {
        if (memberTable[i].mcastAddr == mcastAddr)
        {
            ...
            for (j = 0; j < numSrc; j++)
            {
                if (getSimTime(node) - memSrcTable[j].timestamp >=
                    ODMRP_MEM_TIMEOUT)
                {
                                ...
                                clocktype  timeVal)
{
    ...
    for (i = 0; i < numEntry; i++)
    {
        ...
        {
            tempSrcTable[i].timestamp = getSimTime(node);
            tempSrcTable[i].FGExpireTime = getSimTime(node)
                + timeVal;
            return TRUE;
                            ...
                            OdmrpTtNode* tempTable)
{
        ...
        odmrp->expireTime))
    {
        ...
        tempSrcTable.timestamp = getSimTime(node);
        tempSrcTable.FGExpireTime = getSimTime(node) + odmrp->expireTime;
                         ...
                         clocktype timeVal)
{
    ...
    tempSrcTable.srcAddr = srcAddr;
    tempSrcTable.timestamp = getSimTime(node);
    tempSrcTable.FGExpireTime = getSimTime(node) + timeVal;
                           ...
                           DataBuffer* buffer)
{
    ...
    for (i = 0; i < numEntry; i++)
    {
        if (tempTable[i].mcastAddr == mcastAddr)
        {
            ...
            for (j = 0;j < numSrc; j++)
            {
                if (getSimTime(node) - tempSrcTable[j].timestamp >=
                    ODMRP_JR_LIFETIME)
                {
                           ...
                           OdmrpAtNode* ackTable)
{
    ...
    {
        ++(ackTable->size);
        ackTable->lastSent = getSimTime(node);
                    ...
                    NodeAddress mcastAddr)
{
    ...
    if (!OdmrpLookupMembership(mcastAddr, &odmrp->memberFlag))
    {
        ...
        memberFlag.timestamp = getSimTime(node);
                        ...
                        DataBuffer* buffer)
{
    ...
    {
        if ((memTable[i].mcastAddr == mcastAddr) &&
            (getSimTime(node) - memTable[i].lastSent
            > ODMRP_JR_PAUSE_TIME))
        {
            return TRUE;
        }
                         ...
                         DataBuffer* buffer)
{
    ...
    for (i = 0; i < numEntry; i++)
    {
        if (memTable[i].mcastAddr == mcastAddr)
        {
            memTable[i].lastSent = getSimTime(node);
            return;
        }
    }
    return;
                          ...
                          DataBuffer* buffer)
{
    ...
    {
        if ((sentTable[i].mcastAddr == mcastAddr)
            && (getSimTime(node) > (sentTable[i]).minExpireTime))
        {
            return TRUE;
                              ...
                              DataBuffer* buffer)
{
    ...
    {
        if ((memTable[i].mcastAddr == mcastAddr) &&
            (getSimTime(node) - memTable[i].queryLastReceived
    ...
    for (i = 0; i < numEntry; i++)
    {
...

        {
            sentTable[i].minExpireTime
                = getSimTime(node) + odmrp->refreshTime;
            return;
        }
    }
    return;
...

{
    ...
    if (tempReplyPkt.replyCount > 0)
    {
        ...
        {
            char clockStr[100];
            ctoa(getSimTime(node) + delay, clockStr);
               ...
               int interfaceIndex)
{
    ...
    if (odmrp == NULL)
    {
        ...
        ERROR_Assert (FALSE,
            "Odmrp Can not allocate memeory\n");
    }
                            ...
                            OdmrpData* odmrp)
{
     ...
     Node* node)
 {
     ...
     while (current != NULL) {
         if (getSimTime(node) - current->lastRecvTime >
             odmrp->clsExpireTime)
         {
             toFree = current;
             if (ODMRP_PASSIVE_DEBUG)
...

 {
     ...
     if (clsHeadList->head == NULL)
     {
         ...
         newOne->lastRecvTime =  getSimTime(node);
         newOne->next = NULL;
     ...
     else
     {
         ...
         while (current != NULL)
         {
             ...
             {
                 current->lastRecvTime =  getSimTime(node);
                 isFound = TRUE;
                 break;
             }
             else
