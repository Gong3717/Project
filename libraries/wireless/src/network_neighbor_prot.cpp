    ...
    NodeAddress ipAddr)
{
    ...
    for (i = 0; i < neighborProt->numNbrInfoRecords; i++)
    {
        if ((neighborProt->nbrInfo[i].neighbor == ipAddr) &&
            (neighborProt->nbrInfo[i].lastHeard == CLOCKTYPE_MAX))
        {
            neighborProt->nbrInfo[i].lastHeard = getSimTime(node);
            added = TRUE;
        }
    ...
    if (!added)
    {
        ...
        neighborProt->nbrInfo[index].lastHeard = getSimTime(node);
    ...
    else
    {
        ...
        APP_SetTimer(
            ...
            expirationTime - getSimTime(node));
    ...
    Message * msg)
{
    ...
    {
        infoRecord->lastHeard = getSimTime(node);
    }
    ...
    Node* node)
{
    ...
    tooOldTime = getSimTime(node) - neighborProt->entryLifetime;
    ...
    Message * msg)
{
    switch (MESSAGE_GetEvent(msg))
    {
        ...
        case MSG_APP_TimerExpired:
            ...
            break;
        default:
            fprintf(stderr, "(node %d) NeighborProtocolProcessMessage received "
                    "unknown Message Event %d\n", node->nodeId,
