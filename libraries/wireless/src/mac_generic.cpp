...
static void enterGenericMacEplrsDequeuePacket(Node* node,
                                              GenericMacData* dataPtr,
                                              Message *msg);
static void GenericMacUpdateScheduler(Node* node, GenericMacData* dataPtr, Message* msg);
...
static void GenericMacSetBO(Node* node,GenericMacData* dataPtr) {
    if (dataPtr->BO == 0) {
#ifdef MILITARY_RADIOS_LIB
        if (dataPtr->csmaMode == MGENERIC_CSMA_SHORT)
        {
             dataPtr->BO = dataPtr->slot;
             return ;
        }else if (dataPtr->csmaMode == MGENERIC_CSMA_NORMAL)
        {
            ...
            //dataPtr->BO = dataPtr->waitTimeInTSB * dataPtr->slot - dataPtr->phyIdle;
            clocktype tclk = getSimTime(node);
            ERROR_Assert(dataPtr->slot, "Slotted CSMA timeslot equal to 0");
            ERROR_Assert(dataPtr->waitTimeInTSB, "Slotted CSMA-NORMAL waitTimeInTSB is 0");

            if (dataPtr->phyIdle == 0){
                dataPtr->BO = dataPtr->waitTimeInTSB * dataPtr->slot;
            }
            else {
                dataPtr->BO = dataPtr->waitTimeInTSB * dataPtr->slot - (tclk - dataPtr->phyIdle);
            }
            if (MAC_GENERIC_DEBUG) {
                printf(" Node %d slotted CSMA-NORMAL Backoff timer = %ld \n",
                    node->nodeId, dataPtr->BO);
            }
            if (dataPtr->BO < 0)
            {
                dataPtr->BO = 1;
            }
            ERROR_Assert((dataPtr->BO>0), "Slotted CSMA-NORMAL Backoff timer <= 0");
            return ;
        }
#endif // MILITARY_RADIOS_LIB
        clocktype nRand = RANDOM_nrand(dataPtr->seed);
        dataPtr->BO = (nRand % dataPtr->CW) * dataPtr->backoff_mul;
        dataPtr->lastBOTimestamp = getSimTime(node);
...
static void GenericMacHandOffSuccessfullyReceivedUnicast(
    ...
    EplrsData* eplrsData = ip->eplrsData;
    if (EplrsActiveOnInterface(node, dataPtr->myMacData->interfaceIndex))
    {
        EplrsHandOffSuccessfullyReceivedPacket(
                                           node,
...
#ifdef MILITARY_RADIOS_LIB
    if (dataPtr->csmaMode == MGENERIC_CSMA_BASIC){
#endif // MILITARY_RADIOS_LIB
...
#ifdef MILITARY_RADIOS_LIB
void MacGenericMacEplrsIncHasPacketToSend(Node* node, int interfaceIndex)
{
    ...
    ERROR_Assert (idle == TRUE, "EPLRS slotted CSMA start of slot should be idle");


    dataPtr->state = GENERICMAC_DEQUEUEPACKET;
    enterGenericMacEplrsDequeuePacket(node, dataPtr, NULL);
     ...
     if (dataPtr->status == MGENERIC_S_YIELD && data)
     {
        ...
        //during backoff, process the packets.
         if (ifMine){
            ...
            dataPtr->BO = dataPtr->BO - (getSimTime(node) - dataPtr->lastBOTimestamp);
            dataPtr->lastBOTimestamp = getSimTime(node);
            process = TRUE;
        }
        else{
...
static void GenericMacRecvOrSrcEOT(Node * node, GenericMacData* dataPtr)
{
    ...
    if (dataPtr->phyIdle == 0)
    {
        ...
        dataPtr->phyIdle = getSimTime(node);
    }
...
{
    clocktype backoff = (getSimTime(node) - dataPtr->lastBOTimestamp);
...
static void enterGenericMacCSMA_CSMACA_Branch(Node* node,
            ...
            || dataPtr->channelAccess == SL_CSMA)
    {
        if (dataPtr->BO != 0)
        {
            ...
            dataPtr->BO = dataPtr->BO - (getSimTime(node)
            ...
            else
            {
             ...
             /*dataPtr->lastBOTimestamp = getSimTime(node); // not required*/
                 Message *msg = NULL;
...
static void enterGenericMacBusyToIdle(Node* node,
    ...
    *((int*)MESSAGE_ReturnInfo(newMsg)) = dataPtr->timerSequenceNumber;
    ...
    dataPtr->csma_ca_idle_start = TIME_getSimTime(node);
...
#ifdef MILITARY_RADIOS_LIB
    // if running EPLRS, no need to yield and dequeue packet in next slot
    // everything should be controlled by EPLRS
    if (EplrsActiveOnInterface(node, dataPtr->myMacData->interfaceIndex))
    {
...
static void enterGenericMacIdleToBusy(Node* node,
    ...
    clocktype timePassed = TIME_getSimTime(node) - dataPtr->csma_ca_idle_start;
                                              ...
                                              Message *msg)
{
    ...
    ERROR_Assert(!MAC_OutputQueueIsEmpty(node, dataPtr->myMacData->interfaceIndex),
                 "MacGenericDequeuePacket: "
                 "No packet in the Queue.\n");

    MAC_OutputQueueTopPacket(node, interfaceIndex, &dataPtr->currentMessage,
                             &(dataPtr->currentNextHopAddress), &networkType, &priority);

    MAC_OutputQueueDequeuePacketForAPriority(node, interfaceIndex, priority,
                                             &(dataPtr->currentMessage),
                                             &(dataPtr->currentNextHopAddress),
                                             &networkType);

    dataPtr->ipNextHopAddr = dataPtr->currentNextHopAddress;

    EplrsCTUHeader* headerField = (EplrsCTUHeader*)
        MESSAGE_ReturnPacket(dataPtr->currentMessage);

    dataPtr->ipSourceAddr = headerField->srcAddr;
    dataPtr->ipDestAddr = headerField->destAddr;


    if (MAC_GENERIC_DEBUG)
    {
        //printf("Message Packet : %d\n", MESSAGE_ReturnPacket(dataPtr->currentMessage));
        printf("DEQUEUING node %d, destination address : %s\n", node->nodeId,
                nextHopStr);
    }

    NetworkDataIp *ip = (NetworkDataIp *) node->networkData.networkVar;
    EplrsData* eplrsData = ip->eplrsData;
    MacDataEplrsTdma *eplrs = &eplrsData->iface[interfaceIndex];

    //printf ("node %d ackMode %d ipSrcAddr 0x%x ipDestAddr 0x%x headerField->destAddr 0x%x nodeSeqNum %d\n",
   //     node->nodeId, eplrs->ackMode, dataPtr->ipSourceAddr, dataPtr->ipDestAddr, headerField->destAddr, headerField->nodeSeqNum);

    if ((eplrs->ackMode)&&
        (!NetworkIpIsMulticastAddress(node, dataPtr->ipDestAddr))&&
        (dataPtr->ipDestAddr != ANY_DEST))
    {
        //if the packet is not an eplrs originated ACK and if the packet is a DATA packet
        //originated by this node, copy the msg into retx queue
        if ((headerField->originatingProtocol != TRACE_EPLRS)
            &&(NetworkIpIsMyIP(node, dataPtr->ipSourceAddr)))
        {
            Message* newMessage;
            QueuedPacketInfo *infoPtr;
            QueuedPacketInfo* infoCurrentMsgPtr;
            const void *infoField = NULL;
            BOOL QueueStatus = FALSE;
            EplrsSeqNoEntry* entry;

            entry = EplrsGetSeqNo(node, eplrsData, dataPtr->ipDestAddr, FALSE );

            ERROR_Assert(entry,
                         "Unable to find or create sequence number entry.\n");

            if (entry->pendingAckCount < (eplrsData->slidingWindowSize-1))
            {
                entry->pendingAckCount++;
                entry->waitingForAck = FALSE;
            }
            else
            {
                entry->pendingAckCount++;
                entry->waitingForAck = TRUE;  //wait for dest to ACK before continue sending
            }

            headerField->unicastSeqNum = entry->toSeqNo;

            newMessage = MESSAGE_Duplicate(node, dataPtr->currentMessage);

            MESSAGE_InfoAlloc(node, newMessage, sizeof(QueuedPacketInfo));
            infoPtr = (QueuedPacketInfo *) MESSAGE_ReturnInfo(newMessage);
            infoCurrentMsgPtr = (QueuedPacketInfo *)
                               MESSAGE_ReturnInfo((dataPtr->currentMessage));
            infoPtr->nextHopAddress = infoCurrentMsgPtr->nextHopAddress;
            infoPtr->destinationAddress.ipv4DestAddr = dataPtr->ipDestAddr;

            memcpy(infoPtr->macAddress,
                   dataPtr->currentNextHopAddress.byte,
                   MAC_ADDRESS_DEFAULT_LENGTH);
            infoPtr->hwLength = MAC_ADDRESS_DEFAULT_LENGTH;
            infoPtr->hwType = HW_TYPE_ETHER;

            if (!MESSAGE_ReturnInfo(newMessage, INFO_TYPE_OriginalInsertTime))
            {
                ...
                memcpy(MESSAGE_ReturnInfo(newMessage, INFO_TYPE_OriginalInsertTime),
                       &currentTime, sizeof(clocktype));

            }

            int queue_index = EplrsIncReturnNeedlineQueueIndex(node, interfaceIndex);
            ERROR_Assert(queue_index >= 0, "should not enter here");
            EplrsNeedlineQueue* thisNeedline = &eplrs->needlineQueueList[queue_index];

            thisNeedline->retxQueue->insert(newMessage,
                                    infoField,
                                    &QueueStatus,
                                    TIME_getSimTime(node));

        }
        else if (headerField->originatingProtocol != TRACE_EPLRS)
        {

        }
        else
        {
            //printf ("%f: eplrs node %d sending ACK to 0x%x\n",
            //    (double)getSimTime(node)/SECOND, node->nodeId, dataPtr->ipDestAddr);
        }
    }
...
static void enterGenericMacDequeuePacket(Node* node,
    ...
    dataPtr->state = GENERICMAC_DEQUEUEPACKET;
#ifdef MILITARY_RADIOS_LIB
    //if EPLRS is enabled, use EPLRS specific dequeue at desired location
                            ...
                            GenericMacData* dataPtr)
{
    ...
    clocktype nclk = getSimTime(node);
...
GenericMacInitScheduler(Node* node, GenericMacData* dataPtr)
{
    ...
    if (MAC_GENERIC_SCHEDULER) {
        printf("node %d initialize scheduler at %f, next slot at %f\n",
            node->nodeId, (double)TIME_getSimTime(node)/SECOND,
            (double)(TIME_getSimTime(node) + dataPtr->slot)/SECOND);
    }
}
...
static
BOOL GenericMacEplrsActive(Node* node,
...
GenericMacInit(Node* node, int interfaceIndex,
    ...
    {
        if (GenericMacEplrsActive(node, nodeInput, interfaceIndex) &&
             dataPtr->channelAccess == PSMA)
        {
            // eplrs will allocate and set a lookahead EOT handle
        }
        else
        {
            PARALLEL_SetProtocolIsNotEOTCapable(node);
