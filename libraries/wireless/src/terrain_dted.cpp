    ...
    double*            lowest)
{
    ...
    north = MIN(ne->latlonalt.latitude,
                record->northEastCorner.latlonalt.latitude);
    south = MAX(sw->latlonalt.latitude,
                record->southWestCorner.latlonalt.latitude);
    east  = MIN(ne->latlonalt.longitude,
                record->northEastCorner.latlonalt.longitude);
    west  = MAX(sw->latlonalt.longitude,
                record->southWestCorner.latlonalt.longitude);

    normalizedLongitude =
        (west - record->southWestCorner.latlonalt.longitude)
        / record->resolution[0];
    normalizedLatitude =
        (south - record->southWestCorner.latlonalt.latitude)
        / record->resolution[1];

    southIndex = (int) ceil(normalizedLatitude);
    westIndex  = (int) floor(normalizedLongitude);

    normalizedLongitude =
        (east - record->southWestCorner.latlonalt.longitude)
        / record->resolution[0];
    normalizedLatitude =
        (north - record->southWestCorner.latlonalt.latitude)
        / record->resolution[1];

    northIndex = (int) ceil(normalizedLatitude);
    eastIndex  = (int) floor(normalizedLongitude);

    assert(northIndex >= southIndex);
    assert(eastIndex >= westIndex);
