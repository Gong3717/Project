    ...
    BOOL incrementSeq)
{
    ...
    current->lifetime = getSimTime(node) + HWMP_DELETION_PERIOD;
    ...
    BOOL isRelay)
{
    ...
    hwmp->lastBroadcastSent = getSimTime(node);
    ...
    HWMP_RouteEntry* rtToDest)
{
    ...
    rrepData.lifetime = rtToDest->lifetime - getSimTime(node);
    ...
    int interfaceIndex)
{
    ...
    ptrToPrevHop =
        HwmpRouteTable_UpdateEntry(node, dot11, hwmp,
            ...
            sourceAddr,
            getSimTime(node) + HWMP_ACTIVE_ROUTE_TIMEOUT,
            TRUE,                // is active
            interfaceIndex);
    ...
    HWMP_Data* hwmp)
{
    ...
    rootItem->lastRannTime = getSimTime(node);
    ...
    int interfaceIndex)
{
    ...
    if (rannData.rootAddr != prevHopAddr)
    {
        HwmpRouteTable_UpdateEntry(node, dot11, hwmp,
            ...
            metricToPrevHopAddr,
            prevHopAddr,
            getSimTime(node) + rannData.lifetime,
            TRUE,                   // isActive
            interfaceIndex);
    ...
    HWMP_RannData* rxRannData)
{
    ...
    rannData.lifetime -=
        getSimTime(node) - hwmp->currentParent->lastRannTime;
    ...
    HWMP_Data* hwmp)
{
    ...
    case HWMP_TBR_S_EVENT_RREP_RECEIVED:
    {
        ...
        if (parent != NULL)
        {
            ...
            parent->lastValidationTime = getSimTime(node);
