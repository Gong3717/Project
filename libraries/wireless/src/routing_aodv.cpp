                 ...
                 BOOL incrementSeq)
{
    ...
    current->lifetime = getSimTime(node) + AODV_DELETE_PERIOD;
         ...
         Address previousHopAddress)
{
    ...
    if (rtEntryToDest->lifetime < getSimTime(node)
        + AODV_ACTIVE_ROUTE_TIMEOUT)
    {
        rtEntryToDest->lifetime = getSimTime(node) +
                                      AODV_ACTIVE_ROUTE_TIMEOUT;
         ...
         BOOL isRelay)
{
    ...
    aodv->lastBroadcastSent = getSimTime(node);
         ...
         AodvRouteEntry* rtEntryToSrc)
{
    ...
    if ((unsigned int) ((rtEntryToDest->lifetime - getSimTime(node)) /
        MILLI_SECOND) == 0)
    {
        return;
    }
         ...
         AodvRouteEntry* rtToDest)
{
    ...
    // we don't want to send RREP if life time becomes 0
    if ((unsigned int) ((rtToDest->lifetime - getSimTime(node)) /
                            MILLI_SECOND) == 0)
    {
        return;
    }
                                     ...
                                     &aodv->seenTable))
    {
        ...
        if (!Address_IsSameAddress(&sourceAddress,  &srcAddr))
        {
            ...
            ptrToPrevHop = AodvReplaceInsertRouteTable(
                               ...
                               1,
                               srcAddr,
                               getSimTime(node) + AODV_ACTIVE_ROUTE_TIMEOUT,
                               TRUE,
                               interfaceIndex,
                               &aodv->routeTable);
         ...
         Address previousHopAddress)
{
    ...
    else
    {
        ...
        if (rtToDest && (!isValidRoute) && aodv->localRepair)
        {
            ...
            rtToDest->lifetime = getSimTime(node) + AODV_DELETE_PERIOD;
    ...
    Message* msg)
{
    ...
    switch (MESSAGE_GetEvent(msg))
    {
        ...
        case MSG_NETWORK_CheckRouteTimeout:
        {
            ...
            while (current && current->lifetime <= getSimTime(node))
            {
                rtPtr = current;
        ...
        case MSG_NETWORK_SendHello:
        {
            ...
            if (aodv->lastBroadcastSent < (getSimTime(node) -
                AODV_HELLO_INTERVAL))
            {
