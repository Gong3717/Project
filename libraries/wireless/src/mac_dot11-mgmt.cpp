    ...
    DOT11_Frame* msg)
{
    ...
    *frameInfo = NULL;
    ...
    if (dot11->isHTEnable)
    {
        unsigned char* htCapabilitiesElementInBeacon = NULL;
    ...
    DOT11_Frame* rxFrame)
{
      ...
      if (Dot11_GetCfsetFromBeacon(node, dot11, rxFrame, &cfSet))
        ...
        if (dot11->isHTEnable)
        {
            ...
            PHY_GetRxVector(node,
                            ...
                            rxVector);
            beaconTransmitTime = 
                getSimTime(node) - PHY_GetTransmissionDuration(
                                        node,
                                        dot11->myMacData->phyNumber,
                                        rxVector);
        }
        else
        {
            ...
            beaconTransmitTime = getSimTime(node) -
    ...
    if (MacDot11IsBssStation (dot11)){
        ...
        if (dot11->isHTEnable)
        {
            ...
            unsigned char* htCapabilitiesElementInBeacon = NULL;
            int length = MESSAGE_ReturnPacketSize(rxFrame);
            DOT11_HT_CapabilityElement htCapabilityElement;
            BOOL isPresent = Dot11_GetHTCapabilities(
                node,
                dot11,
...
//--------------------HCCA-Updates Start---------------------------------//
        ...
        BOOL isPresent = Dot11_GetHTOperation(node, dot11, rxFrame, &operElem,
                sizeof(DOT11e_AssocRespFrame) /*+ 
                    sizeof(DOT11e_EDCAParameterSet) + 2 + sizeof(htCapabilityElement)*/);
        if (isPresent)
        {
    ...
    MacDataDot11* dot11)
{
    ...
    // Alloc Association Request frame
    Message* msg = NULL;/*MESSAGE_Alloc(node, 0, 0, 0);*/
