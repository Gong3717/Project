    ...
    int channelIndex)
{
    ...
    phy_abstract->rxTimeEvaluated = getSimTime(node);
                                   ...
                                   double *sinrp)
{
    ...
    if (phy_abstract->thisPhy->phyRxModel == SNR_THRESHOLD_BASED) {
        ...
        if (BER != 0.0) {
            ...
            {
                numBits =
                        ((double)(getSimTime(node) - phy_abstract->rxTimeEvaluated)
                        * (double)phy_abstract->dataRate /(double)SECOND);
    ...
    const NodeInput *nodeInput)
{
    ...
    IO_ReadInt(
        node->nodeId,
        phy_abstract->thisPhy->networkAddress,
        nodeInput,
    ...
    if (hierarchy->IsEnabled()) {
        ...
        if (createDataRatePath) {
            hierarchy->AddObject(
                dataRatePath,
                new D_Int32Obj(&phy_abstract->dataRate));
        }
...
;
    ...
    phy_abstract->stats.turnOnTime             = getSimTime(node);
    phy_abstract->stats.numUpdates = 0.0;
#ifdef CYBER_LIB
    ...
    if (DYNAMIC_STATS) {
         ...
         phy_abstract->stats.totalRxSignalsToMacId =
             GUI_DefineMetric("Signals received and forwarded to MAC",
                              ...
                              GUI_INTEGER_TYPE, GUI_CUMULATIVE_METRIC);
         phy_abstract->stats.currentSINRId =
             GUI_DefineMetric("Current SINR (dB)",
                              node->nodeId, GUI_PHY_LAYER, 0,
    ...
    PropRxInfo *propRxInfo)
{
	...
	//xyt
	/*if (node->nodeId == 1)
		cout << "signal from " << propRxInfo->txNodeId << " to 1 "  << " rxpower=" << ANTENNA_GainForThisSignal(node, phyIndex, propRxInfo)
		<<"+"<< propRxInfo->rxPower_dBm << endl;*/
	//xyt end
    ...
    if (phy_abstract->thisPhy->phyStats)
    {
        ...
        if (DYNAMIC_STATS) {
            GUI_SendUnsignedData(node->nodeId,
                                 phy_abstract->stats.totalSignalsDetectedId,
                                 (unsigned)phy_abstract->thisPhy->stats->GetSignalsDetected().GetValue(getSimTime(node)),
                                 getSimTime(node));
        }
    }
        ...
        case PHY_SENSING:
        {
            ...
            if (rxPower_mW >= phy_abstract->rxThreshold_mW ||
                ...
                if (thisPhy->phyRxModel == PCOM_BASED)
                {
                    ...
                    {
                        char timeStr[MAX_STRING_LENGTH];
                        TIME_PrintClockInSecond(getSimTime(node)+thisPhy->syncCollisionWindow, timeStr);
                ...
                if (DYNAMIC_STATS) {
                  if (phy_abstract->thisPhy->phyStats)
                  {
                      GUI_SendRealData(node->nodeId,
                                       phy_abstract->stats.totalSignalsLockedId,
                                       (unsigned)phy_abstract->thisPhy->stats->GetSignalsLocked().GetValue(getSimTime(node)),
                                       getSimTime(node));
                  }
                }
        ...
        if (!receiveErrorOccurred) {
            ...
            if (phy_abstract->thisPhy->phyStats)
            {
                ...
                if (DYNAMIC_STATS) 
                {
                    GUI_SendUnsignedData(node->nodeId,
                        phy_abstract->stats.totalRxSignalsToMacId,
                        (unsigned) phy_abstract->thisPhy->stats->GetSignalsToMac().GetValue(getSimTime(node)),
                        getSimTime(node));
                }
            }
#ifdef CYBER_LIB
            ...
            if (phy_abstract->thisPhy->phyStats)
            {
                ...
                if (DYNAMIC_STATS)
                {
                    GUI_SendUnsignedData(node->nodeId,
                        phy_abstract->stats.totalSignalsWithErrorsId,
                        (unsigned)phy_abstract->thisPhy->stats->GetSignalsWithErrors().GetValue(getSimTime(node)),
                        getSimTime(node));
                }
            }
#ifdef CYBER_LIB
    ...
    clocktype initDelayUntilAirborne)
{
    ...
    if (phy_abstract->thisPhy->phyStats)
    {
        ...
        if (DYNAMIC_STATS)
        {
            GUI_SendUnsignedData(node->nodeId,
                 phy_abstract->stats.totalTxSignalsId,
                 phy_abstract->thisPhy->stats->GetSignalsTransmitted().GetValue(getSimTime(node)),
                 getSimTime(node));
        }
    }
...
int PhyAbstractGetDataRate(PhyData *thisPhy) {
    PhyDataAbstract* phy_abstract = (PhyDataAbstract*) thisPhy->phyVar;
