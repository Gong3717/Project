    ...
    DsrRequestTable* reqTable)
{
    ...
    if (!reqEntry)
    {
        ...
        DsrAddToRequestTable(reqTable, ROUTING_DSR_SEEN_ENTRY,
            srcAddr, (void *) seenEntry, getSimTime(node));
    ...
    else
    {
        ...
        reqEntry->timeStamp = getSimTime(node);
    ...
    DsrRequestTable* reqTable)
{
    ...
    DsrAddToRequestTable(reqTable, ROUTING_DSR_SENT_ENTRY,
            destAddr, (void *) newNode, getSimTime(node));
}
    ...
    int pathListLength)
{
                 ...
                 && current->hopCount <= hopCount)))
    {
            ...
            && !memcmp(current->path, pathList, pathListLength))
        {
            ...
            current->routeEntryTime = getSimTime(node);
            return current;
        }
        previous = current;
        current = current->next;
   ...
   // newEntry = (DsrRouteEntry *) MEM_malloc(sizeof(DsrRouteEntry));
    ...
    newEntry->routeEntryTime = getSimTime(node);
    newEntry->prev = NULL;
    newEntry->next = NULL;
    ...
    Message* msg)
{
    ...
    while (current && nextTimeOutTime <= getSimTime(node))
    {
    ...
    nextTimeOutTime -= getSimTime(node);
    ...
    NodeAddress nextHopAddress)
{
    ...
    if (rexmtBufEntry && rexmtBufEntry->count < DSR_MAX_MAIN_TREXMT)
    {
        ...
        rexmtBufEntry->timeStamp = getSimTime(node);
        ...
        else
        {
            ...
            *packetWasRouted = TRUE;
            ...
            else
            {
                ...
                if (!reqEntry ||
                    (getSimTime(node) > reqEntry->timeStamp
