         ...
         double cost)
{
  ...
  switch(node->battery->model)
    ...
    /*
      ...
      clocktype Tp = ( getSimTime(node)- uscBat->lastTimeTrans );
  ...
  switch (MESSAGE_GetEvent(msg))
    ...
    switch(node->battery->model)
      ...
      case SERVICE_LIFE_ACCURATE_MODEL:
        ...
        if (thisPhy->curLoad )
          {
            duration = (double)(getSimTime(node) -
                    thisPhy->curLoad->lastUpdate)/(double)SECOND;

            if (duration > period)
              duration = period;
            thisPhy->curLoad->lastUpdate = getSimTime(node);
      ...
      case RESIDUAL_LIFE_ACCURATE_MODEL:
         ...
         (rlaBat->remainingCap <= 0.0))
        {
          ...
          node->battery->deadTime = getSimTime(node);
          shutDownNode(node);
        }
