    ...
    DymoRoutingTable* routeTable)
{
    ...
    current->UsedRouteTimeout = getSimTime(node);
    ...
    Address *prevAddress)
{
    ...
    if (current && Address_IsSameAddress(&current->destination, &targtAddr))
    {
        ...
        {
            if ( current->UsedRouteTimeout < getSimTime(node)
    ...
    Address previousHopAddress)
{
    ...
    if (rtEntryToDest->UsedRouteTimeout
            < (getSimTime(node) + DYMO_USED_ROUTE_TIMEOUT))
    {
        rtEntryToDest->UsedRouteTimeout = getSimTime(node) +
                                      DYMO_USED_ROUTE_TIMEOUT;
    ...
    BOOL isRelay)
{
    ...
    // dymo->lastBroadcastSent = getSimTime(node);
    ...
    ManetMessageType msgType)
{
    ...
    if((rtToSrc == NULL)  ||(rtToSrc->SeqNum <= nodeSeqNum))
    {
        ...
        if((rtToSrc != NULL) && rtToSrc->SeqNum == nodeSeqNum){
            ...
            if((nodeHopCnt == rtToSrc->hopCount) &&
                ...
                (rtToSrc->UsedRouteTimeout > getSimTime(node))*/ &&
                (rtToSrc->activated))
            {
    ...
    if(isRouteNeedToUpdate){
        ...
        DymoReplaceInsertRouteTable(
            ...
            nodeHopCnt,
            srcAddr,
            getSimTime(node) + validityPeriod,
            TRUE,
            interfaceIndex,
            &dymo->routeTable,
            prefixLength);
    ...
    Message* msg)
{
    ...
    switch (msg->eventType)
    {
        ...
        case MSG_NETWORK_CheckRouteTimeout:
        {
            ...
            while (current && current->UsedRouteTimeout <= getSimTime(node))
            {
                rtPtr = current;
       ...
       case MSG_NETWORK_SendHello:
        {
            ...
            clocktype delay = (clocktype)
                (RANDOM_nrand(dymo->dymoJitterSeed) %
                DYMO_BROADCAST_JITTER);
            if (dymo->lastBroadcastSent <= (getSimTime(node) -
                DYMO_HELLO_INTERVAL))
            {
