    ...
    UmtsRrcNodebDataInRnc* nodebRrc)
{
    ...
    newMntNodeb->timeStamp = getSimTime(node);
    ...
    UInt32 cellId)
{
    ...
    IO_ReadString(rrcData->nodebId,
                  MAPPING_GetDefaultInterfaceAddressFromNodeId(
                      node, rrcData->nodebId),
                  node->partitionData->nodeInput,
                  "PHY-UMTS-HSDPA-CAPABLE",
                  &wasFound,
                  buf);
    ...
    const UmtsRcvPhChInfo* dlDpdchInfo)
{
    ...
    {
        char timeStr[20];
        TIME_PrintClockInSecond(TIME_getSimTime(node), timeStr);
    ...
    UmtsNodebSrbConfigPara& nodebSrbConfig)
{
    ...
    nodebRrc->lastUlResUpdateTime = getSimTime(node);
    ...
    nodebRrc->lastDlResUpdateTime = getSimTime(node);
}
             ...
             Message* msg)
{
    ...
    ueRrc->timerMeasureCheck = UmtsLayer3SetTimer(
                                        ...
                                        &ueId,
                                        sizeof(ueId));
    ueRrc->tsLastMeasRep = TIME_getSimTime(node);
}
         ...
         signed char measVal)
{
    ...
    if (it == mntNodeBList->end())
    {
        ...
        nodeBInfo->timeStamp = getSimTime(node);
                                    ...
                                    Address srcAddr)
{
    ...
    if (helloPkt.nodeType == CELLULAR_NODEB)
    {
        ...
        if (itemPtr == NULL)
        {
            ...
            ListInsert(node, rncL3->nodebList, getSimTime(node), itemPtr);
        }
    ...
    else if (helloPkt.nodeType == CELLULAR_RNC)
    {
        ...
        if (itemPtr == NULL)
        {
            ...
            itemPtr->nodeId = helloPkt.nodeId;
            ListInsert(
                node, rncL3->neighRncList, getSimTime(node), itemPtr);
        }
