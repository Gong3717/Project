                            ...
                            const NodeInput* nodeInput)
{
    ...
    if (macCellularData->nodeType == CELLULAR_NODEB)
    {
        ...
        if (!retVal || umtsMacData->currentDLChannel < 0 ||
            umtsMacData->currentDLChannel >= node->numberChannels)
        {
            ERROR_ReportError("Wrong downlink channel!");
        }
        ...
        if (!retVal || umtsMacData->currentULChannel < 0 ||
            umtsMacData->currentULChannel >= node->numberChannels)
        {
            ERROR_ReportError("Wrong uplink channel!");
        }
        else if (PHY_CanListenToChannel(
                     node,
