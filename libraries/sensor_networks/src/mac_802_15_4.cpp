// Copyright (c) 2001-2012, Scalable Network Technologies, Inc.  All Rights Reserved.
//                          6100 Center Drive
//                          Suite 1250
                            ...
                            else
                            {
                                ...
                                if (mac->numLostBeacons == 0)
                                {
                                    ...
                                    clocktype timeSinceLastBeacon
                                        = getSimTime(node) - mac->macBcnRxTime;
           ...
           clocktype wtime =
             ...
             clocktype wtime =
                ...
                * SECOND)
                ...
                mac->mpib.macBeaconTxTime = getSimTime(node);
                mac->macBcnTxTime = getSimTime(node);
                                 ...
                                 M802_15_4_enum mStatus)
{
    ...
    if (step == 0)
    {
                ...
                || (TxOptions > 0x0f))
        {
            ...
            ERROR_ReportWarning(
                    "Data Packet Size after is more"
                    " than maximum size 802.15.4 supports (127 bytes)"
                    " at MAC layer. Packet will be"
                    "Dropped");
            ...
            case INDIRECT_INIT_STEP:
            {
                if (DEBUG)
                {
                    printf("%" TYPES_64BITFMT "d : Node %d: 802.15.4MAC :"
                           ...
                           getSimTime(node), node->nodeId, DstAddr);
                }
                mac->taskP.mcps_data_request_STEP++;
    ...
    switch(step)
    {
        case ASSOC_INIT_STEP:
        {
            ...
            if (DEBUG)
            {
                ...
                getSimTime(node), node->nodeId, CoordAddress);
            }
            Mac802_15_4CsmacaBegin(node, interfaceIndex, 'C');
            break;
        }
                ...
                *((UInt16 *)payld) = mac->mpib.macPANId;
                ...
                if (DEBUG)
                {
                    printf("%" TYPES_64BITFMT "d : Node %d: 802.15.4MAC"
                            ...
                            getSimTime(node), node->nodeId, OrphanAddress);
                }
                 Mac802_15_4CsmacaBegin(node, interfaceIndex, 'c');
    ...
    if (mac->gtsSpec.count > 0)
    {
        ...
        for (i = mac->gtsSpec.count - 1; i >= 0 ; i--)
        {
            delay = (mac->gtsSpec.slotStart[i] * sSlotDuration)
                        - (getSimTime(node) - mac->macBcnTxTime);
            if (delay > 0)
            {
                scheduleTimer = TRUE;
                break;
                                    ...
                                    BOOL frmPhyLayer)
{
     ...
     Float64 totalTransmissionTime = 0;
     Float64 beaconInterval = 0;
     Float64 slotDuration = 0;
     Float64 numPacketsPerSecond = 0;
     M802_15_4SuperframeSpec* sfSpecsToCheck = NULL;
     clocktype referenceBeaconTime = 0;
     M802_15_4GTSSpec* gtsSpecsToCheck = NULL;
    ...
    Float64 t_IFS = 0, t_transacTime = 0;
                ...
                for (i = 0; i < mac->gtsSpec2.count; i++)
                {
                    if (mac->gtsSpec2.recvOnly[i])
                    {
                                ...
                                > mac->gtsSpec2.endTime[i])
                        {
                            ...
                            clocktype delay = 0;
                            if (zigbeeAppInfo->endTime <= getSimTime(node))
                            {
                            ...
                            else
                            {
                                mac->gtsSpec2.endTime[i]
                                    = zigbeeAppInfo->endTime;
                                delay = zigbeeAppInfo->endTime - getSimTime(node);
                            }
                ...
                else
                {
                            ...
                            > MESSAGE_ReturnPacketSize(msg))
                    {
                        ...
                        if (gtsSpecsToCheck->permit
                            ...
                            && zigbeeAppInfo->endTime > getSimTime(node))
                        {
