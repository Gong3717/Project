...
void RoutingPimHandleProtocolEvent(Node* node, Message* msg)
{
    ...
    switch (msg->eventType)
    {
        ...
        case MSG_ROUTING_PimDmNeighborTimeOut:
        {
            ...
            if ((getSimTime(node) >= neighbor->NLTTimer)
            ...
            {
                clocktype delay = neighbor->NLTTimer - getSimTime(node);
                ...
                neighbor->NLTTimer = getSimTime(node) + delay;
            }
            break;
        }
        ...
        case MSG_ROUTING_PimDmPruneTimeoutAlarm:
        {
            ...
            if ((downstreamInfo->isPruned)
                && ((getSimTime(node) - downstreamInfo->lastPruneReceived) >=
                    (downstreamInfo->pruneTimer)))
            {
                ...
                if (rowPtr->state == ROUTING_PIM_DM_PRUNE)
                {
                    ...
                    rowPtr->expirationTimer = getSimTime(node) +
        ...
        case MSG_ROUTING_PimDmAssertTimeOut:
        {
            ...
            if (pim->interface[interfaceIndex].assertOptimization == FALSE)
            {
                ...
                {
                    clocktype delay = assertTime - getSimTime(node);
                    ...
                    assertTime = getSimTime(node) + delay;
        ...
        case MSG_ROUTING_PimDmDataTimeOut:
        {
            ...
            if (getSimTime(node) >= rowPtr->expirationTimer)
            {
                if (DEBUG_TIMER)
                {
                    char srcStr[MAX_STRING_LENGTH];
                ...
                while (listItem)
                {
                    ...
                    if ((downstream->isPruned)
                        && (getSimTime(node)-downstream->lastPruneReceived >=
            ...
            {
                clocktype delay = rowPtr->expirationTimer - getSimTime(node);
                RoutingPimDmDataTimeoutInfo cacheTimer;
                ...
                rowPtr->expirationTimer = getSimTime(node) + delay;
            }
        ...
        case MSG_ROUTING_PimDmScheduleJoin:
        {
            ...
            int interfaceId = joinTimer->interfaceIndex;
            if (rowPtr && rowPtr->delayedJoinTimer)
            {
                ...
                {
                    if (joinTimer->seqNo >= rowPtr->delayedJoinTimerSeqNo)
                    {
                        RoutingPimDmSendJoinPrunePacket(
                                            node,
                                            joinTimer->srcAddr,
        ...
        case MSG_ROUTING_PimSmScheduleCandidateRP:
        {
            ...
            rpTimerInfo.srcAddr = candidateRPTimer->srcAddr;
...
#ifdef DEBUG
    {
        ...
        case MSG_ROUTING_PimSmAssertTimerTimeout:
        {
            ...
            #endif
            if (getSimTime(node) >= downStreamListPtr->assertTime)
            {
            ...
            {
                clocktype delay =
                   downStreamListPtr->assertTime - getSimTime(node);
        ...
        if (!invoke_Sparse)
        {
                ...
                i++)
            {
...
#endif
                {
                    ...
                    rowPtr[i].assertTime = getSimTime(node);
                   ...
                   if (rowPtr[i].inIntf == thisInterface->ipAddress)
                                ...
                                interfaceId)))
                    {
                        ...
                        if (rowPtr[i].state == ROUTING_PIM_DM_PRUNE)
                        {
                            ...
                            /*  Set DataTimeout timer */
                            rowPtr[i].expirationTimer = getSimTime(node);
