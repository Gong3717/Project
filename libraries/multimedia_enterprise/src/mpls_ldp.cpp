    ...
    if (matchHelloPeer == TRUE)
    {
        ...
        while (i < ldp->numHelloAdjacency)
        {
            BOOL removalContition = (helloAdjacency[i].lastHeard <
                 (getSimTime(node) - ldp->keepAliveTime));
    ...
    NodeAddress LSR_Id)
{
    ...
    helloAdjacency->outboundCache[index].exit_time = getSimTime(node);
    helloAdjacency->outboundCacheNumEntries++;
}
    ...
    clocktype helloTimerInterval)
{
    ...
    ldp->helloAdjacency[index].lastHeard = getSimTime(node);
    ...
    NodeAddress LSR_Id)
{
    ...
    helloAdjacency->incomingCache[index].entry_time = getSimTime(node);
    helloAdjacency->incomingCacheNumEntries++;
}
