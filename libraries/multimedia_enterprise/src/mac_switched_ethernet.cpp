    ...
    Message *msgToSend)
{
    ...
    txDelay = (clocktype) (MESSAGE_ReturnPacketSize(msgToSend) * 8 * SECOND
                           / eth->myMacData->bandwidth);
    ...
    timeToClearPipe = eth->busyUntil - getSimTime(destNode);
    ...
    eth->busyUntil = MAX(eth->busyUntil + arrivalTime,
                         getSimTime(destNode) + arrivalTime);

    MESSAGE_Send(destNode, msgToSend, eth->busyUntil - getSimTime(destNode));
