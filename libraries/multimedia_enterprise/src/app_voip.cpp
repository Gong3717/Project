    ...
    VoipHostData* voipHost, Node * node)
{
    ...
    else
    {
        printf ("Node %d calling %s at %" TYPES_64BITFMT
                ...
                node->nodeId, called, node->partitionData->theCurrentTime);
    }
}
#endif
...
void VoipConnectionEstablished(Node* node, VoipHostData* voip)
{
    ...
    voip->sessionEstablished = getSimTime(node);
...
void VoipCloseSession(Node* node, VoipHostData* voip)
{
    ...
    {
        voip->sessionFinish = getSimTime(node);
    ...
    unsigned short callSignalSourcePort)
{
    ...
    while (appList)
    {
        if (appList->appType == APP_VOIP_CALL_INITIATOR)
        {
            ...
            if (voipHost->callSignalAddr == sourceAddr
                && voipHost->callSignalPort == callSignalSourcePort)
            {
                return voipHost;
            }
        }
    ...
    BOOL endSession)
{
    ...
    voipHostData->sessionLastSent = getSimTime(node);

    if (endSession)
    {
        voipHostData->sessionFinish = getSimTime(node);
    ...
    VoipHostData* voipHostData)
{
    ...
    if (DEBUG_VOIP)
    {
        ...
        TIME_PrintClockInSecond(getSimTime(node), currentTimeStr);
    ...
    BOOL initiator)
{
    ...
    if (db != NULL)
    {
        ...
        {
            if (voipHostData->stats->
                GetMessagesReceived().GetValue(getSimTime(node)) == 1)
            {
            ...
            SequenceNumber::Status seqStatus =
                APP_ReportStatsDbReceiveEvent(
                    ...
                    delay,
                    voipHostData->stats->
                        GetJitter().GetValue(getSimTime(node)),
                    MESSAGE_ReturnPacketSize(msg),
                    voipHostData->stats->
                        GetMessagesReceived().GetValue(getSimTime(node)),
                    APP_MSG_NEW);
        }
    }
#endif // ADDON_DB
    ...
    voipHostData->sessionLastReceived = getSimTime(node);
    ...
    clocktype currRtpTs = getSimTime(node);
    if (DEBUG_VOIP_Err)
    {
        char clockStr[MAX_STRING_LENGTH];
    ...
    {
        if (getSimTime(node) >= voipHostData->endTime)
        {
            voipHostData->sessionFinish = getSimTime(node);
            voipHostData->sessionIsClosed = TRUE;
            if (node->appData.appStats)
...
void VoipStartTransmission(Node* node, VoipHostData* voip)
{
    ...
    if (getSimTime(node) >= voip->endTime)
    {
        if (DEBUG_VOIP_Err)
        {
...
void VoipPrintStats(Node* node, VoipHostData* voipHostData)
{
    ...
    if (voipHostData->stats)
    {
        numPktsRecvd = voipHostData->stats->
                        GetMessagesReceived().GetValue(getSimTime(node));
    }
    if (numPktsRecvd == 0)
    {
