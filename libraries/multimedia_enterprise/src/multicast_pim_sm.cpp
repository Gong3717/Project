                                    ...
                                    LocalGroupMembershipEventType event)
{
    ...
    case LOCAL_MEMBER_JOIN_GROUP:
    {
            ...
            == pim->interface[interfaceId].smInterface->drInfo.ipAddr)
        {
            ...
            if (pimDataSm->switchDirectToSPT == TRUE
#ifdef CYBER_CORE
               && defaultBlkGrp == FALSE
#endif
               )
            {
                ...
                srcGrpListItem = pimDataSm->srcGrpList->first;
    ...
    case LOCAL_MEMBER_LEAVE_GROUP:
    {
        ...
        if (treeInfoBaseRowPtr == NULL
            &&
            (pimDataSm->switchDirectToSPT == FALSE
            ))
        {
                                    ...
                                    NodeAddress prevHop)
{
            ...
            incomingInterface) && (sourceInterface == incomingInterface)))
    {
        ...
        treeInfoBaseRowPtr->keepAliveExpiryTimer = getSimTime(node) +
                              ...
                              unsigned int pktSize, int interfaceIndex)
{
    ...
    if (neighbor)
    {
                    ...
                    if (rowPtr->upstreamState == PimSm_JoinPrune_Join)
                    {
                        ...
                        if ((getSimTime(node) + delay) <
                                    rowPtr->lastJoinTimerEnd)
                         {
                            ...
                            rowPtr->joinTimerSeq++;
                            rowPtr->lastJoinTimerEnd = getSimTime(node) +
                                                       delay;
        ...
        neighbor->lastHelloReceive = getSimTime(node);
        ...
        if (holdTime != 0xFFFF)
        {
            neighbor->NLTTimer = getSimTime(node) +
                            (clocktype) holdTime*SECOND;
        }
                          ...
                          RoutingPimSmTreeInfoBaseRow * treeInfoBaseRowPtr)
{
            ...
            && ((unsigned) outIntfId != (unsigned) NETWORK_UNREACHABLE))
    {
        ...
        treeInfoBaseRowPtr->lastJoinPruneSend = getSimTime(node);
    }
        ...
        clocktype joinPruneHoldTime)
{
    ...
    if (treeInfoBaseRowPtr->treeState != ROUTING_PIMSM_SGrpt)
    {
        if (isJoinOrPrune == ROUTING_PIM_JOIN)
        {
            ...
            downstreamListPtr->lastJoinReceived = getSimTime(node);
    ...
    if (isJoinOrPrune == ROUTING_PIM_PRUNE)
    {
                ...
                && (treeInfoBaseRowPtr->treeState != ROUTING_PIMSM_SGrpt))
        {
            ...
            downstreamListPtr->lastPruneReceived = getSimTime(node);
            if (DEBUG_JP)
            {
                char clockStr[100];
                ctoa(getSimTime(node), clockStr);
                                   ...
                                   NodeAddress srcAddr, int incomingIntf)
{
    ...
    if (IamRP)
    {
        ...
        if (( (RoutingPimSmInheritedOutListSG(node, groupAddr,
        ...
        treeInfoBaseRowPtr->keepAliveExpiryTimer = getSimTime(node) + delay;
...
RoutingPimSmPrintTreeInfoBase(Node* node)
{
    ...
    ctoa(getSimTime(node), clockTime);
        ...
        BOOL suppressJoinPrune)
{
    ...
    {
        RoutingPimSmHandleGUpstreamStateMachine(node, srcAddr,
                                                interfaceId, groupAddr,
                                                joinPruneHoldTime,
                                                treeInfoBaseRowPtr,
                                                suppressJoinPrune);
    ...
    {
        RoutingPimSmHandleSGUpstreamStateMachine(node, srcAddr,
                interfaceId, groupAddr,
                joinPruneHoldTime,
                treeInfoBaseRowPtr,
                suppressJoinPrune);
    ...
    case ROUTING_PIMSM_SGrpt:
    {
        ...
        {
        RoutingPimSmHandleSGrptUpstreamStateMachine(node, srcAddr,
                groupAddr,
                joinPruneHoldTime,
                treeInfoBaseRowPtr);
        }
    ...
    if (treeInfoBaseRowPtr->upstreamState == PimSm_JoinPrune_NotJoin)
    {
        ...
        if (RoutingPimSmJoinDesiredG(node, groupAddr, srcAddr, outInterface))
        {
            ...
            if (!suppressJoinPrune)
            {
                ...
                treeInfoBaseRowPtr->lastJoinTimerEnd = getSimTime(node) + delay;
                RoutingPimSetTimer(node,
                                   outInterface,
                                   MSG_ROUTING_PimSmJoinTimerTimeout,
                                   (void*) &joinTimer, delay);
    ...
    else /* inJoin state */
    {
        if (!suppressJoinPrune)
        {
                    ...
                    ROUTING_PIM_BROADCAST_INTERFACE)
            {
                ...
                if (treeInfoBaseRowPtr->joinSeenInUpIntf)
                {
                    ...
                    if ((getSimTime(node) + t_joinsuppress) >
...

        {
                 ...
                 treeInfoBaseRowPtr->lastJoinTimerEnd)
            {
                /* note the latest join timer Seq no */
                treeInfoBaseRowPtr->joinTimerSeq++;
                treeInfoBaseRowPtr->lastJoinTimerEnd = getSimTime(node) +
                                                        delay;
                             ...
                             RoutingPimSmAssertType assertType)
{
    ...
    if ((pim->interface[outIntfId].interfaceType ==
            ROUTING_PIM_BROADCAST_INTERFACE)
            && ((outIntfId >= 0) &&
               ((unsigned)outIntfId != (unsigned)NETWORK_UNREACHABLE)))
    {
                            ...
                            RoutingPimSmMulticastTreeState treeState)
{
    ...
    downStreamListPtr->assertTime = getSimTime(node) + delay;
}
                                  ...
                                  int interfaceIndex)
{
        ...
        ((unsigned)outIntf != (unsigned)NETWORK_UNREACHABLE))
    {
        ...
        pimDataSm ->lastCandidateRPSend = getSimTime(node);
#ifdef ADDON_DB
                       ...
                       int incomingIntf)
{
    ...
    if (bootstrapSent == TRUE)
    {
        ...
        pimSmData->lastBootstrapSend = getSimTime(node);
#ifdef ADDON_DB
                         ...
                         int interfaceIndex)
{
    ...
    if (bootstrapOriginated == TRUE)
    {
        ...
        pimDataSm->lastBootstrapSend = getSimTime(node);
    }
        ...
        if (rowPtr->treeState == ROUTING_PIMSM_G)
        {
            ...
            if (RPOld != RPNew)
            {
               ...
               if (rowPtr->nextHopForRP != nextHopForRPNew)
               {
                   ...
                   rowPtr->lastJoinTimerEnd = getSimTime(node) + delay;
            ...
            if (rowPtr->upstreamState == PimSm_JoinPrune_Join)
            {
                ...
                if (rowPtr->upstream != nextHop)
                {
                    ...
                    if (joinPruneType == ROUTING_PIMSM_G_JOIN_PRUNE)
                    {
                     ...
                     (rowPtr->treeState == ROUTING_PIMSM_G))
                      ||
                      ((pimDataSm->switchDirectToSPT == TRUE) &&
                      (rowPtr->upstreamState == PimSm_JoinPrune_NotJoin) &&
                      (rowPtr->treeState == ROUTING_PIMSM_SG)))
            {
                RoutingPimSmHandleUpstreamStateMachine(node,
