    ...
    BOOL flush)
{
    ...
    if ((thisArea->groupMembershipLSAOriginateTime == 0)
        || ((getSimTime(node) - thisArea->groupMembershipLSAOriginateTime)
    ...
    BOOL flush)
{
    ...
    thisArea->groupMembershipLSAOriginateTime = getSimTime(node);
    ...
    MospfForwardingTableRow*  newEntry)
{
    ...
    for (j = 0; j < node->numberInterfaces; j++)
    {
        ...
        if ((ospf->iface[j].type == OSPFv2_BROADCAST_INTERFACE)
            && (ospf->iface[j].designatedRouterIPAddress
                    != ospf->iface[j].address))
        {
            if (DEBUG)
            {
                printf("Node %u: no check for localMbr for intf %d \n",
                    node->nodeId, j);
                printf("DR %x & I am %x \n",
                    ospf->iface[j].designatedRouterIPAddress,
                    ospf->iface[j].address);
