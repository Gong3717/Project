...

{
    ...
    if (routingProtocolType == ROUTING_PROTOCOL_OSPFv2_EXTERNAL)
    {
...
static void HandleRouteTableUpdate(Node* node, Message* msg)
{
    ...
    NetworkDataIp* ip = (NetworkDataIp*) node->networkData.networkVar;
    distributorProtocolType = ip->rtRedistributeInfo->distributorProtocolType;
               ...
               for (int i = 0; i < node->numberInterfaces; i++)
                {
                    ...
                    routingProtocolType =
                        NetworkIpGetUnicastRoutingProtocolType(node, i, NETWORK_IPV4);
