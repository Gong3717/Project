    ...
    RsvpAllObjects* allObjects)
{
    ...
    ListInsert(node, rsvp->psb, getSimTime(node), newPSB);
    return newPSB;
}
    ...
    NodeAddress nextHop)
{
    ...
    ListInsert(node, rsvp->lsp, getSimTime(node), lsp);
    ...
    RsvpAllObjects* allObjects)
{
    ...
    ListInsert(node, rsb->filtSSList, getSimTime(node), newFiltSS);
    return TRUE;
}
    ...
    RsvpPathStateBlock* cPSB)
{
    ...
    if (!rsbList->size)
    {
        ...
        ListInsert(node, filterSpecList, getSimTime(node), filtSS);
        ...
        ListInsert(node, rsvp->rsb, getSimTime(node), newRSB);
    }
    ...
    RsvpPathStateBlock* rPSB)
{
    ...
    ListInsert(node, rsvp->timerStoreList, getSimTime(node), timerStore);
    ...
    RsvpTeObjectSession* session)
{
    ...
    while (item)
    {
            ...
            && (rsb->session.tunnelId == session->tunnelId))
        {
            ...
            memcpy(newRSB, rsb, sizeof(RsvpResvStateBlock));
            ListInsert(node, rsbList, getSimTime(node), newRSB);
        }
        item = item->next;
    }
    return rsbList;
    ...
    NodeAddress currentPhop)
{
    ...
    if (!isAlreadyExist)
    {
        NodeAddress* phopPtr = (NodeAddress*) MEM_malloc(
            sizeof(NodeAddress));
        *phopPtr = currentPhop;
        ListInsert(node, phopList, getSimTime(node), (void*) phopPtr);
    }
}
                ...
                filtSS->filterSpec->lspId))
        {
            ...
            while (interfaceItem)
            {
                ...
                if (outInterface == OI)
                {
                    ...
                    ListInsert(node, psbList, getSimTime(node), newPSB);
                }
    ...
    unsigned int neighSrcIns)
{
    ...
    ListInsert(node, rsvp->helloList, getSimTime(node), hello);
}
    ...
    RsvpUpcallFunctionType upcallFunctionPtr)
{
    ...
    ListInsert(node, rsvp->sessionList, getSimTime(node), newEntry);
