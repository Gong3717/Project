...
void Ospfv3PrintLSDB(Node* node)
{
    ...
    TIME_PrintClockInSecond(getSimTime(node), timeStringInSecond);
    ...
    BOOL flush)
{
    ...
    {
        delay = (clocktype) (OSPFv3_MIN_LS_INTERVAL - (getSimTime(node)
    ...
    BOOL flush)
{
    ...
    else if ((getSimTime(node) - LSAOriginateTime) >= OSPFv3_MIN_LS_INTERVAL)
    {
        delay = (clocktype) (RANDOM_nrand(ospf->seed) % OSPFv3_BROADCAST_JITTER);
    }
    else
    {
        delay = (clocktype) (OSPFv3_MIN_LS_INTERVAL
                            - (getSimTime(node)- LSAOriginateTime));
    }
    ...
    Ospfv3LinkStateType refLSType)
{
    ...
    else if ((getSimTime(node) - thisArea->interAreaPrefixLSOriginateTime)
            >= OSPFv3_MIN_LS_INTERVAL)
    {
        delay = (clocktype) (RANDOM_nrand(ospf->seed) % OSPFv3_BROADCAST_JITTER);
    }
    ...
    Ospfv3PacketType type)
{
...
#ifdef OSPFv3_DEBUG_ERRORS
    {
        ...
        ctoa(getSimTime(node) + timerDelay, clockStr);
    ...
    int interfaceId)
{
    ...
    {
        nbrInfo->DDSequenceNumber =
            (unsigned int) (getSimTime(node) / SECOND);
    }
    else
    {
        nbrInfo->DDSequenceNumber++;
    ...
    BOOL retx)
{
...
#ifdef OSPFv3_DEBUG
    {
        ...
        TIME_PrintClockInSecond(getSimTime(node), clkStr);
    ...
    char* LSA)
{
    ...
    ListInsert(node, nbrInfo->linkStateRetxList, getSimTime(node), newLSA);
    ...
    BOOL flush)
{
    ...
    thisArea->interAreaPrefixLSOriginateTime = getSimTime(node);
    ...
    BOOL flush)
{
    ...
    if ((LSAItem
        && (getSimTime(node) - LSAItem->timeStamp < OSPFv3_MIN_LS_INTERVAL))
        || (!retVal))
    {
        MEM_free(prefixLSA);
    ...
    BOOL flush)
{
    ...
    Ipv6GetGlobalAggrAddress(node, interfaceId, &info.prefixAddr);
        ...
        (Ospfv3LinkStateHeader* ) (((char* ) LSHeader) + LSHeader->length))
    {
        ...
        if (moreRecentIndicator < 0)
        {
            ...
            else
            {
                ...
                if ((listItem) && (getSimTime(node) - listItem->timeStamp
                    > OSPFv3_MIN_LS_ARRIVAL))
                {
                    Ospfv3SendUpdatePacket(
                        node,
