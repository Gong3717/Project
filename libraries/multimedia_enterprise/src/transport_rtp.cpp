                                                    ...
                                                    Node* node)
{
    ...
    if (srcInfo != NULL)
    {
        ...
        srcInfo->lastActive = getSimTime(node);
        return srcInfo;
    }
    RtpCheckRtpSourceDatabase(session);
                                               ...
                                               Node* node)
{
    ...
    clocktype eventTime = getSimTime(node);
...
//Currently, we support only cname.
    ...
    if (session->ssrcCount < session->ssrcPrevCount)
    {
        ...
        session->nextRtcpPacketSendTime = getSimTime(node);
        session->lastRtcpPacketSendTime = getSimTime(node);
...
void RtcpSendByePacketNow(Node* node, RtpSession* session)
{
    ...
    if ((session->weSentFlag == FALSE) && (session->intialRtcp == TRUE))
    {
        ...
        session->sessionEnd = getSimTime(node);
    ...
    *((unsigned int*) ptr) = session->ownSsrc;
    ...
    APP_UdpSendNewDataWithPriority(
        node,
        (AppType)session->rtcpRemotePort,
        session->localAddr,
     ...
     void*          RTPSendFunc)
{
    ...
    {
        session->localPort = localPort + 1;
    }
    session->rtcpLocalPort = session->localPort + 1;
    ...
    session->sessionStart       = getSimTime(node);
...
//    }
    ...
    Message* sentMsg = APP_UdpSendNewDataWithPriority(
                            node,
                            (AppType)session->remotePort,
                            srcAddr,
                            (short)session->localPort,
                            destAddr,
                            ANY_INTERFACE,
                            packet,
                            dataLength,
    ...
    session->lastRtpPacketSendTime = getSimTime(node);
        ...
        case MSG_APP_FromTransport:
        {
                    ...
                    session, rtpHeader, packetLength))
            {
                                            ...
                                            rtpHeader->seqNum))
                {
                    ...
                    session->totalEndToEndDelay += (getSimTime(node) -
...
void RtpPrintSessionStats(Node* node, RtpSession* session)
{
    ...
    if (session->activeNow)
    {
        ...
        TIME_PrintClockInSecond(getSimTime(node), closeStr);
    }
    else
    {
                              ...
                              Message* msg)
{
    ...
    clocktype arrivalTime = getSimTime(node);
    clocktype rtpTimestamp
                = rtpHeader->timeStamp * MILLI_SECOND;
