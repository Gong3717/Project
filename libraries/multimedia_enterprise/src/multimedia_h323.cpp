                              ...
                              int connectionId)
{
    ...
    if (h323->callSignalState == H323_IDLE)
    {
        ...
        h323->callIdentifier = getSimTime(node);
    ...
    TransportToAppDataReceived *dataRecvd)
{
    ...
    voip = VoipGetReceiverDataForCallSignal(
                    node,
                    initiatorCallSignalTransAddr.ipAddr,
              ...
              unsigned short callSignalPort)
{
    ...
    homeUnicastAdr = NetworkIpGetInterfaceAddress(node, DEFAULT_INTERFACE);

    h225Terminal->terminalCallSignalTransAddr.ipAddr = homeUnicastAdr;
    ...
    if (h323->gatekeeperAvailable)
    {
        H225RasTerminalInit(node, nodeInput, h323, homeUnicastAdr);
    }
