...
void PrintTopologyTable(Node* node)
{
    ...
    ctoa(getSimTime(node) / SECOND, timeStr);
    ...
    int ipProtocolType)
{
    ...
    if (DEBUG_EIGRP_ROUTING)
    {
        ...
        ctoa(getSimTime(node) / SECOND, time);
    ...
    for (i = 0; i < numTos; i++)
    {
        ...
        for (j = 0; j < numEntries; j++)
        {
            if (eigrpRoutingTableEntry[j].feasibleRoute == unreachableEntry)
            {
                ...
                eigrpRoutingTableEntry[j].lastUpdateTime = getSimTime(node);
    ...
    ListItem* newTopologyEntry)
{
    ...
    for (i = 0; i < numTos; i++)
    {
        ...
        if (j == numEntries)
        {
            ...
            newRoute.lastUpdateTime = getSimTime(node);
    ...
    int interfaceIndex)
{
        ...
        ((neighborInfo) && (neighborInfo->state == EIGRP_NEIGHBOR_OFF)))
    {
        ...
        // eigrp->lastUpdateSent = getSimTime(node);
