    ...
    BOOL *PacketWasRouted)
{
    ...
    else if (isPathFound && (getPath->numRouteAddresses == 1))
    {
    ...
    int sessionId)
{
    ...
    // this session
    getPath->sessionAdmittedAt = getSimTime(node);
    ...
    Message*   msgForConnectionRequest)
{
    ...
    if (!ReturnSessionId(node, qospf, info))
    {
        ...
        tempPath->sessionAdmittedAt = getSimTime(node);
