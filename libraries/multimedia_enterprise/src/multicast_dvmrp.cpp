    ...
    switch (msg->eventType)
    {
        ...
        case MSG_ROUTING_DvmrpPeriodUpdateAlarm:
        {
            ...
            dvmrp->routingTable.nextBroadcastTime =
                                   getSimTime(node) + DVMRP_FULL_UPDATE_RATE + delay;
        ...
        case MSG_ROUTING_DvmrpRouteTimeoutAlarm:
        {
            ...
            if ((rowPtr) && (getSimTime(node) - rowPtr->lastModified >
        ...
        case MSG_ROUTING_DvmrpNeighborTimeoutAlarm:
        {
            ...
            if ((getSimTime(node) - neighbor->lastProbeReceive) >=
        ...
        NodeAddress srcAddr, NodeAddress destAddr, int interfaceId)
{
    ...
    #ifdef DEBUG_Route3
    {
        ...
        if ((getSimTime(node) < DOWN_TIME) && (node->nodeId == 4))
        {
            ctoa(getSimTime(node), clockStr);
...
static void RoutingDvmrpInitRoutingTable(Node *node)
{
    ...
    for (i = 0; i < (signed)routingTable->numEntries; i++)
    {
        ...
        routingTable->rowPtr[i].lastModified = getSimTime(node);
        ...
        unsigned char metric, int upstream)
{
    ...
    for (i = 0; i < node->numberInterfaces; i++)
    {
        ...
        if (dvmrp->interface[i].neighborCount != 0)
        {
            ...
            while (count < dvmrp->interface[i].neighborCount)
            {
                ...
                *addr = tempNBInfo->ipAddr;
                ...
                /* Insert to list */
                ListInsert(node, tempItem->dependentNeighbor, getSimTime(node),
                           (void *) addr);
        ...
        /* Now add to list */
        ListInsert(node, *childList, getSimTime(node), (void *) tempItem);
    }
}
        ...
        unsigned char metric, int upstream)
{
    ...
    for (i = 0; i < node->numberInterfaces; i++)
    {
        ...
        /* Now add to list */
        ListInsert(node, tempChildList, getSimTime(node), (void *) tempItem);
    }
        ...
        int size, int interfaceIndex)
{
    ...
    if (!neighbor)
    {
        ...
        neighbor->lastProbeReceive = getSimTime(node);
        ...
        int size, int interfaceIndex)
{
    ...
    while (payloadRowIndex < payloadSize)
    {
        ...
        /* Check if the route is new or had been expired */
        ...
        clocktype delay)
{
    ...
    switch (eventType)
    {
        ...
        case MSG_ROUTING_DvmrpTriggeredUpdateAlarm:
        {
             ...
             * b) Next periodic update is not very soon (> getSimTime(node) + 5s)
             */
                if (rowPtr->triggeredUpdateScheduledFlag == FALSE
                    && routingTable->nextBroadcastTime
                       > getSimTime(node) + DVMRP_TRIGGERED_UPDATE_RATE)
                {
        ...
        RoutingDvmrpChildListItem *child)
{
    ...
    while (tempListItem)
    {
        ...
        /* Insert item to list */
        ListInsert(node, *list, getSimTime(node), (void *) tempNBItem);
        ...
        NodeAddress addr, RoutingDvmrpListActionType type)
{
    ...
    switch (type)
    {
        case ROUTING_DVMRP_INSERT_TO_LIST:
        {
            ...
            ListInsert(node, list, getSimTime(node), (void *) tempNbInfo);
        ...
        NodeAddress srcAddr, int interface, RoutingDvmrpListActionType type)
{
    ...
    switch (type)
    {
        case ROUTING_DVMRP_INSERT_TO_LIST:
        {
            ...
            /* Add item to list */
            ListInsert(node, list, getSimTime(node), (void *) tempItem);
        ...
        NodeAddress srcAddr)
{
    ...
    /* Insert to list */
    ListInsert(node, srcList, getSimTime(node), (void *) tempListItem);
}
        ...
        NodeAddress groupAddr, BOOL *packetWasRouted)
{
    ...
    *packetWasRouted = FALSE;
    ...
    if ((srcPtr->pruneActive) &&
        ((getSimTime(node) - srcPtr->lastDownstreamPruned) < srcPtr->pruneLifetime))
    {
        clocktype pruneTime = srcPtr->pruneLifetime -
                (getSimTime(node) - srcPtr->lastDownstreamPruned);
    ...
    *packetWasRouted = FALSE;
    ...
    if (!retVal)
    {
        ...
        if (!anyDependent)
        {
            ...
            srcPtr->lastDownstreamPruned = getSimTime(node);
        ...
        RoutingDvmrpPrunePacketType *prunePacket, NodeAddress srcAddr, int interfaceIndex)
{
    ...
    while (tempItem)
    {
        ...
        if (dependentNbInfo->addr == srcAddr)
        {
            ...
            dependentNbInfo->lastPruneReceived = getSimTime(node);
    ...
    if (tempItem == NULL)
    {
        ...
        if (!localMember)
        {
            ...
            downstreamInfo->whenPruned = getSimTime(node);
...
//                {
                    ...
                    /* Determine prune lifetime */
                    RoutingDvmrpSetPruneLifetime(srcPtr, getSimTime(node));
...
static void RoutingDvmrpPrintRoutingTable(Node *node)
{
    ...
    timestamp = getSimTime(node) / SECOND;
