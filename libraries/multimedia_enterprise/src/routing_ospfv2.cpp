...
void Ospfv2PrintLSDB(Node* node)
{
    ...
    TIME_PrintClockInSecond(getSimTime(node)+getSimStartTime(node),
        timeStringInSecond);
...
void Ospfv2ScheduleSPFCalculation(Node* node)
{
    ...
    if (ospf->SPFTimer > getSimTime(node))
    {
        return;
    }
    ...
    ospf->SPFTimer = getSimTime(node) + delay;
    ...
    BOOL flush)
{
    ...
    else if ((getSimTime(node) - thisArea->routerLSAOriginateTime) >=
    ...
    else
    {
        delay = (clocktype) (OSPFv2_MIN_LS_INTERVAL
                - (getSimTime(node) - thisArea->routerLSAOriginateTime));
    }
...
void Ospfv2ScheduleNetworkLSA(Node* node, int interfaceId, BOOL flush)
{
    ...
    if ((thisInterface->networkLSAOriginateTime == 0)
        || ((getSimTime(node) - thisInterface->networkLSAOriginateTime) >=
                     OSPFv2_MIN_LS_INTERVAL))
    {
        delay = (clocktype)
                (RANDOM_erand(ospf->seed) * OSPFv2_BROADCAST_JITTER);
    ...
    unsigned int areaId)
{
    ...
    else
    {
        ...
        LSAItem->timeStamp = getSimTime(node);
...
void Ospfv2SendHelloPacket(Node* node, Int32 interfaceIndex)
{
    ...
    // Determine destination address
        ...
        if (getSimTime(node) - lastHelloPrint >= OSPFv2_PRINT_INTERVAL)
        {
            lastHelloPrint = getSimTime(node);
...
//#endif // CYBER_CORE
    ...
    if (getSimTime(node) - lastDDPrint >= OSPFv2_PRINT_INTERVAL)
    {
        lastDDPrint = getSimTime(node);
        char clockStr[MAX_STRING_LENGTH];
        TIME_PrintClockInSecond(getSimTime(node), clockStr);
    ...
    nbrInfo->lastDDPktSent = getSimTime(node);
...
#if COLLECT_DETAILS
    if (getSimTime(node) - lastLSReqPrint >= OSPFv2_PRINT_INTERVAL)
    {
        lastLSReqPrint = getSimTime(node);
        char clockStr[MAX_STRING_LENGTH];
        TIME_PrintClockInSecond(getSimTime(node), clockStr);
        ...
        case S_Exchange:
        {
            ...
            if (isDuplicate)
            {
                ...
                else
                {
                    ...
                    if (getSimTime(node) - lastRetxDDPrint >=
                        OSPFv2_PRINT_INTERVAL)
                    {
                        lastRetxDDPrint = getSimTime(node);
                        char clockStr[MAX_STRING_LENGTH];
                        TIME_PrintClockInSecond(getSimTime(node), clockStr);
        ...
        case S_Full:
        {
            if (isDuplicate)
            {
                ...
                else
                {
                    ...
                    if (getSimTime(node) - nbrInfo->lastDDPktSent <
                        ospf->iface[interfaceIndex].routerDeadInterval)
                    {
                        if (Ospfv2DebugSync(node))
                        {
                      ...
                      nbrInfo->ddReTxCount++;
                            ...
                            OSPFv2_PRINT_INTERVAL)
                        {
                            lastRetxDDPrint = getSimTime(node);
                            char clockStr[MAX_STRING_LENGTH];
                            TIME_PrintClockInSecond(getSimTime(node),
                                                    clockStr);
                            printf("%s Number of ReTx DD bytes sent = %ld\n",
                                   clockStr, numRetxDDBytesSent);
                        }
    ...
    while (listItem)
    {
...
#endif
        {
            ...
            if (Ospfv2CompareLSA(node, LSHeader, listLSHeader) > 0)
            {
                ...
                listItem->timeStamp = getSimTime(node);
    ...
    int interfaceId)
{
    ...
    while (listItem)
    {
        ...
        if ((getSimTime(node)-listItem->timeStamp) <
        ...
        if (sendListItem)
        {
            sendListItem->timeStamp = getSimTime(node);
        }
        else
        {
            Ospfv2InsertToNeighborSendList(nbrInfo->linkStateSendList,
    ...
    if (getSimTime(node) - lastRetxLSUpdatePrint >= OSPFv2_PRINT_INTERVAL)
    {
        lastRetxLSUpdatePrint = getSimTime(node);
        char clockStr[MAX_STRING_LENGTH];
        TIME_PrintClockInSecond(getSimTime(node), clockStr);
    ...
    int LSACount)
{
    ...
    if (getSimTime(node) - lastLSUpdatePrint >= OSPFv2_PRINT_INTERVAL)
    {
        lastLSUpdatePrint = getSimTime(node);
        char clockStr[MAX_STRING_LENGTH];
        TIME_PrintClockInSecond(getSimTime(node), clockStr);
...
#if COLLECT_DETAILS
    if (getSimTime(node) - lastAckPrint >= OSPFv2_PRINT_INTERVAL)
    {
        lastAckPrint = getSimTime(node);
        char clockStr[MAX_STRING_LENGTH];
        TIME_PrintClockInSecond(getSimTime(node), clockStr);
    ...
    while (listItem)
    {
...
#endif
        {
            ...
            if ((!(Ospfv2LsaIsSelfOriginated(node, LSA))) &&
                (getSimTime(node) - listItem->timeStamp) <
    ...
    int interfaceIndex)
{
    ...
    Ospfv2Area* thisArea = Ospfv2GetArea(
                                    node,
                                    ospf->iface[interfaceIndex].areaId);
    unsigned int areaId = ospf->iface[interfaceIndex].areaId;
    Ospfv2LinkStateHeader* LSHeader = NULL;
    Ospfv2LinkStateUpdatePacket* updatePkt = NULL;
    Ospfv2Neighbor* nbrInfo = NULL;
    int count;
                ...
                (((char*) LSHeader) + LSHeader->length))
    {
        ...
        if (moreRecentIndicator < 0)
        {
            ...
            else
            {
                ...
                {
                    if ((sendListItem)
                        && (getSimTime(node) - sendListItem->timeStamp >
                            OSPFv2_MIN_LS_ARRIVAL))
                    {
                        if (Ospfv2DebugFlood(node))
                        {
    ...
    NodeAddress* areaID)
{
    ...
    {
        qualifier = new char[MAX_STRING_LENGTH];
    }
    ...
    if (i == ospfConfigFile->numLines)
    {
        ...
        if (ospfConfigFile)
        {
            errStr = new char[ospfConfigFile->maxLineLen];
        }
        else
        {
            errStr = new char[MAX_STRING_LENGTH];
        }
    ...
    const NodeInput* ospfConfigFile)
{
    ...
    if (ospfConfigFile)
    {
        maxLineLen = ospfConfigFile->maxLineLen;
    }
    else
    {
        maxLineLen = MAX_STRING_LENGTH;
    }
    qualifier         = new char[maxLineLen];
    parameterName     = new char[maxLineLen];
    ipAddressString   = new char[maxLineLen];
    areaIdString      = new char[maxLineLen];
    ...
    BOOL flood)
{
    ...
    thisArea->routerLSAOriginateTime = getSimTime(node);
    ...
    BOOL flush)
{
    ...
    if ((LSAItem && (getSimTime(node) - LSAItem->timeStamp
                        < OSPFv2_MIN_LS_INTERVAL))
        || (!retVal))
    {
        MEM_free(LSA);
    ...
    unsigned int areaId)
{
    ...
    if (item != NULL)
    {
        ...
        item->timeStamp = getSimTime(node);
    ...
    {
        newLSA = Ospfv2CopyLSA(LSA);
        Ospfv2InsertToList(list, getSimTime(node), (void*) newLSA);
        retVal = TRUE;
    }
...
#ifdef ADDON_BOEINGFCS
    NetworkEmptyForwardingTable(node, ROUTING_PROTOCOL_OSPFv2_EXTERNAL);
#endif
        ...
        int interfaceId)
{
    ...
    {
        nbrInfo->DDSequenceNumber = (unsigned int) (getSimTime(node)
                                        / SECOND);
    }
    else
    {
    ...
    Ospfv2PacketType type)
{
    ...
    if (foundPacket)
    {
        ...
        if (type == OSPFv2_LINK_STATE_UPDATE)
        {
            ...
            if (getSimTime(node) - lastRetxLSUpdatePrint >=
                                                    OSPFv2_PRINT_INTERVAL)
            {
                lastRetxLSUpdatePrint = getSimTime(node);
                char clockStr[MAX_STRING_LENGTH];
        ...
        else if (type == OSPFv2_DATABASE_DESCRIPTION)
        {
            ...
            if (getSimTime(node) - lastRetxDDPrint >= OSPFv2_PRINT_INTERVAL)
            {
                lastRetxDDPrint = getSimTime(node);
                char clockStr[MAX_STRING_LENGTH];
                TIME_PrintClockInSecond(getSimTime(node), clockStr);
        ...
        else if (type == OSPFv2_LINK_STATE_REQUEST)
        {
            ...
            if (getSimTime(node) - lastRetxLSReqPrint >=
                                                    OSPFv2_PRINT_INTERVAL)
            {
                lastRetxLSReqPrint = getSimTime(node);
                char clockStr[MAX_STRING_LENGTH];
                TIME_PrintClockInSecond(getSimTime(node), clockStr);
...
void Ospfv2FindShortestPath(Node* node)
{
    ...
    if ((!ospf->nssaCandidateNode
        && (!(node->appData.exteriorGatewayVar && ospf->asBoundaryRouter))
         ) || ospf->nssaAreaBorderRouter)
    {
        // QA-Review Start
    ...
    const NodeInput* ospfConfigFile)
{
    ...
    char* qualifier       = new char[maxLineLen];
    char* parameterName   = new char[maxLineLen];
    char* addressString   = new char[maxLineLen];
    char* maskString      = new char[maxLineLen];
    char* advertiseString = new char[maxLineLen];
    char* areaIdString    = new char[maxLineLen];
    ...
    int interfaceIndex)
{
    ...
    if ((NetworkIpGetUnicastRoutingProtocolType(node, interfaceIndex) !=
        ROUTING_PROTOCOL_OSPFv2)
        || (getSimTime(node) < ospf->staggerStartTime))
    {
        //Trace drop
        ActionData acnData;
    ...
    switch (ospfHeader->packetType)
    {
        ...
        case OSPFv2_ROSPF_DBDG:
        {
            ...
            {
                char clockStr[100];
                ctoa(getSimTime(node)+getSimStartTime(node), clockStr);
                char sourceStr[MAX_ADDRESS_STRING_LENGTH];
                IO_ConvertIpAddressToString(sourceAddr,sourceStr);
    ...
    if (!ospf)
    {
              ...
              (msg->eventType == MSG_STATS_MULTICAST_InsertSummary)))
        {
            ...
            if (msg->eventType == MSG_ROUTING_QospfSetNewConnection)
                sprintf(errStr,
                    "Q-OSPF works only with OSPF routing protocol\n",
                    node->nodeId);
            else
                sprintf(errStr,
        ...
        while (item)
        {
            ...
            if (item->unreachableTimeStamp == (clocktype)0)
            {
                                   ...
                                   LSHeader->advertisingRouter) == FALSE)
                {
                    ...
                    item->unreachableTimeStamp = getSimTime(node);
                }
            }
...
void Ospfv2RemoveStaleDoNotAgeLSA(Node* node, Ospfv2Data* ospf)
{
    ...
    for (listItem = ospf->area->first; listItem; listItem = listItem->next)
    {
        ...
        while (item)
        {
            ...
            if (Ospfv2CheckLSAForDoNotAge(ospf, LSHeader->linkStateAge))
            {
                ...
                if (((getSimTime(node) - item->timeStamp) >=
                        OSPFv2_LSA_MAX_AGE)
                     &&
                    (item->unreachableTimeStamp!=0)
                     &&
                     ((getSimTime(node) - item->unreachableTimeStamp) >=
                            OSPFv2_LSA_MAX_AGE))
                {
                   //Flush this LSA
                    Ospfv2FlushLSA(node, (char*) LSHeader, thisArea->areaID);
    ...
    while (item)
    {
        ...
        {
            //Check for atleast MaxAge Seconds
            if (((getSimTime(node) - item->timeStamp) >= OSPFv2_LSA_MAX_AGE)
                 &&
                (item->unreachableTimeStamp!=0)
                 &&
                 ((getSimTime(node) - item->unreachableTimeStamp) >=
...
void HandleStatsDBOspfSummaryTableInsertion(Node* node)
{
    ...
    char ipAddr[MAX_ADDRESS_STRING_LENGTH];
    Int64 bytesSent = 0;
    double period = (double) getSimTime(node) / SECOND;
    while (traverseNode != NULL)
    {
        if (traverseNode->partitionId != partition->partitionId)
        {
...
void HandleStatsDBOspfAggregateTableInsertion(Node* node)
{
    ...
    for (int i = 1; i < node->partitionData->numPartitions; i++)
    {
                        ...
                        Ospfv2LinkStateType LSType)
{
    ...
    if (LSType == OSPFv2_AS_SCOPE_OPAQUE)
    {
        ...
        else if ((getSimTime(node) - ospf->ASOpaqueLSAOriginateTime) >=
                         OSPFv2_MIN_LS_INTERVAL)
        {
            delay = (clocktype)
                    (RANDOM_erand(ospf->seed) * OSPFv2_BROADCAST_JITTER);
        ...
        {
            delay = (clocktype) (OSPFv2_MIN_LS_INTERVAL
                    - (getSimTime(node) - ospf->ASOpaqueLSAOriginateTime));
        }
