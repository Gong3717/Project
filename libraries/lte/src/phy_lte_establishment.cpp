                                    ...
                                    PropRxInfo* propRxInfo)
{
    ...
    else
    {
        ...
        lte::LteLog::InfoFormat(
            ...
            rxPreamble.arrivalTime     = *(node->currentTime);
                   ...
                   it++)
            {
                if (propRxInfo->txMsg == (*it).txMsg)
                {
                    ...
                    double prachRxPower_dBm
                        = IN_DB((*it).rxPower_mW * PHY_LTE_PRACH_RB_NUM);
#ifdef LTE_LIB_LOG
                                                  ...
                                                  int phyIndex)
{
    ...
    if (selected == TRUE)
    {
        ...
        phyLte->phySelectedTime = *node->currentTime;
        ...
        if (PhyLteIsInStationaryState(node, phyIndex) == true)
        {
            clocktype elapsedTime
                    = *node->currentTime - phyLte->phySelectedTime;
                                ...
                                const LteRaPreamble* preambleInfoFromMac)
{
    ...
    clocktype frameDuration     = PHY_LTE_MAX_NUM_AVAILABLE_SUBFRAME
                                  * subframeDuration;
    clocktype currentTime       = *(node->currentTime);
