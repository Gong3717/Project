                              ...
                              PropTxInfo* propTxInfo) // Just for log
{
    ...
    partOfPacketSize = (double)txBlockSize * numSfTti
        * ((double)(*(node->currentTime) - tbsInfo->rxTimeEvaluated) /
...
#endif
    tbsInfo->rxTimeEvaluated = *(node->currentTime);
                                    ...
                                    BOOL* wasFound)
{
    ...
    if (*wasFound)
    {
        if ((PHY_LTE_MIN_DL_CHANNEL_NO <= dlChannelNo) &&
            (dlChannelNo < node->numberChannels))
        {
            phyLte->dlChannelNo = dlChannelNo;
        }
        else{
            char errorStr[MAX_STRING_LENGTH];
            sprintf(errorStr,
                         "PHY-LTE: Downlink Channel Number "
                    "should be %d to %d",
                    PHY_LTE_MIN_DL_CHANNEL_NO,
    ...
    else
    {
        char warnBuf[MAX_STRING_LENGTH];
        sprintf(warnBuf,
            "Phy-LTE: Downlink Channel Number should be set."
            "Change Downlink Channel Number to %d.",
                                    ...
                                    BOOL* wasFound)
{
    ...
    if (*wasFound)
    {
        if ((PHY_LTE_MIN_UL_CHANNEL_NO <= ulChannelNo) &&
            (ulChannelNo < node->numberChannels))
        {
            phyLte->ulChannelNo = ulChannelNo;
        }
        else{
            char errorStr[MAX_STRING_LENGTH];
            sprintf(errorStr,
                         "PHY-LTE: Uplink Channel Number "
                    "should be %d to %d",
                    PHY_LTE_MIN_UL_CHANNEL_NO,
                    (node->numberChannels - 1));
            ERROR_Assert(FALSE, errorStr);
        }
    }
    else
    {
        char warnBuf[MAX_STRING_LENGTH];
        sprintf(warnBuf,
            "Phy-LTE: Uplink Channel Number should be set."
            "Change Uplink Channel Number to %d.",
                       ...
                       PhyLteState status)
{
    ...
    case PHY_LTE_RA_GRANT_WAITING:
        ...
        phyLte->txState = status;
        phyLte->rxState = status;
// 		if ((string)(node->hostname) == "eNB1")
// 		{
// 			ofstream outfile("E:\\eNB1.txt", ofstream::app);
// 
// 			outfile << LteGetPhyStateString(phyLte->previousTxState)<<endl;
// 			outfile << *(node->currentTime) / (double)SECOND << endl;
// 			outfile << LteGetPhyStateString(phyLte->txState) << endl<<endl;
// 			outfile.close();
// 		}
        break;
                               ...
                               PhyLteConnectedUeInfo* ueInfo)
{
    ...
    ueInfo->connectedTime = getSimTime(node);
...
void PhyLteFinalize(Node* node, const int phyIndex)
{
    ...
    double simTime = getSimTime(node)/(double)SECOND
                                    ...
                                    PropRxInfo* propRxInfo)
{
	...
	//string infoNames;
	//returnAllInfoTypeName(infoNames, propRxInfo->txMsg);
	//cout << infoNames << endl;
    ...
    newRxMsgInfo->rxArrivalTime = *(node->currentTime);
