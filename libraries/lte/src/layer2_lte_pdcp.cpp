    ...
    BOOL isReEstablishment)
{
    ...
    if (valData != NULL)
    {
        ...
        clocktype delay = getSimTime(node) - txInfo->txTime;
        ...
        list<Message*>::iterator& itr)
{
    ...
    if (pdcpEntity->lastPdcpSduSentToNetworkLayerTime < getSimTime(node)) {
        pdcpEntity->lastPdcpSduSentToNetworkLayerTime = getSimTime(node);
