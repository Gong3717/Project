...
#endif
        layer3Data->establishmentStatData.lastStartTime = getSimTime(node);
    ...
    Node* node, UInt32 interfaceIndex, Message* msg)
{
    ...
    clocktype establishmentTime =
        getSimTime(node) - layer3Data->establishmentStatData.lastStartTime;
        ...
        case LTE_STATION_TYPE_UE:
        {
            ...
            else
            {
                sprintf(warnBuf,
                    "RRC-LTE: Waiting RRC_CONNECTED_RECONF Time "
                    "should be set. "
                    "Change Waiting RRC_CONNECTED_RECONF Time to %dmsec.",
                    RRC_LTE_DEFAULT_WAIT_RRC_CONNECTED_TIME/MILLI_SECOND);
                ERROR_ReportWarning(warnBuf);
    ...
    Node* node, int interfaceIndex, const LteLayer3StatData& rrcStat)
{
    ...
    sprintf(buf, "Average count of Retry RRC Connection Establishment "
        "= %.2lf",
