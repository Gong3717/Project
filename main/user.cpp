...
#ifdef UMTS_LIB // not fully implemented to support PHONE CALL
...
static void USER_ReadStatusInfo(Node *node, const NodeInput *nodeInput)
{
    ...
    while (TRUE)
    {
        ...
        if (!retVal)
        {
            sprintf(
                errorString,
                "User Status %s does not have corresponding start time",
                statusName);
...
void USER_SendStatusMsg(Node *node)
{
    ...
    {
        if (node->userData->statusList->startTime < getSimTime(node))
        {
...
void USER_SetApplicationArrival(Node *node)
{
    ...
    TIME_PrintClockInSecond(appArrival+getSimTime(node), clockStr);
    ...
    if (node->userData->behaviorDuration > (appArrival+getSimTime(node)))
    {
...
void USER_HandleCellularRetry(Node *node, int appId)
{
            ...
            node->userData->phoneOn)
    {
        ...
        appInfo->numRetries++;
        appInfo->retry =
            ((getSimTime(node) + retryInterval + appDuration) <
             node->userData->behaviorDuration);

        node->userData->totalRetries++;

        TIME_PrintClockInSecond(retryInterval, clockStr);
        TIME_PrintClockInSecond(retryInterval + getSimTime(node),
                                timeStr);
