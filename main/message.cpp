...
void Message::initialize(PartitionData* partition)
{
    ...
    packetCreationTime    = partition->theCurrentTime;
    subChannelIndex       = -1;
    isPacked              = FALSE;
    actualPktSize         = 0;
...
void MESSAGE_PrintMessage(Message* msg) {
    ...
    int numInfoFields = msg->infoArray.size();
    ...
    for (i = 0; i < numInfoFields; i ++)
    {
        ...
        {
            printf("%d", msg->infoArray[i].info[j]);
        }
        printf("\n");
    }
...
//              the destination message.
// PARAMETERS::
// + node : Node* : Pointer to partition data
// + dsgMsg  :  Message* : Destination message
// + srcMsg  :  Message* : Source message
// RETURN    :: void : NULL
// **/
...
// + originatingProtocol: TraceProtocolType : Protocol allocating this packet
                         ...
                         bool isMT)
{
    ...
    msg->packetCreationTime = partition->theCurrentTime;
                             ...
                             bool mt)
{
    ...
    { // set the rest of the fields
        ...
        newMsg->hdrLength           = tmpMsg->hdrLength;
...
void MESSAGE_Send(Node *node, Message *msg, clocktype delay, bool isMT) {
    ...
    printf("at %lld, p%d, node %d scheduling %d on intf %d, delay is %lld\n",
           getSimTime(node), node->partitionData->partitionId, node->nodeId,
    ...
    if (isMT)
    {
        ...
        msg->eventTime = node->partitionData->theCurrentTime + delay;
#ifdef USE_LOCK_FREE_QUEUE
        node->partitionData->externalLFMsgQ->pushBack(msg);
#else
        {
            // Rich, testing externalLFMsgQ
            if (DEBUG)
            {
    ...
    if (destNode == NULL) {
        ...
        if (DEBUG) {
            printf("at %015" TYPES_64BITFMT "d, partition %d sending "
                   ...
                   getSimTime(node), node->partitionData->partitionId,
...
void MESSAGE_RouteReceivedRemoteEvent(Node*    node,
    ...
    SCHED_InsertMessage(node,
                        msg,
                        msg->eventTime - partitionData->theCurrentTime);
}
    ...
    else
    {
        ...
        for (i = 0; i < NUM_FRAG; ++i)
        {
            ...
            else
            {
                ...
                fragList[i]->packetCreationTime = getSimTime(node);
...
void MESSAGE_SendAsEarlyAsPossible(Node *node, Message *msg)
{
    ...
    {
        if (getSimTime(node) < node->partitionData->safeTime + 1)
        {
            safeDelay = node->partitionData->safeTime + 1 - getSimTime(node);
        }
    }
                        ...
                        clocktype delay)
{
    ...
    if (node->partitionData->theCurrentTime + delay < node->partitionData->safeTime + 1)
    {
        delay = node->partitionData->safeTime + 1 - node->partitionData->theCurrentTime;
    }
...
/* tracing at a time */
/*
            if (getSimTime (node) == 195087716224)
            {
                printf("at %s: Node %03d executing event %03d (layer %03d, protcol %03d) on interface %02d %s\n",
                       clockStr,
                       (int) node->nodeId,
                       (int) msg->eventType,
                       (int) msg->layerType,
