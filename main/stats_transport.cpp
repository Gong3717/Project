...
void STAT_TransportStatistics::AddReceiveFromUpperLayerDataPoints(Node* node, Message* msg)
{
    ...
    timing->receivedTransportUp = getSimTime(node);
}
    ...
    int destPort)
{
    ...
    STAT_Timing* timing = STAT_GetTiming(node, msg);
    timing->sentTransportDown = getSimTime(node);
    ...
    int destPort)
{
    ...
    if (overheadSize > 0)
    {
        addr->m_OverheadBytesReceived.AddDataPoint(getSimTime(node), overheadSize);
        stat->GetOverheadBytesReceived(type).AddDataPoint(getSimTime(node), overheadSize);
    }
    if (dataSize > 0)
    {
        addr->m_DataSegmentsReceived.AddDataPoint(getSimTime(node), 1);
        addr->m_DataBytesReceived.AddDataPoint(getSimTime(node), dataSize);
        stat->GetDataSegmentsReceived(type).AddDataPoint(getSimTime(node), 1);
        stat->GetDataBytesReceived(type).AddDataPoint(getSimTime(node), dataSize);
    }
    else
    {
        addr->m_ControlSegmentsReceived.AddDataPoint(getSimTime(node), 1);
        addr->m_ControlBytesReceived.AddDataPoint(getSimTime(node), controlSize);
        stat->GetControlSegmentsReceived(type).AddDataPoint(getSimTime(node), 1);
        stat->GetControlBytesReceived(type).AddDataPoint(getSimTime(node), controlSize);
    }

    clocktype delay = getSimTime(node) - timing->sentTransportDown;

    addr->m_AverageDelay.AddDataPoint(
        getSimTime(node), (double) delay / SECOND);
    ...
    if (addr->m_LastDelay != -1)
    {
        ...
        addr->m_AverageJitter.AddDataPoint(getSimTime(node), (Float64) diffDelay / SECOND);
    }
    ...
    if (stat->GetLastDelay(type) != -1)
    {
        ...
        stat->GetAverageJitter(type).AddDataPoint(getSimTime(node), (Float64) diffDelay / SECOND);
    }
    ...
    int destPort)
{
    ...
    addr->m_Goodput.AddDataPoint(getSimTime(node), MESSAGE_ReturnPacketSize(msg));
    stat->GetGoodput(type).AddDataPoint(
        getSimTime(node),
        MESSAGE_ReturnPacketSize(msg));
    
    clocktype delay = getSimTime(node) - timing->receivedTransportUp;
    addr->m_AverageDeliveryDelay.AddDataPoint(getSimTime(node), (Float64) delay / SECOND);        
    ...
    if (addr->m_LastDeliveryDelay != -1)
    {
        ...
        addr->m_AverageDeliveryJitter.AddDataPoint(getSimTime(node), (Float64) diffDelay / SECOND);
    }
