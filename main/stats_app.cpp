    ...
    int overheadSize)
{
    ...
    timing->sentApplicationDown = getSimTime(node);
}
    ...
    STAT_DestAddressType type)
{
    ...
    m_AddrStats[type].m_FirstMessageSent.AddDataPoint(
        now, (double) getSimTime(node) / SECOND);
    ...
    STAT_DestAddressType type)
{
    ...
    m_AddrStats[type].m_FirstMessageReceived.AddDataPoint(
        getSimTime(node), (double) now / SECOND);
    ...
    STAT_DestAddressType type)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        clocktype delay = getSimTime(node) - timing->sentApplicationDown;
        clocktype now = getSimTime(node);
        // Add data points
...
void STAT_AppStatistics::AddMessageDroppedDataPoints(
    ...
    STAT_DestAddressType type)
{
    m_AddrStats[type].m_MessagesDropped.AddDataPoint(getSimTime(node), 1);
}

void STAT_AppStatistics::SessionStart(Node* node, STAT_DestAddressType type)
{
    clocktype now = getSimTime(node);
    GetThroughput(type).SessionStart(getSimTime(node));
    GetOfferedLoad(type).SessionStart(getSimTime(node));
    m_AddrStats[type].m_SessionStart.AddDataPoint(
        getSimTime(node), (double) getSimTime(node) / SECOND);
        ...
        if (m_StatsType != STAT_AppReceiver)
        {
            ...
            if (memberCount > 0)
            {
                     ...
                     nodeIt++)
                {
                    ...
                    STAT_MulticastAppSessionStatistics* stats =
                        GetMulticastSession(getSimTime(node),
                                            node->nodeId,
                                            nodeIt->first,
                                            m_SessionId);
                }
