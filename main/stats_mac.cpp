    ...
    int interfaceIndex)
{
    ...
    timing->leftMacOutputQueue = getSimTime(node);
...
{    
    ...
    if (dataSize > 0)
    {
        ...
        GetDataFramesSent(type).AddDataPoint(getSimTime(node), 1);
        GetDataBytesSent(type).AddDataPoint(getSimTime(node), dataSize);
        stat->GetDataFramesSent(type).AddDataPoint(getSimTime(node), 1);
        stat->GetDataBytesSent(type).AddDataPoint(getSimTime(node), dataSize);

        // TODO: This should be considered overhead
        if (controlSize > 0)
        {
            GetControlBytesSent(type).AddDataPoint(getSimTime(node), controlSize);
            stat->GetControlBytesSent(type).AddDataPoint(getSimTime(node), controlSize);        
        }
    }
    else
    {
        // Pure control
        GetControlFramesSent(type).AddDataPoint(getSimTime(node), 1);
        GetControlBytesSent(type).AddDataPoint(getSimTime(node), controlSize);
        stat->GetControlFramesSent(type).AddDataPoint(getSimTime(node), 1);
        stat->GetControlBytesSent(type).AddDataPoint(getSimTime(node), controlSize);        
    }
    ...
    {
        //if uninitialized set it
        timing->sentMacDown = getSimTime(node);
    }
    ...
    int interfaceIndex)
{
    ...
    clocktype delay = getSimTime(node) -
        timing->sentMacDown;
    ...
    if (dataSize > 0)
    {
        GetDataFramesReceived(type).AddDataPoint(getSimTime(node), 1);
        GetDataBytesReceived(type).AddDataPoint(getSimTime(node), dataSize);
        stat->GetDataFramesReceived(type).AddDataPoint(getSimTime(node), 1);
        stat->GetDataBytesReceived(type).AddDataPoint(getSimTime(node), dataSize);

        // TODO: This data should be considered overhead
        if (controlSize > 0)
        {
            GetControlBytesReceived(type).AddDataPoint(getSimTime(node), controlSize);
            stat->GetControlBytesReceived(type).AddDataPoint(getSimTime(node), controlSize);
        }
    }
    else
    {
        GetControlFramesReceived(type).AddDataPoint(getSimTime(node), 1);
        GetControlBytesReceived(type).AddDataPoint(getSimTime(node), controlSize);
        stat->GetControlFramesReceived(type).AddDataPoint(getSimTime(node), 1);
        stat->GetControlBytesReceived(type).AddDataPoint(getSimTime(node), controlSize);
    }

    GetAverageMediumDelay(type).AddDataPoint(getSimTime(node), (Float64) delay / SECOND);
    stat->GetAverageMediumDelay(type).AddDataPoint(getSimTime(node), (Float64) delay / SECOND);
    ...
    if (lastDelay != -1)
    {
        ...
        GetAverageJitter(type).AddDataPoint(getSimTime(node), (Float64) d / SECOND);
        stat->GetAverageJitter(type).AddDataPoint(getSimTime(node), (Float64) d / SECOND);        
    }
    SetLastDelay(delay, type);
    stat->SetLastDelay(delay, type);
    
    GetCarriedLoad().AddDataPoint(getSimTime(node), controlSize + dataSize);
}
    ...
    UInt64 bytes)
{
    ...
    stat->GetFramesDroppedSender().AddDataPoint(getSimTime(node), 1);
    stat->GetBytesDroppedSender().AddDataPoint(getSimTime(node), bytes);
}
    ...
    BOOL isAnyFrame)
{
    ...
    stat->GetFramesDroppedReceiver().AddDataPoint(getSimTime(node), 1);
    stat->GetBytesDroppedReceiver().AddDataPoint(getSimTime(node), bytes);
}
