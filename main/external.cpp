...
clocktype EXTERNAL_QuerySimulationTime(EXTERNAL_Interface *iface)
{
    ...
    return iface->partition->theCurrentTime;
}
...
#ifdef SOCKET_INTERFACE
    if (EXTERNAL_ConfigStringIsYes(nodeInput, "CES-SOCKET") ||
        EXTERNAL_ConfigStringIsYes(nodeInput, "SOCKET-INTERFACE"))
    {
            ...
            switch (msg->eventType)
            {
                case MSG_EXTERNAL_Heartbeat:
                {
                    ...
                    if (list->warmupPhase == EXTERNAL_InWarmup
                        && getSimTime(node) == list->warmupTime)
                    {
    ...
    SimulationProperties* simProps)
{
    ...
    strcpy(defPath, simProps->qualnetHomePath);
    ...
    strcpy(simProps->configFileName, miniConfigurationFile);
}
#endif
    ...
    PartitionData * partitionData)
{
    ...
    // is called.
    if (GUI_EXTERNAL_Bootstrap (argc, argv, nodeInput,
        partitionData->numPartitions, partitionData->partitionId))
    {
        partitionData->interfaceList.isActive = TRUE;
