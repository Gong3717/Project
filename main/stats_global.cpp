...
STAT_StatisticsList::STAT_StatisticsList(PartitionData* partition)
{
    ...
    if (partition->numPartitions > 1)
    {
        ...
        {
            gIncomingData = (volatile STAT_DataList***) MEM_malloc(sizeof(STAT_DataList**) * partition->numPartitions);
            for (int i = 0; i < partition->numPartitions; i++)
            {
                gIncomingData[i] = (volatile STAT_DataList**) MEM_malloc(sizeof(STAT_DataList*) * partition->numPartitions);
                for (int j = 0; j < partition->numPartitions; j++)
                {
    ...
    else
{
    ...
    if (partition->numPartitions > 1)
    {
        gIncomingData = (STAT_DataList**) MEM_malloc(sizeof(STAT_DataList*) * partition->numPartitions);
        for (int i = 0; i < partition->numPartitions; i++)
        {
...
{
    if (partition->theCurrentTime > m_LastAggregateTime)
    {
        m_LastAggregateTime = partition->theCurrentTime;
    }
    else
    {
    ...
    for (unsigned int i = 0; i < m_AggregatedStats.size(); i++)
    {
        ...
        m_AggregatedStats[i]->Aggregate(partition->theCurrentTime);
    ...
    if (partition->numPartitions > 1)
    {
        ...
        for (unsigned i = 0; i < m_AggregatedStats.size(); i++)
        {
            ...
            m_AggregatedStats[i]->Serialize(partition->theCurrentTime, &data, &size);
            if (data != NULL)
            {
    ...
    {
        m_OrderedAggregatedStats[i]->Aggregate(partition->theCurrentTime);
    }
    ...
    if (partition->partitionId == 0)
    {
        for (int i = 0; i < m_AggregatedStats.size(); i++)
        {
            printf("%s = %g (%s)\n",
                ...
                m_AggregatedStats[i]->GetValue(partition->theCurrentTime),
...
void STAT_StatisticsList::SummarizeMulticastApp(PartitionData* partition)
{
    ...
    if (partition->theCurrentTime > multicastSessionSummary.m_LastSummarizeTime)
    {
        multicastSessionSummary.clear();

        multicastSessionSummary.m_LastSummarizeTime = partition->theCurrentTime;
            ...
            ++it)
        {
                 ...
                 sessionIt++)
            {
                 ...
                 summary.InitializeFromModel(
                     getSimTime(partition->firstNode),
                     sessionIt->second,
                     appStats);
...
void STAT_StatisticsList::SummarizeApp(PartitionData* partition)
{
    ...
    if (partition->theCurrentTime > unicastSummary.m_LastSummarizeTime
        || partition->theCurrentTime > multicastSummary.m_LastSummarizeTime)
    {
        // Empty old stats
        unicastSummary.clear();
        multicastSummary.clear();

        unicastSummary.m_LastSummarizeTime = partition->theCurrentTime; 
        multicastSummary.m_LastSummarizeTime = partition->theCurrentTime;
...
void STAT_StatisticsList::SummarizeTransport(PartitionData* partition)
{
    ...
    if (partition->theCurrentTime > transportSummary.m_LastSummarizeTime)
    {
        // Empty old stats
        transportSummary.clear();

        transportSummary.m_LastSummarizeTime = partition->theCurrentTime;
...
void STAT_StatisticsList::SummarizeNet(PartitionData* partition)
{
    ...
    if (partition->theCurrentTime > netSummary.m_LastSummarizeTime)
    {
        netSummary.m_LastSummarizeTime = partition->theCurrentTime;
    }
    else 
    {
        // already summarized. 
...
void STAT_StatisticsList::SummarizeMac(PartitionData* partition)
{
    ...
    if (partition->theCurrentTime > macSummary.m_LastSummarizeTime)
    {
        macSummary.m_LastSummarizeTime = partition->theCurrentTime;
        }
    else 
    {
        // already summarized. 
...
void STAT_StatisticsList::SummarizePhy(PartitionData* partition)
{
    ...
    if (partition->theCurrentTime > phySummary.m_LastSummarizeTime)
    {
        phySummary.m_LastSummarizeTime = partition->theCurrentTime;
    }
    else 
    {
        // already summarized. 
...
void STAT_StatisticsList::SummarizeQueue(PartitionData* partition)
{
    ...
    if (partition->theCurrentTime > queueSummary.m_LastSummarizeTime)
    {
        queueSummary.m_LastSummarizeTime = partition->theCurrentTime;
    }
    else 
    {
        // already summarized. 
