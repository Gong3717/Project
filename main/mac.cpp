    ...
    Address ipAddr)
{
    ...
    if (!macAddrConfigured)
    {
        if (strcmp(macProtocolName, "MAC802.3")==0 ||
		   ...
		   strcmp(macProtocolName, "MAC-FX") == 0||
    ...
    short       subnetIndex)
{
    ...
    if (strcmp(macProtocolName, "MAC802.3") == 0
        ...
        ||strcmp(macProtocolName, "SWITCHED-ETHERNET") == 0
#endif //ENTERPRISE_LIB
		|| strcmp(macProtocolName, "TDMA") == 0//xyt
		|| strcmp(macProtocolName, "GENERICMAC") == 0//xyt
        )
    {
        /* Automatically add directly connected route to forwarding table. */
        NetworkUpdateForwardingTable(
...
//JW
    ...
    if (strncmp(macProtocolName, "FCSC-", 5) == 0) {
		...
		else if (strcmp(phyModelName, "PHY-FX") == 0) {//xyt
			PHY_CreateAPhyForMac(
				node,
				nodeInput,
				interfaceIndex,
    ...
    STATSDB_HandlePhyDescTableInsert(node, interfaceIndex,
		...
		// xyt
		else if (strcmp(macProtocolName, "MAC-FX") == 0) {
			node->macData[interfaceIndex]->macProtocol = MAC_PROTOCOL_FX;
			FxLayer2Init(node, interfaceIndex, nodeInput);
		}
...
MAC_ProcessEvent(Node *node, Message *msg)
{
    ...
    if (MESSAGE_GetEvent(msg) == MSG_MAC_StartFault)
    {
        ...
        node->macData[interfaceIndex]->interfaceDownTime = getSimTime(node);
    ...
    else if (MESSAGE_GetEvent(msg) == MSG_MAC_EndFault)
    {
        ...
        node->macData[interfaceIndex]->interfaceUpTime = getSimTime(node);
   ...
   else if (SRW::Nexus::ConsumeMessage(protocol))
   {
    ...
    switch (node->macData[interfaceIndex]->macProtocol)
    {
		...
		case MAC_PROTOCOL_FX:
		{
			FxLayer2ProcessEvent(node, interfaceIndex, msg);
			break;
		}
...
MAC_NetworkLayerHasPacketToSend(Node *node, int interfaceIndex)
{
    ...
    switch (node->macData[interfaceIndex]->macProtocol)
    {
		...
		case MAC_PROTOCOL_FX:
		{
			FxLayer2UpperLayerHasPacketToSend(node, interfaceIndex, (Layer2DataFx*)node->macData[interfaceIndex]->macVar);
			break;
		}
    ...
    int phyNum)
{
    ...
    case MAC_PROTOCOL_CES_SRW_PORT:
    {
		...
		case MAC_PROTOCOL_FX:
		{
			FxLayer2ReceivePacketFromPhy(node, interfaceIndex,
				(Layer2DataFx*)node->macData[interfaceIndex]->macVar,
				packet);
			break;
		}
...
MAC_Finalize(Node *node)
{
    ...
    case MAC_PROTOCOL_LINK_EPLRS:
    {
        ...
        break;
				...
				case MAC_PROTOCOL_FX:
				{
					FxLayer2Finalize(node, interfaceIndex);
					break;
				}
...
{
    // removed the code that depended on m_IsFault variable
    // since m_IsFault variable has removed.
}
    ...
    BOOL isAnyFrame)
{
    ...
    if (eventType == MAC_SendToPhy)
    {
        ...
        macSummaryInfo->macSummaryPktSendTime = getSimTime(node);
    ...
    BOOL isAnyFrame)
{
    ...
    if (eventType == MAC_SendToPhy)
    {
        ...
        macAggregateInfo->macSummaryPktSendTime = getSimTime(node);
