...
void PARTITION_PrintUsage( const char* commandName ) {
...
#ifdef PARALLEL //Parallel
        "  -loose L                  Loose synchronization does not require that \n"
        "                            parallel threads to synch up exactly.  This \n"
        "                            can provide some additional performance at \n"
        "                            the expense of repeatability.  The loose \n"
        "                            factor: L is a positive integer used in \n"
        "                            determining how far threads are allowed \n"
        "                            to go out of synch. \n"
        "  -greedy                   Opposite of loose. This is the default \n"
    ...
    || productType == PRODUCT_EXATA_CYBER) {
        printf(
...
BOOL PARTITION_ParseArgv( int argc, char **argv,
    ...
    while (thisArg < argc) {
        ...
        if (!strcmp(argv[thisArg], "-seed")) {
...
#ifdef PARALLEL //Parallel
        else if (!strcmp(argv[thisArg], "-loose")) {
            ...
            {
                ERROR_ReportError(
                      "Not enough arguments to -loose\n"
                      "Correct usage:\t -loose <loose-factor>\n");
            } 
        ...
        || !strcmp(argv[thisArg], "-alcesports")) {
            ...
            numberOfProcessors = atoi(argv[thisArg+1]);
            ERROR_Assert(numberOfProcessors >= 1,
            ...
            {
                // Config file name
                strncpy(simProps.configFileName,
                        argv[thisArg], MAX_STRING_LENGTH);
    ...
    clocktype duration)
{
    ...
    PARTITION_COMMUNICATION_SendToAllPartitions(
        partitionData,
        setDurationMessage,
        (partitionData->safeTime - partitionData->theCurrentTime) + 1);
#endif //Parallel
    ...
    if (duration != CLOCKTYPE_MAX)
    {
        ...
        if (duration <= partitionData->theCurrentTime)
        {
            duration = partitionData->theCurrentTime + 1;
        }
    }
    ...
    if (duration < CLOCKTYPE_MAX)
    {
        ...
        MESSAGE_Send(
            partitionData->firstNode,
            heartbeat,
            duration - partitionData->theCurrentTime);
    }
}
    ...
    int             numPartitions)
{
    ...
    partitionData->numNodes = 0;
    partitionData->seedVal = 0;
    partitionData->nodePositions = NULL;
    partitionData->terrainData = NULL;
    partitionData->addressMapPtr = NULL;
    memset(&partitionData->nodeIdHash, 0, sizeof(IdToNodePtrMap) * 32);
    partitionData->safeTime = 0;
    partitionData->nextInternalEvent = 0;
    partitionData->externalInterfaceHorizon = 0;
    partitionData->theCurrentTime = 0;
    partitionData->theCurrentTimeDynamic = 0;
    partitionData->maxSimClock = 0;
    partitionData->startRealTime = 0;
    partitionData->guiOption = FALSE;
    partitionData->interfaceList.isActive = FALSE;
    partitionData->nodeInput = NULL;
    partitionData->nodeData = NULL;
    ...
    partitionData->gridUpdateTime = NULL;
    ...
    partitionData->firstNode = NULL;
    partitionData->firstRemoteNode = NULL;

    partitionData->allNodes = new NodePointerCollection ();

    partitionData->msgFreeListNum = 0;
    partitionData->msgFreeList = NULL;
    partitionData->msgPayloadFreeListNum = 0;
    partitionData->msgPayloadFreeList = NULL;
    partitionData->msgInfoFreeListNum = 0;
    partitionData->msgInfoFreeList = NULL;
    partitionData->splayNodeFreeListNum = 0;
    partitionData->splayNodeFreeList = NULL;
    partitionData->eventSequence = 0;
    memset(&partitionData->heapSplayTree, 0, sizeof(HeapSplayTree));
    partitionData->heapStdlib = NULL;
    memset(&partitionData->mobilityHeap, 0, sizeof(MobilityHeap));
    partitionData->looseEvsHeap = NULL;
    memset(&partitionData->genericEventTree, 0, sizeof(SimpleSplayTree));
    memset(&partitionData->processLastEventTree, 0, sizeof(SimpleSplayTree));
    partitionData->numberOfEvents = 0;
    partitionData->numberOfMobilityEvents = 0;
    partitionData->weatherPatterns = 0;
    partitionData->numberOfWeatherPatterns = 0;
    partitionData->weatherMobilitySequenceNumber = 0;
    partitionData->weatherMovementInterval = 0;
    partitionData->statFd = NULL;
    partitionData->traceEnabled = FALSE;
    partitionData->traceFd = NULL;
    partitionData->activeNode = NULL;
    partitionData->realTimeLogEnabled = FALSE;
    partitionData->realTimeFd = NULL;
    memset(&partitionData->interfaceList, 0, sizeof(EXTERNAL_InterfaceList));
    memset(partitionData->interfaceTable, 0, sizeof(EXTERNAL_Interface*) * EXTERNAL_TYPE_MAX);
    partitionData->externalForwardCommunicator = 0;
    partitionData->externalSimulationDurationCommunicator = 0;
    partitionData->schedulerInfo = NULL;
    partitionData->numAntennaPatterns = 0;
    partitionData->numAntennaModels = 0;
    partitionData->antennaPatterns = NULL;
    partitionData->antennaModels = NULL;
    partitionData->wallClock = new WallClock(partitionData);
    partitionData->userProfileDataMapping = NULL;
    partitionData->trafficPatternMapping = NULL;
    partitionData->isRealTime = FALSE;
    ...
    partitionData->communicators = new PartitionCommunicatorMap ();
    partitionData->communicatorArraySize = 2;
    partitionData->handlerArray = (PartitionCommunicationHandler *)
        MEM_malloc (sizeof (PartitionCommunicationHandler) * 32);
    partitionData->handlerArray [0] = NULL;
    // NOTE, we skip slot 0, because we want an ID of 0 to be invalid
    partitionData->nextAvailableCommunicator = 1;
    // mutex       communicatorsLock;
    partitionData->communicatorsFrozen = false;

    partitionData->clientState = new ClientStateDictionary ();

#ifdef PARALLEL
    partitionData->lookaheadCalculator = new LookaheadCalculator;
    memset(&partitionData->remoteNodeIdHash, 0, sizeof(IdToNodePtrMap) * 32);
    partitionData->reportedEOT = 0;
    partitionData->looseSynchronization = FALSE;
#endif

#ifdef ADDON_NGCNMS
    memset(partitionData->ngcHistFilename, 0, sizeof(char) * MAX_STRING_LENGTH);
    memset(partitionData->ngcStatFilename, 0, sizeof(char) * MAX_STRING_LENGTH);
    partitionData->ngcHistUpdateInterval = 0;
#endif

    memset(&partitionData->subnetData, 0, sizeof(PartitionSubnetData));

#ifdef ADDON_NGCNMS
    partitionData->isLazy = FALSE;
    partitionData->isGreedy = FALSE;
    partitionData->errorBound = FALSE;
#endif
#ifdef ADDON_STATS_MANAGER
    partitionData->statsManagerData = NULL;
#endif
#ifdef ADDON_DB
    partitionData->statsDb = NULL;
#endif
    partitionData->partitionId          = partitionId;
    partitionData->numPartitions        = numPartitions;
    partitionData->looseSynchronization = g_looseSynchronization;
    partitionData->looseEvsHeap = new StlHeap ();

    partitionData->isAgiInterfaceEnabled = FALSE;

    partitionData->dynamicPropDelayNodePositionData = 
        new PARALLEL_PropDelay_NodePositionData;

    // SendMT
    partitionData->sendMTListMutex      = new QNThreadMutex ();
    partitionData->sendMTList           = new std::list <Message *> ();

#ifdef EXATA
    partitionData->virtualLanGateways = new std::map<int, int>;
#endif
    ...
    clocktype       startSimClock)
{
    ...
    if (partitionData->partitionId == 0) {
        ...
        UTIL_GlobalInit(partitionData->numPartitions);
    }
#endif
...
#ifdef EXATA
    NodeInput hostFile;
    char hostStr[MAX_STRING_LENGTH];
    ...
    if (!wasFound)
    {
        ...
        for (int i = 0; i < partitionData->numPartitions; i++)
        {
...
/* Rich-Merge, commented out to simplify correctness checking
        IO_PrintStat(
                minNode,
                (const char *)"",
                (const char *)"",
                ANY_DEST,
                -1, // will not print anything
                "Simulation End Time (seconds) = %s",
                timeStrBuf);
*/
    }    
   ...
   if (partitionData->partitionId == 0)
   {
    ...
    {
        if (partitionData->partitionId == (partitionData->numPartitions - 1))
        {
...
PARTITION_ProcessSendMT (PartitionData * partitionData)
{
    ...
    if (partitionData->sendMTList->size () > 0)
    {
        ...
        clocktype now = partitionData->theCurrentTime;
        clocktype delay;
    ...
    bool printDateTime)
{
    ...
    if (printSimTime)
    {
        ...
        TIME_PrintClockInSecond(partitionData->theCurrentTime +
