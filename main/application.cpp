        ...
        BOOL fromGUI)
{
    ...
    sscanf(cyberInput, "%s", appStr);

    if (fromGUI)
    {
        IO_ConvertStringToUpperCase(appStr);
    }
    ...
    else if (strcmp(appStr, "JAMMER") == 0)
    {
        ...
        char silentString[MAX_STRING_LENGTH];
        NodeAddress sourceNodeId;
        Address sourceAddr;
        NodeAddress destNodeId;
        Address destAddr;
        ...
        if (!success) {
            char errorString[MAX_STRING_LENGTH];
            sprintf(errorString,
                    "Cannot find a node for the jammer: %s", sourceString);
            ERROR_SmartReportError(errorString, !fromGUI);
            return;
        }
        ...
        case 5:
        case 6:
            if (strcmp(silentString, "SILENT") && strcmp(silentString,"silent"))
            {
                char errorString[MAX_STRING_LENGTH];
                sprintf(errorString,
                    "Unrecognized option %s", silentString);
                ERROR_SmartReportError(errorString, !fromGUI);
                return;
            }
            AppJammerStartJamming(
...
void APP_InitializeGuiHitl(Node* node, const NodeInput* nodeInput)
{
    ...
    for (int line = 0; line < hitlInput.numLines; line++)
    {
        ...
        char *command = MESSAGE_InfoAlloc(node, timerMsg, strlen(cmd+1) + 1);
        memcpy (command, cmd + 1, strlen(cmd+1) + 1);

        MESSAGE_Send(node,
                  timerMsg,
                  eventTime - getSimTime(node));
    }
}
        ...
        for (node = firstNode; node; node = node->nextNodeData)
        {
            ...
            if (wasFound) //if File Access is enabled
            {
                ...
                if (Product::GetProductHome(home))
                {
                    ...
                    strncpy(baseDir, fullPath.c_str(), MAX_STRING_LENGTH);
                }
            }

            for (interfaceIndex = 0; interfaceIndex < node->numberInterfaces; interfaceIndex++)
            {
                AppEFtpServerInit(node,
                    MAPPING_GetInterfaceAddressForInterface(node, node->nodeId, NETWORK_IPV4, interfaceIndex),
                    wasFound,
                    baseDir);
            }
            /********************************************
            ...
            {
                snprintf(baseDir, MAX_STRING_LENGTH, "%s/data/webfiles/", Product::GetHomeVariable());
            }

            for (interfaceIndex = 0; interfaceIndex < node->numberInterfaces; interfaceIndex++)
            {
                AppEHttpServerInit(node,
                    MAPPING_GetInterfaceAddressForInterface(node, node->nodeId, NETWORK_IPV4, interfaceIndex),
                    baseDir);
            }
            /********************************************
    ...
    const NodeInput *nodeInput)
{
...
#ifdef JNE_LIB
    JNE_InitializeApplications(firstNode, nodeInput);
#endif
    ...
    if (retVal == TRUE)
    {
        for (i = 0; i < appInput.numLines; i++)
        {
            ...
            if (firstNode->networkData.networkProtocol == IPV6_ONLY)
            {
                    ...
                    && strcmp(appStr, "SUPER-APPLICATION") !=0)
                {
                    ...
                    sprintf(buf,
                        ...
                        "JAMMER, SUPER-APPLICATION and TRAFFIC-GEN \n"
            ...
            char appNameTempStore[MAX_STRING_LENGTH];
            Int32 appNameLen = 0;

            // first clean up the storage buffer
            memset(appNameTempStore, 0, MAX_STRING_LENGTH);
            ...
            if (appNamePtr)
            {
                ...
                memcpy(appNameTempStore, appNamePtr, appNameLen);
                appNameTempStore[appNameLen] = '\0';
            }
            ...
            if (strcmp(appStr, "CBR") == 0)
            {
                ...
                {
                    GUI_CreateAppHopByHopFlowFilter(TRACE_CBR, appStr,
                        sourceString, destString, destString);
                }
            ...
            if (strcmp(appStr, "MCBR") == 0)
            {
                ...
                if (GUI_IsAppHopByHopFlowEnabled())
                {
                    ...
                    {
                        sprintf(destAddrString, "%u", destNodeId);
                    }
                    else
                    {
                        sprintf(destAddrString, "%u",
                            destAddr.interfaceAddr.ipv4);
                    }
                    GUI_CreateAppHopByHopFlowFilter(TRACE_MCBR, appStr,
                        sourceString, destAddrString, destString);
                }
    ...
    //EndSuperApplication
#ifdef ADDON_BOEINGFCS
            else if (strcmp(appStr, "CNR-VOICE") == 0)
            {
                UInt16 callId;
                char sourceStr[MAX_STRING_LENGTH];
                char startTimeStr[MAX_STRING_LENGTH];
                char durationStr[MAX_STRING_LENGTH];
                char callTypeStr[MAX_STRING_LENGTH];
                int itemSize;
                int callPriority;
                NodeId srcId;
                BOOL isNodeId;
                ...
                if (numValues != 6)
                {
                    char errorString[MAX_STRING_LENGTH];
                    sprintf(errorString,
                            "Incorrect CNR-VOICE configuration format!\n"
                            "CNR-VOICE <src> <call id> <start time> <duration> <call type: LOCAL | GLOBAL> <call priority>\n");
                    ERROR_ReportError(errorString);
                }
                int callType;
                if (strcmp(callTypeStr,"LOCAL") == 0)
                {
                    callType = 1; //Local call
                }
                else if (strcmp(callTypeStr,"GLOBAL") == 0)
                {
                    callType = 0; //Global call
                }
                else
                {
                    char errorString[MAX_STRING_LENGTH];
                    sprintf(errorString,"CNR Voice Client: Call Type should be either LOCAL or GLOBAL");
                    ERROR_ReportError(errorString);
                }
                ...
                int interfaceIndex =
                    MAPPING_GetInterfaceIndexFromInterfaceAddress(firstNode, srcAddr);

                node = MAPPING_GetNodePtrFromHash(nodeHash, srcId);
                if (node != NULL)
                {

                    if (!CnrVoiceServicesIsRunningOnSRW(node, interfaceIndex))
                    {
                        char errorString[MAX_STRING_LENGTH];
                        sprintf(errorString,"CNR Voice Services can only run on SRW!");
                        ERROR_ReportError(errorString);
                    }
                    clocktype startTime = TIME_ConvertToClock(startTimeStr);
                    clocktype duration = TIME_ConvertToClock(durationStr);
...
#ifdef DEBUG
                    char clockStr[MAX_CLOCK_STRING_LENGTH];
                    printf("Starting CNR Voice client with:\n");
                    printf("  call Id:    %u\n", callId);
                    printf("  src nodeId:    %u\n", srcId);
                    ctoa(startTime, clockStr);
                    printf("  start time:    %s\n", clockStr);
                    ctoa(duration, clockStr);
                    printf("  duration:      %s\n", clockStr);
                    printf("  call type:           %s\n", callTypeStr);
                    printf("  call priority:        %d\n", callPriority);
#endif /* DEBUG */
                    CnrVoiceServicesClientInit(node, interfaceIndex, nodeInput,
                                            (UInt16)callId, srcId, startTime,
                                            duration, (CALL_TYPE) callType,
                                            callPriority);
                }
    ...
    //InsertPatch APP_INIT_CODE
            ...
            if (appNamePtrLoc
                ...
                && (strcmp(appStr, "THREADED-APP") != 0))
            {
                memcpy(appNamePtrLoc, appNameTempStore, appNameLen);
            }
        }
    }
#ifdef CYBER_LIB
...
void APP_ProcessEvent(Node *node, Message *msg)
{
    ...
    switch(protocolType)
    {
...
#ifdef ADDON_BOEINGFCS
        case APP_SRW_CNR_VOICE_CLIENT:
        {
            CnrVoiceServicesHandleTimerEvent(node, msg);
            break;
        }
#endif /* ADDON_BOEINGFCS */
        case APP_FORWARD:
        ...
        /* Fall through */
        case APP_JNE_MONITORING_AGENT:
        /* Fall through */
        case APP_JNE_JWNM_PRAGENT:
        /* Fall through */
        case APP_JNE_AUDIT_APPLICATION:
        /* ---> Add other JNE cases here <--- */

            // Dispatch event to JNE event processor
            //
            JNE_ProcessEvent(node, msg);
            break;
#endif /* JNE_LIB */
        ...
        default:
        {
            ...
            if (ip->isIcmpEnable && icmp->portUnreachableEnable && udp)
            {
               ...
               HandleStatsDBAppDropEventsInsertion(node,msg,
                "AppDrop",-1, MESSAGE_ReturnPacketSize(msg), "Unknown Application");
#endif
...
APP_Finalize(Node *node)
{
    ...
    for (appList = node->appData.appPtr; appList != NULL; appList = nextApp)
    {
        switch (appList->appType)
        {
            ...
            case APP_SRW_CNR_VOICE_CLIENT:
            {
                CnrVoiceServicesClientFinalize(node, appList);
                break;
            }
            case APP_SRW_CNR_VOICE_SERVER:
            {
                CnrVoiceServicesServerFinalize(node, appList);
                break;
            }
#endif
