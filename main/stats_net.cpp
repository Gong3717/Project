    ...
    BOOL isData)
{
    ...
    clocktype delay = getSimTime(node) - timing->sourceSentNetworkDown;
    
    m_AverageDelay.AddDataPoint(getSimTime(node), (double) delay / SECOND);
    
    if (isData)
    {
        m_AverageDataHopCount.AddDataPoint(getSimTime(node), timing->hopCount);
    }
    else
    {
        m_AverageControlHopCount.AddDataPoint(getSimTime(node), timing->hopCount);
    }
    ...
    if (m_LastDelay[interfaceIndex] != -1)
    {
        ...
        m_AverageJitter.AddDataPoint(getSimTime(node), (Float64) d / SECOND);
    }
    ...
    BOOL isForward)
{
    ...
    if (!isForward)
    {
        if (timing->sourceSentNetworkDown == -1)
        {
            timing->sourceSentNetworkDown = getSimTime(node);
        }
    }
    ...
    int interfaceIndex)
{
    ...
    clocktype delay = getSimTime(node) - timing->sourceSentNetworkDown;
    clocktype unicastDelay = getSimTime(node) - timing->sourceSentNetworkDown;
    clocktype multicastDelay = getSimTime(node) - timing->sourceSentNetworkDown;
    clocktype broadcastDelay = getSimTime(node) - timing->sourceSentNetworkDown;

    if (type == STAT_Unicast)
    {
        m_AverageDelayUnicast.AddDataPoint(getSimTime(node),
                                             (double) unicastDelay / SECOND);
    }
    else if (type == STAT_Multicast)
    {
        m_AverageDelayMulticast.AddDataPoint(getSimTime(node),
                                           (double) multicastDelay / SECOND);
    }
    else if (type == STAT_Broadcast)
    {
        m_AverageDelayBroadcast.AddDataPoint(getSimTime(node),
    ...
    if (type == STAT_Unicast)
    {
        if (m_LastUnicastDelay[interfaceIndex] != -1)
        {
            ...
            m_AverageJitterUnicast.AddDataPoint(getSimTime(node), (Float64) d / SECOND);
        }
    ...
    if (type == STAT_Multicast)
    {
        if (m_LastMulticastDelay[interfaceIndex] != -1)
        {
            ...
            m_AverageJitterMulticast.AddDataPoint(getSimTime(node), (Float64) diffDelay / SECOND);
        }
    ...
    if (type == STAT_Broadcast)
    {
        if (m_LastBroadcastDelay[interfaceIndex] != -1)
        {
            ...
            m_AverageJitterBroadcast.AddDataPoint(getSimTime(node), (Float64) diffDelay / SECOND);
        }
    ...
    if (m_LastDelay[interfaceIndex] != -1)
    {
        ...
        m_AverageJitter.AddDataPoint(getSimTime(node), (Float64) diffDelay / SECOND);
    }
    m_LastDelay[interfaceIndex] = delay;
}
