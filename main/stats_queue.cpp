    ...
    const clocktype currentTime)
{
    ...
    m_PacketsEnqueued.AddDataPoint(getSimTime(m_Node), 1);
    m_BytesEnqueued.AddDataPoint(getSimTime(m_Node),
        MESSAGE_ReturnPacketSize(msg));
    m_PeakQueueLength.AddDataPoint(getSimTime(m_Node),
    ...
    const clocktype currentTime)
{
    m_AverageQueueLength.AddDataPoint(currentTime,
        totalBytesInQueue);
    m_PacketsDequeued.AddDataPoint(getSimTime(m_Node), 1);
    m_BytesDequeued.AddDataPoint(getSimTime(m_Node),
        MESSAGE_ReturnPacketSize(msg));    
}
    ...
    bool fromAging)
{
    ...
    if (!fromAging)
    {
        m_PacketsDroppedForcefully.AddDataPoint(getSimTime(m_Node), 1);
        m_BytesDroppedForcefully.AddDataPoint(getSimTime(m_Node),
            MESSAGE_ReturnPacketSize(msg));
    }
    else
    {
        m_PacketsDroppedForcefullyFromAging.AddDataPoint(getSimTime(m_Node), 1);
    }    
}
    ...
    const clocktype currentTime)
{
    m_AverageQueueLength.AddDataPoint(currentTime,
        totalBytesInQueueBeforeOperation);
    m_PacketsDropped.AddDataPoint(getSimTime(m_Node), 1);
    m_BytesDropped.AddDataPoint(getSimTime(m_Node),
        MESSAGE_ReturnPacketSize(msg));
}
...
void STAT_QueueStatistics::DelayDetected(clocktype delay)
{
    if (delay > 0)
    {
        m_totalDelays.AddDataPoint(getSimTime(m_Node), ((double) delay / SECOND));
        m_LongestTimeInQueue.AddDataPoint(getSimTime(m_Node), ((double) delay / SECOND));
    }
}
