...
void AutoIPNE_SetVirtualNode(
    ...
    char* value3)
{
    NodeAddress physicalAddress;
    NodeAddress    virtualAddress;
    ...
    StringToAddress(value2, &virtualAddress);
    StringToAddress(value3, &physicalAddress);

    physicalAddress = htonl(physicalAddress);
    virtualAddress = htonl(virtualAddress);
    ...
    int data)
{
    ...
    *(int *)MESSAGE_ReturnPacket(msg) = data;
    ...
    for (int i = 1; i < partitionData->numPartitions; i++)
    {
...
void GUI_HandleHITLInput(const char *args, PartitionData *partition)
{
    ...
    {
        int attackerNodeId, victimNodeId;
            ...
            switch (command.type) {
                case GUI_STEP:
                {
                    ...
                    if (partitionData->isRealTime)
                    {
                        ...
                        // any time.
                        nextEventTime = partitionData->theCurrentTime;
                    }
                    else
                    {
                    ...
                    if (nextEventTime < CLOCKTYPE_MAX - data->guiStepSize)
                    {
                        ...
                        else if (nextEventTime > iface->horizon)
                        {
                            // make sure the horizon advances past the
                            // next event
                            iface->horizon = nextEventTime + 1;
                        }
                    }
                    else
                    {
...
void GuiVisObjCmd(GuiVisObjCommands cmd, clocktype time, const char* text)
{
    ...
    clocktype   time)
{
    ...
    GuiVisObjCmd(GUI_ADD_FILTER, time, buffer);
}
...
void GUI_DeleteObjects(const char* id, clocktype time)
{
    char buffer[GUI_MAX_COMMAND_LENGTH];
    sprintf(buffer, "\"%s\"", id);
    GuiVisObjCmd(GUI_DELETE_OBJECTS, time, buffer);
}
    ...
    clocktype       time)
{
    ...
    GuiVisObjCmd(GUI_DRAW_LINE, time, buffer);
}
    ...
    clocktype   time)
{
    ...
    GuiVisObjCmd(GUI_DRAW_FLOW_LINE, time, buffer);
}
    ...
    clocktype       time)
{
    ...
    GuiVisObjCmd(GUI_DRAW_SHAPE, time, buffer);
}
    ...
    clocktype       time)
{
    ...
    sprintf(buffer, "\"%s\" %u %d %d \"%s\"", id, node, interfaceIndex,
        order, text);
    GuiVisObjCmd(GUI_DRAW_TEXT, time, buffer);
}
...
{
    stringstream i;
    i << appTraceProtocolType << '_' << src << '_' << dst;
    ...
    if (it != g_appHopByHopFlowColors.end())
    {
        ...
        GUI_DrawFlowLine(hop, nextHop, it->second.c_str(), filter.c_str(),
            g_appHopByHopFlowFilters[i.str()].c_str(), 3, 0xffff, 1, 10,
            time);
    }
}
    ...
    const char* dstLabel)
{
    ...
    // Select a unique color for the app_src_dst
    text << appTraceProtocolType << '_' << src << '_' << dst;
    filter = text.str();
