...
{
    // Add data points
    m_SignalsTransmitted.AddDataPoint(getSimTime(node), 1);
    m_TotalTransmitTime.AddDataPoint(getSimTime(node), (double) duration / SECOND);
    m_Utilization.AddDataPoint(getSimTime(node), (double) duration / SECOND);
}

void STAT_PhyStatistics::AddSignalDetectedDataPoints(Node* node)
{
    // Add data points
    m_SignalsDetected.AddDataPoint(getSimTime(node), 1);
}
    ...
    double power_mW)
{
    ...
    m_SignalsLocked.AddDataPoint(getSimTime(node), 1);
    m_AverageSignalPower_dBm.AddDataPoint(getSimTime(node), power_mW);
    ...
    BOOL deleteMsg)
{
    ...
    // Add data points
    m_SignalsWithErrors.AddDataPoint(getSimTime(node), 1);
    ...
    if (phyMode == PHY_LOCKING)
    {
        ...
        duration = getSimTime(node) - m_LockTime;

        m_TotalReceiveTime.AddDataPoint(getSimTime(node),
                                       (double)duration / SECOND);
        m_Utilization.AddDataPoint(getSimTime(node),
                                   (double)duration / SECOND);
        m_LockTime = CLOCKTYPE_MAX;
    }
    else if (phyMode == PHY_NOLOCKING)
    {
        ...
        if (msgIter != signalLockMsgList->end())
        {
            ...
            {
                // do nothing in case of 'm_LockTime == getSimTime(node)'
    ...
    {
        m_AverageInterference_dBm.AddDataPoint(getSimTime(node),
                                               interference_mW);
    }
...
{
    // Add data points
    m_SignalsInterferenceErrors.AddDataPoint(getSimTime(node), 1);
}
    ...
    BOOL deleteMsg)
{
    ...
    m_SignalsToMac.AddDataPoint(getSimTime(node), 1);
    m_AveragePathloss_dB.AddDataPoint(getSimTime(node), pathloss_dB);
    ...
    clocktype duration)
{
    ...
    if (endSignal)
    {
        ...
        if (phyMode == PHY_LOCKING)
        {
            ...
            sess->utilization += getSimTime(node) - sess->lastSignalStartTime;
        }
        else if (phyMode == PHY_NOLOCKING)
        {
    ...
    else
    {
        if (phyMode == PHY_LOCKING)
        {
            ...
            sess->lastSignalStartTime = getSimTime(node);
        }
    }
}
